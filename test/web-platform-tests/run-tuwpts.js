...
const toRunFilename = path.resolve(__dirname, "tuwpt-to-run.yaml");
...
function checkToRun() {
  ...
  for (const doc of toRunDocs) {
    ...
    if (!fs.existsSync(path.resolve(__dirname, "to-upstream", doc.DIR))) {
      throw new Error(`The directory "${doc.DIR}" does not exist`);
    }

    if (doc.DIR < lastDir) {
      throw new Error(`Bad lexicographical directory sorting in tuwpt-to-run.yaml: ${doc.DIR} should come before ${lastDir}`);
    }
    lastDir = doc.DIR;
