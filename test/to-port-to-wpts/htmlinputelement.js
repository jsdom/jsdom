...
describe("htmlinputelement", () => {
  ...
  specify(
    "the property valueAsNumber of an input of type number should parse value when possible",
    () => {
      const doc = (new JSDOM(`<input id="input" type="number" />`)).window.document;
      const inputEl = doc.getElementById("input");
      assert.isNaN(inputEl.valueAsNumber);

      inputEl.valueAsNumber = 0;
      assert.strictEqual(inputEl.value, "0");
      assert.strictEqual(inputEl.valueAsNumber, 0);

      inputEl.value = "10";
      assert.strictEqual(inputEl.value, "10");
      assert.strictEqual(inputEl.valueAsNumber, 10);
      ...
      inputEl.value = "1e6";
      assert.strictEqual(inputEl.value, "1e6");
      assert.strictEqual(inputEl.valueAsNumber, 1000000);

      inputEl.value = "-2.3";
      assert.strictEqual(inputEl.value, "-2.3");
      assert.approximately(inputEl.valueAsNumber, -2.3, 1e-5);
    }
  );

  specify(
    "the property valueAsNumber of an input of type time should parse value when possible",
    () => {
      const doc = (new JSDOM(`<input id="input" type="time" />`)).window.document;
      const inputEl = doc.getElementById("input");
      assert.isNaN(inputEl.valueAsNumber);

      inputEl.valueAsNumber = 0;
      assert.strictEqual(inputEl.value, "00:00");
      assert.strictEqual(inputEl.valueAsNumber, 0);
      ...
      inputEl.value = "10:00";
      assert.strictEqual(inputEl.value, "10:00");
      assert.strictEqual(inputEl.valueAsNumber, 36000000);

      inputEl.value = "10:00";
      assert.strictEqual(inputEl.value, "10:00");
      assert.strictEqual(inputEl.valueAsNumber, 36000000);

      inputEl.valueAsNumber = -5000;
      assert.strictEqual(inputEl.value, "23:59:55.000");
      assert.strictEqual(inputEl.valueAsNumber, 86395000);

      inputEl.value = "22:59:23.123";
      assert.strictEqual(inputEl.value, "22:59:23.123");
      assert.strictEqual(inputEl.valueAsNumber, 82763123);
    }
  );
});
