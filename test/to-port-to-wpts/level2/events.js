...
describe("level2/events", () => {
  ...
  test('create event with each event type', () => {
    var doc = require('../level1/core/files/hc_staff.xml').hc_staff(),
  ...
  describe('dispatch event', () => {
    ...
    test('a null reference passed to dispatchEvent', () => {
      var doc = this.doc;
    ...
    test('a created but not initialized event passed to dispatchEvent', () => {
      var doc = this.doc,
    ...
    test('EventListener with capture false', () => {
    ...
    test('EventListener with capture true', () => {
      var monitor = new EventMonitor();
    ...
    test('EventListener is registered twice', () => {
      var monitor = new EventMonitor();
      this.doc.addEventListener("foo", monitor.handleEvent, false);
      this.doc.addEventListener("foo", monitor.handleEvent, false);
      var event = this.doc.createEvent("Events");
    ...
    test('EventListener is registered twice, removed once', () => {
      var monitor = new EventMonitor();
      this.doc.addEventListener("foo", monitor.handleEvent, false);
      this.doc.addEventListener("foo", monitor.handleEvent, false);
    ...
    test('EventListener is registered, other listeners (similar but not identical) are removed', () => {
      var monitor = new EventMonitor();
    ...
    test('two EventListeners which both handle by unregistering itself and the other', () => {
      // setup
      var es = [];
      var ls = [];
  ...
  describe('init event', () => {
    ...
    test('set state from params, bubble no cancel', () => {
    ...
    test('set state from params, cancel no bubble', () => {
      this._events.forEach(function(event){
        assert.notEqual(event, null, 'event should not be null for' + event.eventType);
    ...
    test('initEvent called multiple times, final time is definitive', () => {
      this._events.forEach(function(event){
        assert.notEqual(event, null, 'event should not be null for ' + event.eventType);
        // rotate
  ...
  describe('capture event', () => {
    ...
    test('all capturing listeners in a direct line from dispatched node will receive the event', () => {
    ...
    test('only capture listeners in a direct line from target to the document node should receive the event', () => {
      var self = this;
  ...
  describe('bubble event', () => {
    ...
    test('all non-capturing listeners in a direct line from dispatched node will receive a bubbling event', () => {
    ...
    test('only bubble listeners in a direct line from target to the document node should receive the event', () => {
      this.win.addEventListener("foo", this.monitor.handleEvent, false);
    ...
    test('if an event does not bubble, bubble listeners should not receive the event', () => {
      this.win.addEventListener("foo", this.monitor.handleEvent, false);
  ...
  describe('stop propagation', () => {
    ...
    test('should prevent the target from receiving the event', () => {
      this.win.addEventListener("foo", this.monitor.handleEvent, false);
    ...
    test('should prevent all listeners from receiving the event', () => {
      this.win.addEventListener("foo", this.monitor.handleEvent, false);
    ...
    test('stopPropagation should not prevent listeners on the same element from receiving the event', () => {
      this.win.addEventListener("foo", this.monitor.handleEvent, false);
      this.body.addEventListener("foo", this.monitor.handleEvent, false);
      this.plist.addEventListener("foo", this._handleEvent('stopPropagation'), true);
      this.plist.addEventListener("foo", this._handleEvent('stopPropagation'), false);
  ...
  describe('prevent default', () => {
    ...
    test('the defaultPrevented flag is set when the event is prevented', () => {
    ...
    test('the defaultPrevented flag is not set when the event is not prevented', () => {
      this.title.addEventListener("foo", this.monitor.handleEvent, false);
      var return_val = this.title.dispatchEvent(this.event);
      assert.equal(this.event.defaultPrevented, false, 'the defaultPrevented flag should be false when the event is not prevented');
    });

    test('a cancelable event can have its default event disabled', () => {
      this.body.addEventListener("foo", this.monitor.handleEvent, true);
      this.plist.addEventListener("foo", this._handleEvent('preventDefault'), false);
      this.plist.firstChild.addEventListener("foo", this.monitor.handleEvent, false);
      var return_val = this.plist.firstChild.dispatchEvent(this.event);
    ...
    test('a non-cancelable event cannot have its default event disabled', () => {
      this.body.addEventListener("foo", this.monitor.handleEvent, true);
      this.plist.addEventListener("foo", this._handleEvent('preventDefault'), false);
      this.plist.firstChild.addEventListener("foo", this.monitor.handleEvent, false);
      this.event.initEvent("foo",true,false);
  ...
  test('remove listener in handler', () => {
