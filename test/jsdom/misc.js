...
describe("jsdom/miscellaneous", () => {
  ...
  specify("jsdom_takes_html", () => {
    ...
    assert.equal(document.getElementById("test").getAttribute("href"), '#test', 'Passing html into jsdom() should populate the resulting doc');
  });

  specify("jsdom_empty_html", () => {
    const emptyDoc = jsdom.jsdom('');
    const blankDoc = jsdom.jsdom(' ');
    assert.equal(emptyDoc.innerHTML, blankDoc.innerHTML, 'Passing blank and empty strings into jsdom() result in the same html');
  });
  ...
  specify("jquerify_file", {skipIfBrowser: true, async: true}, t => {
  ...
  specify("jquerify_attribute_selector_gh_400", {skipIfBrowser: true, async: true}, t => {
    const window = jsdom.jsdom().defaultView;
    ...
    jsdom.jQueryify(window, 'file:' + path.resolve(__dirname, '../jquery-fixtures/jquery-1.11.0.js'), function () {
      try {
        ...
        console.log(ex);
        assert.ok(false, "Encountered an error");
      }
      finally {
        t.done();
      }
  ...
  specify("env_with_compression", {skipIfBrowser: true, async: true}, t => {
  ...
  specify("env_with_features_and_external_resources", {skipIfBrowser: true, async: true}, t => {
    jsdom.env(
  ...
  specify("ensure_scripts_can_be_disabled_via_options_features", {skipIfBrowser: true, async: true}, t => {
  ...
  specify("ensure_scripts_can_be_executed_via_options_features", {skipIfBrowser: true, async: true}, t => {
    const html = "<html><head><script src='./files/hello.js'></script></head>" +
               "<body><span id='test'>hello from html</span></body></html>";
  ...
  specify("ensure_resolution_is_not_thrown_off_by_hrefless_base_tag", {skipIfBrowser: true, async: true}, t => {
  ...
  specify("ensure_resources_can_be_skipped_via_options_features", {skipIfBrowser: true, async: true}, t => {
  ...
  specify("understand_file_protocol", {skipIfBrowser: true, async: true}, t => {
    const html = '\
      <html>\
        <head>\
    ...
    const doc = jsdom.jsdom(html);
    doc.onload = function() {
      assert.equal(doc.getElementById("test").innerHTML, 'hello from javascript', 'resource with file protocol should work');
      t.done();
    };
  });
  ...
  <html>\
    ...
    function testLocal() {
      ...
      const doc = jsdom.jsdom(html, {url: url});
      assert.equal(doc.getElementById("link1").href, 'http://example.com/', 'Absolute URL should be left alone except for possible trailing slash');
      assert.equal(doc.getElementById("link2").href, 'file:///local.html', 'Relative URL should be resolved');
      assert.equal(doc.getElementById("link3").href, 'file:///path/to/docroot/local.html', 'Relative URL should be resolved');
      assert.equal(doc.getElementById("link4").href, 'file:///path/local.html', 'Relative URL should be resolved');
      assert.equal(doc.getElementById("link5").href, 'file:///path/to/docroot/index.html#here', 'Relative URL should be resolved');
    ...
    function testRemote() {
      ...
      const doc = jsdom.jsdom(html, {url: url});
      assert.equal(doc.getElementById("link1").href, 'http://example.com/', 'Absolute URL should be left alone except for possible trailing slash');
      assert.equal(doc.getElementById("link2").href, 'http://example.com/local.html', 'Relative URL should be resolved');
      assert.equal(doc.getElementById("link3").href, 'http://example.com/path/to/docroot/local.html', 'Relative URL should be resolved');
      assert.equal(doc.getElementById("link4").href, 'http://example.com/path/local.html', 'Relative URL should be resolved');
      assert.equal(doc.getElementById("link5").href, 'http://example.com/path/to/docroot/index.html#here', 'Relative URL should be resolved');
      assert.equal(doc.getElementById("link6").href, 'http://example.com/protocol/avoidance.html', 'Relative URL should be resolved');
    }
  ...
  specify("auto_tostring", {skipIfBrowser: true}, () => {
    const buffer = require("fs").readFileSync(__dirname + "/files/env.html");
    let dom;
    assert.doesNotThrow(function(){ dom = jsdom.jsdom(buffer); }, 'buffers should automatically be stringified');
  ...
  specify("DomSubtreeModifiedEvents", () => {
    ...
    document.getElementsByTagName("div")[0].setAttribute("class", "test-class-2");
    assert.ok(firedAfterChangingAttr == false, "DOMSubtreeModified not be fired when new attribute value same as old one");
    ...
    document.getElementsByTagName("div")[0].removeAttribute("class");
    assert.ok(firedAfterRemovedAttr == false, "DOMSubtreeModified not be fired when try to remove attribute doesn't exists");
  });
  ...
  specify("setting_and_getting_textContent", () => {
    ...
    assert.strictEqual(frag.textContent, '&lt;foo&gt;<b></b>&lt;foo&gt;<b></b>',
  ...
  specify("allow_ender_to_run", {skipIfBrowser: true, async: true}, t => {
  ...
  specify("issues_230_259", () => {
    ...
    assert.ok(serializeDocument(doc).match(/0: *color/) === null);
  });
  ...
  specify("test_body_event_handler_inline", {async: true}, t => {
    const html = "\
      <html>\
        <head>\
          <script>\
  ...
  specify("issue_335_inline_event_handlers", () => {
    ...
    assert.ok(serializeDocument(doc).indexOf('onclick') > -1);
  });
  ...
  specify("issue_509_out_of_memory", {skipIfBrowser: true}, () => {
    const fs = require("fs");
  ...
  specify("iframe_contents", () => {
    ...
    assert.equal(serializeDocument(iframeDocument), "<html><head></head><body></body></html>");
    assert.ok(iframeDocument.documentElement);
    assert.ok(iframeDocument.head);
    assert.ok(iframeDocument.body);
  });

  specify("jquery_val_on_selects", {skipIfBrowser: true, async: true}, t => {
    const window = jsdom.jsdom().defaultView;

    jsdom.jQueryify(window, 'file:' + path.resolve(__dirname, '../jquery-fixtures/jquery-1.11.0.js'), function () {
      window.$("body").append('<html><body><select id="foo"><option value="first">f</option><option value="last">l</option></select></body></html>');

      assert.equal(window.document.querySelector("[value='first']").selected, true, "`selected` property should be `true` for first");
      assert.equal(window.document.querySelector("[value='last']").selected, false, "`selected` property should be `false` for last");
      ...
      window.$('#foo').val("last");
      assert.equal(window.document.querySelector("[value='first']").selected, false, "`selected` property should be `false` for first");
      assert.equal(window.document.querySelector("[value='last']").selected, true, "`selected` property should be `true` for last");
      assert.equal(window.$('#foo').val(), "last", "`val()` should return last <option>'s value");
      t.done();
    });
  });

  specify("jquery_attr_mixed_case", {skipIfBrowser: true, async: true}, t => {
    const window = jsdom.jsdom().defaultView;
  ...
  specify("Calling show() method in jQuery 1.11.0 (GH-709)", {skipIfBrowser: true, async: true}, t => {
  ...
  specify("Calling show() method in jQuery 1.11.0, second case (GH-709)", {skipIfBrowser: true, async: true}, t => {
    const window = jsdom.jsdom("<!DOCTYPE html><html><head></head><body></body></html>").defaultView;
  ...
  specify("redirected_url_equal_to_location_href", {skipIfBrowser: true, async: true}, t => {
  ...
  specify("script_with_cookie", {skipIfBrowser: true, async: true}, t => {
  ...
  specify("xhr_with_cookie", {skipIfBrowser: true, async: true}, t => {
  ...
  specify("addmetatohead", () => {
    ...
    assert.ok(serializeDocument(window.document).indexOf("<meta>") > -1, "meta should have open tag");
    assert.strictEqual(serializeDocument(window.document).indexOf("</meta>"), -1, "meta should not be stringified with a closing tag");
  });
