...
const getTestFixtureUrl = require("../util").getTestFixtureUrl;
const jsdom = require("../..");
...
describe("jsdom/parsing", () => {
  ...
  specify("Chinese characters as a 'tag name' (GH-719)", { async: true }, testCase => {
    jsdom.env({
  ...
  specify("Missing <body> tag with script src (GH-389)", { async: true }, testCase => {
    jsdom.env({
      html: "<html></html>",
      src: "''",
      done(err) {
  ...
  specify(".innerHTML with < character (GH-652)", { async: true }, testCase => {
    jsdom.env({
      html: "<html>",
      done(err, window) {
        assert.ifError(err);
  ...
  specify("real-world page with < inside a text node (GH-800)", { async: true }, testCase => {
    jsdom.env(
      getTestFixtureUrl("jsdom/files/steam.html"),
      (err, window) => {
        assert.ifError(err);
        assert.notEqual(window.document.querySelector(".badge_card_set_text"), null);
  ...
  specify("xmlns doesn't cause empty prefix", { async: true }, testCase => {
