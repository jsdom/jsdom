...
AI-assisted contributions are welcome, but the human submitter is responsible for the entire contribution. Before submitting it, you must review and understand the changes and be prepared to explain and revise them.

Every commit containing material generated by an AI agent must identify the agent with a commit trailer:

```text
Co-authored-by: Agent Name <agent-email>
```

Place the trailer after a blank line at the end of the commit message. Use one trailer per agent and use the identity supplied by the agent or its vendor. For Codex, use:

```text
Co-authored-by: codex <codex@openai.com>
```

Keep pull request descriptions brief and useful to reviewers; walls of text are not welcome. Explain the concrete real-world project or use case that motivated the human contributor to make the change. Do not explain what tests you ran; CI suffices.

Web platform changes must be exercised by the relevant web platform tests, as described below. If an agent cannot run them because its environment cannot configure the required hosts file, it must escalate to the human contributor so that they can run the tests before submission. Pull requests that skip relevant tests because of agent limitations may be closed without review.

Contributions that look especially agent-driven, with little evidence of human involvement, may receive agent-driven reviews in return.
