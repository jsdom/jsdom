...
const { JSDOMDispatcher, DEFAULT_USER_AGENT } = require("./jsdom/browser/resources/jsdom-dispatcher.js");
...
class JSDOM {
  ...
  static async fromURL(url, options = {}) {
    ...
    const { effectiveDispatcher } = normalizeResourcesOption(resourcesForInitialFetch, options.cookieJar);
...
function normalizeResourcesOption(resources, cookieJar) {
...
function transformOptions(options, encoding, mimeType) {
  ...
  const { userAgent, effectiveDispatcher, loadSubresources } =
    normalizeResourcesOption(options.resources, transformed.windowOptions.cookieJar);
