...
const whatwgURL = require("./jsdom/living/helpers/url");
...
const initialURLRecord = Symbol("initial URL record");
...
class JSDOM {
  ...
  reconfigure(settings) {
    ...
    if ("url" in settings) {
      ...
      document._originIdentity = document._origin === "null" ? {} : document._origin;
      if (url.blobURLEntry) {
        document._originIdentity = url.blobURLEntry.environment._originIdentity;
      }
  ...
  static async fromURL(url, options = {}) {
    ...
    const response = await fetchCollected(effectiveDispatcher, {
      url,
      headers,
      environment: "top-level-navigation"
    });
    ...
    options = Object.assign(options, {
      url: response.url,
      [initialURLRecord]: response.urlRecord,
...
function transformOptions(options, encoding, mimeType) {
  ...
  // fromURL() has already fetched this URL. Retain its blob entry even if the URL was revoked during the fetch.
  transformed.windowOptions.urlRecord = options[initialURLRecord];
