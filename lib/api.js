...
class JSDOM {
  ...
  static async fromURL(url, options = {}) {
    ...
    const { effectiveDispatcher } = normalizeResourcesOption(resourcesForInitialFetch);
...
function normalizeResourcesOption(resources) {
  // resources: undefined means no resource fetching
  if (resources === undefined) {
    return {
      userAgent: DEFAULT_USER_AGENT,
      effectiveDispatcher: null
    };
  }

  let userAgent, baseDispatcher, userInterceptors;
  ...
  return {
    userAgent,
    effectiveDispatcher: baseDispatcher.compose(...interceptors)
  };
}
...
function transformOptions(options, encoding, mimeType) {
  ...
  // Resources are fetched if resources option is set (either "usable" or an object)
  const { userAgent, effectiveDispatcher } = normalizeResourcesOption(options.resources);
