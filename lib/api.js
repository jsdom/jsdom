...
class JSDOM {
  ...
  static async fromURL(url, options = {}) {
    // Remove the hash while sending this through the resource loader fetch().
    ...
    const resourceLoaderForInitialRequest = resourceLoader ?? new ResourceLoader();

    const response = await resourceLoaderForInitialRequest.fetch(url, {
      headers: {
    ...
    // Try to use the response's URL by default, to capture the post-redirect behavior. But, synthetic `Response`s from
    // custom resource loaders will have empty string `url`, so in that case we need to fall back to the request URL.
