...
const { RawHeadersDispatcher } = require("./jsdom/browser/resources/raw-headers.js");
...
function normalizeResourcesOption(resources) {
  ...
  return {
    userAgent,
    effectiveDispatcher: new RawHeadersDispatcher(baseDispatcher, interceptors),
    loadSubresources
  };
}
...
function transformOptions(options, encoding, mimeType) {
  ...
  const { userAgent, effectiveDispatcher, loadSubresources } = normalizeResourcesOption(options.resources);
