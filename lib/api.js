...
const createJsdomInterceptor = require("./jsdom/browser/resources/jsdom-interceptor.js");
const { DEFAULT_USER_AGENT } = require("./jsdom/browser/resources/jsdom-interceptor.js");
const RequestInterceptor = require("./jsdom/browser/resources/request-interceptor.js");
const { fetchCollected } = require("./jsdom/browser/resources/jsdom-fetch.js");
...
class JSDOM {
  ...
  static async fromURL(url, options = {}) {
    // Remove the hash while sending this through the fetch.
    ...
    const baseDispatcher = options.dispatcher ?? getGlobalDispatcher();
    const interceptors = [
      createJsdomInterceptor({ userAgent }),
      ...(options.interceptors ? [].concat(options.interceptors) : [])
    ];
    const effectiveDispatcher = baseDispatcher.compose(...interceptors);
    ...
    // The response.url captures the post-redirect URL
    const finalURL = (response.url || parsedURL.href) + originalHash;
    ...
    options = Object.assign(options, {
      ...
      referrer: options.referrer,
      // Pass the dispatcher so sub-resources will be fetched
      // If one was already provided, use it; otherwise use the global dispatcher
...
function transformOptions(options, encoding, mimeType) {
  ...
  // Build the effective dispatcher with jsdom interceptor and user interceptors
  // Resources are fetched if: dispatcher is set, interceptors are set, or resources: "usable"
  const userAgent = options.userAgent ?? DEFAULT_USER_AGENT;
  transformed.windowOptions.userAgent = userAgent;

  const shouldFetchResources = options.dispatcher !== undefined ||
  ...
  if (shouldFetchResources) {
    ...
    transformed.windowOptions.dispatcher = baseDispatcher.compose(...interceptors);
  } else {
    transformed.windowOptions.dispatcher = undefined;
  }
...
exports.JSDOM = JSDOM;
JSDOM.RequestInterceptor = RequestInterceptor;
