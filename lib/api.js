...
class JSDOM {
  constructor(input, options = {}) {
    ...
    if (options.windowOptions.parsingMode == "json") {
      this.html = html;
    } else {
  ...
  static fromRequestArgs(requestArgs, options = {}) {
    return Promise.resolve().then(() => {
      const parsedURL = new URL(requestArgs.url);
      ...
      const requestOptions = {
		...
		referrer: requestArgs.referrer ? requestArgs.referrer : options.referrer,
		timeout: options.timeout,
        resolveWithFullResponse: true,
      }
      const method = requestArgs.method || 'get';
      if (method == 'post') {
        requestOptions.contentType = requestArgs.contentType;
        requestOptions.body = requestArgs.body;
      }
      requestOptions.method = method;

      // Request should not fail on not 2xx response code if we need to redirect on this response code
      let rejectOnFail = true;
      if (options.redirectStatusCode) {
        let statusCodes = [];
        if (options.redirectStatusCode instanceof Array) {
          statusCodes = options.redirectStatusCode;
        } else {
          statusCodes.push(options.redirectStatusCode);
        }
        for (const code of statusCodes) {
          if (!/^2/.test(code.toString())) {
            rejectOnFail = false;
            break;
          }
        }
      }
      requestOptions.simple = rejectOnFail;

      const req = resourceLoaderForInitialRequest.fetch(url, requestOptions);

      return req.then(res => {
        if (res.body.byteLength == 0) {
          return new Error("No body fetched");
        }


		var updatedOptions = Object.assign({}, options);
		updatedOptions = Object.assign(updatedOptions, {
        ...
        if (options.redirectStatusCode) {
            if (options.redirectStatusCode instanceof Array && options.redirectStatusCode.includes(res.statusCode)) {
                needRedirect = true;
            } else if (options.redirectStatusCode == res.statusCode) {
                needRedirect = true;
            }
        }
        if (needRedirect) {
          // 重定向加载
          updatedOptions.runScripts = "dangerously";
          const dom = new JSDOM(res.body, updatedOptions);
          const window = dom.window;

          window._virtualConsole.emit('warn', 'Redirecting url [' + url.slice(0, url.indexOf('?') + 40) + ']......');

          return new Promise(resolve => {
            const t = setTimeout(() => {
              window._virtualConsole.emit('error', 'Timeout on redirecting url [' + url.slice(0, url.indexOf('?') + 40) + ']......')
              resolve(dom);
            }, options.timeout ? options.timeout : 30000);
            window.addEventListener('targetcreated', (event) => {
              clearTimeout(t);
              window._virtualConsole.emit('info', 'Redirected url [' + event.detail.url + '].....');
              resolve(JSDOM.fromRequestArgs(event.detail, options));
              Promise.resolve().then(window.close());
            });
          });
        } else {
          return new JSDOM(res.body, updatedOptions);
        }
      }).catch(err => {
        return err;
      });
...
function transformOptions(options, encoding, mimeType) {
  ...
  const isJSON = (mimeType.type == "application" && mimeType.subtype == "json")
  if (!mimeType.isHTML() && !mimeType.isXML() && !isJSON) {
    throw new RangeError(`The given content type of "${options.contentType}" was not a HTML or XML content type`);
  }

  transformed.windowOptions.contentType = mimeType.essence;
  transformed.windowOptions.parsingMode = mimeType.isHTML() ? "html" : isJSON ? "json" : "xml";
  ...
  if (options.openDatabase !== undefined) {
    transformed.windowOptions.openDatabase = options.openDatabase;
  }

  if (options.indexedDB !== undefined) {
    transformed.windowOptions.indexedDB = options.indexedDB;
  }

  if (options.localStorage !== undefined) {
    transformed.windowOptions.localStorage = options.localStorage;
  }
  ...
  if (options.afterCreateElement != undefined) {
	  transformed.windowOptions.afterCreateElement = options.afterCreateElement;
  }
  ...
  if (options.notifyNavigateOnSameURL !== undefined) {
    transformed.windowOptions.notifyNavigateOnSameURL = options.notifyNavigateOnSameURL;
  }
