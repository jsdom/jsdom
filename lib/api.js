...
const { createWindow } = require("./jsdom/browser/windows.js");
...
class JSDOM {
  constructor(input, options = {}) {
    ...
    const documentImpl = this[window].document;
  ...
  get cookieJar() {
    ...
    return this[window].document._cookieJar;
  }

  serialize() {
    return fragmentSerialization(this[window].document, { requireWellFormed: false, globalObject: this[window] });
  }

  nodeLocation(node) {
    if (!this[window].document._parseOptions.sourceCodeLocationInfo) {
  ...
  getInternalVMContext() {
    const { _vmContext } = this[window];

    if (_vmContext === undefined || !vm.isContext(_vmContext)) {
