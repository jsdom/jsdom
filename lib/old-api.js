...
const { locationInfo } = require("./jsdom/living/helpers/internal-constants");
  ...
  if (config.file && canReadFilesFromFS) {
    req = resourceLoader.readFile(
      ...
      (err, text, res) => {
        ...
        const parsedMIMEType = new MIMEType(res.headers["content-type"]);
        config.encoding = parsedMIMEType.parameters.get("charset");
