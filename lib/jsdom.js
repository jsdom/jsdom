...
exports.jsdom = function (html, options) {
  ...
  options.userAgent = options.userAgent || "Node.js (" + process.platform + "; U; rv:" + process.version + ")";

  options.pool = typeof options.pool !== "undefined" ? options.pool : {
    maxSockets: 6
  };

  options.agentOptions = typeof options.agentOptions !== "undefined" ? options.agentOptions : {
    keepAlive: true,
...
exports.env = exports.jsdom.env = function () {
  ...
  if (config.file && canReadFilesFromFS) {
    ...
    if (url.protocol && url.hostname) {
      ...
      return handleUrl(config.somethingToAutodetect);
    } else if (canReadFilesFromFS) {
      return resourceLoader.readFile(config.somethingToAutodetect, function (err, text) {
        if (err) {
  ...
  function handleUrl() {
    ...
    return resourceLoader.download(config.url, options, config.cookieJar, null, function (err, responseText, res) {
      if (err) {
        reportInitError(err, config);
        return;
      }
...
function getConfigFromArguments(args) {
  ...
  config.pool = typeof config.pool !== "undefined" ? config.pool : {
    maxSockets: 6
  };

  config.agentOptions = typeof config.agentOptions !== "undefined" ? config.agentOptions : {
    keepAlive: true,
    keepAliveMsecs: 115 * 1000
  };
