...
exports.jsdom = function (html, options) {
  ...
  if (html === undefined || html === null || (html.trim && !html.trim())) {
...
function makeFromHTML(html, options) {
  // TODO: this is essentially compensating for the parser not being smart enough to insert these on its own.
  // Get a better parser!
  html += '';
  ...
  return exports.jsdom(html, options);
};
...
function processHTML(config) {
  ...
  var window = makeFromHTML(config.html, options).parentWindow;
