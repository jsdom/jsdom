...
exports.jsdom = function (html, level, options) {
  ...
  if (options.create) {
    options.create(null, doc.parentWindow);
  }
...
function processHTML(config) {
  ...
  if (!window || !window.document) {
    if (config.create) {
      config.create(new Error('JSDOM: a window object could not be created.'));
    }
    if (config.done) {
  ...
  window.addEventListener('load', function() {
    if (config.scripts.length === 0 && config.src.length === 0 && config.done) {
      config.done(errors, window);
    }
    if (config.load) {
      config.load(errors, window);
    }
  });
