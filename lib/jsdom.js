...
exports.jsdom = function (html, options) {
  ...
  var browser = exports.browserAugmentation(options.level, options);
...
function preprocessHTML(html) {
  // TODO: this is essentially compensating for the parser not being smart enough to do this.
  // Get a better parser!
  ...
  var headCloseIndex = htmlLowered.indexOf('</head>');
  var bodyOpenIndex = htmlLowered.indexOf('<body');

  var hasHead = htmlLowered.indexOf('<head') !== -1 && headCloseIndex !== -1;
  var hasBody = bodyOpenIndex !== -1;
  var hasHtml = htmlLowered.indexOf('<html') !== -1;
  ...
  if (!hasHtml) {
    ...
    if (!hasBody && hasHead) {
      var headCloseEndIndex = headCloseIndex + '</head>'.length;
      html = html.substring(0, headCloseEndIndex) + '<body></body>' + html.substring(headCloseEndIndex);
    }
    if (hasBody && !hasHead) {
      html = html.substring(0, bodyOpenIndex) + '<head></head>' + html.substring(bodyOpenIndex);
    }
    // Not handled: !hasBody && !hasHead
  }
