  ...
  markTreeReadonly: function(node) {
    core.visitTree(node, function(el) { 
      ...
      if (el.attributes) {
        ...
        for (i; i<l; i++) {
          core.markTreeReadonly(attributes.item(i));
        }
      }
    });
  }
...
core.Node.prototype = {
  ...
  get nodeName() {
    ...
    if (this.nodeType === ELEMENT_NODE &&
        ...
        this._ownerDocument._doctype.name.indexOf("html") !== -1)
    {
  ...
  get nextSibling() {
    ...
    var currentNode, index = 0, siblings = this._parentNode._childNodes

    while ((currentNode = siblings[index])) {
      index++;
  ...
  get previousSibling() {
      ...
      var currentNode,
          index = 0,
          siblings = this._parentNode._childNodes;
  ...
  insertBefore :  function(/* Node */ newChild, /* Node*/ refChild){
    ...
    var i=0, found = false, children = this._childNodes, l = children.length, current;
  ...
  removeChild : function(/* Node */ oldChild){
    ...
    var i=0, found = false, child, children = this._childNodes, l = children.length;
  ...
  appendChild : function(/* Node */ newChild){
    ...
    if (newChild.nodeType === DOCUMENT_FRAGMENT_NODE) {
      ...
      var transfer = [];
      var length = newChild._childNodes.length;
      var i = length-1;
      var tmpNode;
      var child;
  ...
  cloneNode : function(/* bool */ deep) {
    ...
    switch (this.nodeType) {
      ...
      case this.ENTITY_REFERENCE_NODE:
        var ref = this._ownerDocument.createEntityReference(this._entity.name);
      ...
      case this.ENTITY_NODE:
        var entity = this._ownerDocument.createEntityNode(this._entity.name);
...
core.AttrNodeMap.prototype = {
  ...
  removeNamedItem: function(/* string */ name) {
    ...
    if (doc && doc._doctype && doc._doctype.name.toLowerCase() !== "html") {
      ...
      var elem = doc._doctype._attributes.getNamedItem(this._parentNode.nodeName);
...
core.Element.prototype = {
  ...
  setAttribute: function(/* string */ name, /* string */ value) {
    ...
    if (this._attributes === null) {
        this._attributes = new core.AttrNodeMap(this._ownerDocument, this);
    }
  ...
  setAttributeNode: function(/* Attr */ newAttr) {
    ...
    // only attributes created with this._ownerDocument can be added here
    if (newAttr._ownerDocument !== this._ownerDocument) {
      throw new core.DOMException(WRONG_DOCUMENT_ERR);
    }
