...
core.Node.prototype = {
  ...
  _indexOf: function(/*Node*/ child) {
    if (domSymbolTree.parent(child) !== this) {
      return -1;
    }
    return domSymbolTree.index(child);
  },
  ...
  get innerHTML() {
    ...
    if (tagName === 'SCRIPT' || tagName === 'STYLE') {
      ...
      if (!type || /^text\//i.test(type) || /\/javascript$/i.test(type)) {
        return domToHtml(domSymbolTree.childrenToArray(this), true, true);
      }
    }
    ...
    if (tagName === 'TEMPLATE' && this._namespaceURI === 'http://www.w3.org/1999/xhtml') {
      ...
      if (firstChild && firstChild._templateContent) {
        return domToHtml(domSymbolTree.childrenToArray(firstChild), true);
      }
    }

    return domToHtml(domSymbolTree.childrenToArray(this), true);
  },
...
inheritFrom(core.Node, core.Document, {
  ...
  get doctype() {
    for (const childNode of domSymbolTree.childrenIterator(this)) {
      if (childNode.nodeType === DOCUMENT_TYPE_NODE) {
        return childNode;
      }
    }
    return null;
