...
function EventTarget() {
  this._jsdom_events = Object.create(null);
}
...
EventTarget.prototype.addEventListener = function (type, callback, capture) {
  ...
  if (this._jsdom_events[type] === undefined) {
    this._jsdom_events[type] = {
      capturing: [],
      bubbling: []
    };
  }

  var handlers = this._jsdom_events[type][capture ? "capturing" : "bubbling"];
...
function getListeners(target, type, capturing) {
  var listeners;
  if (target._jsdom_events[type]) {
    listeners = target._jsdom_events[type][capturing ? "capturing" : "bubbling"];
  } else {
    listeners = [];
  }
