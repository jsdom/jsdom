...
function findGlobalDeclarations(ast) {
  ...
  var declarePattern = function (node, varNode) {
    switch (node.type) {
      case 'Identifier':
      ...
      case 'ObjectPattern':
        node.properties.forEach(function (node) {
          declarePattern(node.value || node.argument, varNode);
        });
        break;
      case 'ArrayPattern':
        node.elements.forEach(function (node) {
          if (node) declarePattern(node, varNode);
        });
        break;
      case 'RestElement':
...
exports.runInContext = function (code, contextifiedSandbox, options) {
  ...
  for (let globalDeclaration of globalDeclarations) {
    if (globalDeclaration.type === "VariableDeclaration") {
      let globals = Object.keys(globalDeclaration.globals);
      let expressions = globals.map(getPatchNode).concat({type: "Identifier", name: globals[0]});
      ...
      let parent = globalDeclaration.parent;
      switch (parent.type) {
        case "Program":
        case "BlockStatement":
          parent.body.splice(parent.body.indexOf(globalDeclaration) + 1, 0, getPatchExpressionStatement(globalDeclaration));
          break;
        case "IfStatement":
        ...
        case "SwitchStatement":
          for (let {consequent} of parent.cases) {
            let index = consequent.indexOf(globalDeclaration);
            if (index >= 0) {
