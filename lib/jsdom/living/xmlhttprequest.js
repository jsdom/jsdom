...
module.exports = function createXMLHttpRequest(window) {
  ...
  class XMLHttpRequest extends XMLHttpRequestEventTarget.interface {
    ...
    get response() {
      ...
      switch (this.responseType) {
        ...
        case "blob": {
          ...
          const mimeType = finalMIMEType(this);
          res = Blob.create([
    ...
    get responseXML() {
      ...
      let isXML = false;
      if (mimeType) {
        isHTML = mimeType.isHTML();
        isXML = mimeType.isXML();
        if (!isXML && !isHTML) {
          return null;
        }
      }
    ...
    overrideMimeType(mime) {
      ...
      if (mimeType) {
        this[xhrSymbols.flag].overrideMIMEType = mimeType.type + "/" + mimeType.subtype;

        const charset = mimeType.get("charset");
        if (charset) {
      ...
      try {
        ...
        if (body !== null) {
          ...
          if (mimeType !== null && existingContentType === null) {
            ...
            const mimeType = new MIMEType(existingContentType);
            if (mimeType) {
              for (const param of mimeType.parameterList) {
              ...
              xhrUtils.updateRequestHeader(flag.requestHeaders, "content-type", mimeType.toString());
            }
          }
        }
      } finally {
...
function finalCharset(xhr) {
  ...
  const mimeType = new MIMEType(getResponseHeader(xhr, "content-type"));
  if (mimeType) {
    return whatwgEncoding.labelToName(mimeType.get("charset"));
  }
  return null;
}
