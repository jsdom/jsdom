...
const xmlContentType = /^(?:text\/xml|application\/xml|[^;]*?\+xml)(?:;.*)*$/;
...
module.exports = function createXMLHttpRequest(window) {
  ...
  class XMLHttpRequest extends XMLHttpRequestEventTarget {
    ...
    get responseXML() {
      ...
      let contentType = flag.mimeType || this.getResponseHeader("content-type");
      const splittedMime = contentType.split(";");
      const splittedType = splittedMime[0].split("/");
      if(splittedType.length !== 2 || !tokenRegexp.test(splittedType[0]) || !tokenRegexp.test(splittedType[1])) {
        contentType = null;
      }
      if (contentType && !contentType.match(xmlContentType) && !contentType.match(htmlContentType)) {
        return null;
      }
      const isHtml = contentType && contentType.match(htmlContentType);
    ...
    open(method, uri, asynchronous, user, password) {
      ...
      flag.origin = this._ownerDocument.origin;
      ...
      try {
        if (!flag.body &&
          ...
          if (body instanceof FormData) {
            ...
            const doc = idlUtils.implForWrapper(body);
            let contentType = (doc._parsingMode === "xml" ? "application/xml" : "text/html");
            contentType += ";charset=" + flag.characterSet;
    ...
    setRequestHeader(header, value) {
      ...
      if(lcHeader === "content-type") {
        ...
        const splittedMime = value.split(";");
        const splittedType = splittedMime[0].split("/");
        if (splittedType.length === 2 && tokenRegexp.test(splittedType[0]) && tokenRegexp.test(splittedType[1])) {
          const l = splittedMime.length;
          for (let i = 1; i < l; i++) {
            const splittedParameter = splittedMime[i].split("=");
            if (splittedParameter.length === 2
                && splittedParameter[0].trim() === "charset"
                && lcCharset !== splittedParameter[1].toLowerCase()) {
              splittedParameter[1] = flag.characterSet;
              splittedMime[i] = splittedParameter.join("=");
            }
          }
          value = splittedMime.join(";");
        }
      }
