...
module.exports = function createXMLHttpRequest(window) {
  ...
  class XMLHttpRequest extends XMLHttpRequestEventTarget.interface {
    ...
    overrideMimeType(mime) {
      ...
      const parsedContentType = new MIMEType(mime);
      if (parsedContentType) {
      ...
      try {
        ...
        if (body !== null) {
          ...
          if (mimeType !== null && existingContentType === null) {
            ...
            const parsedContentType = new MIMEType(existingContentType);
            if (parsedContentType) {
...
function finalMIMEType(xhr) {
  const flag = xhr[xhrSymbols.flag];
  return new MIMEType(flag.overrideMIMEType || getResponseHeader(xhr, "content-type"));
}
...
function finalCharset(xhr) {
  ...
  const parsedContentType = new MIMEType(getResponseHeader(xhr, "content-type"));
  if (parsedContentType) {
