...
const contenttype = require("con-type");
...
module.exports = function createXMLHttpRequest(window) {
  ...
  class XMLHttpRequest extends XMLHttpRequestEventTarget {
    ...
    get response() {
      ...
      switch (this.responseType) {
        ...
        case "json":
          ...
          try {
            res = JSON.parse(bufferToString(properties.responseBuffer, contentType));
          } catch (e) {
            res = null;
          }
          break;
    ...
    get responseText() {
      ...
      const flag = this[xhrSymbols.flag];
      const contentType = flag.mimeType || this.getResponseHeader("Content-Type");
      const res = bufferToString(responseBuffer, contentType);
    ...
    get responseXML() {
      ...
      const resText = bufferToString(responseBuffer, contentType);
      const res = Document.create([], { core: window._core });
    ...
    open(method, uri, async, user, password) {
      ...
      const urlObj = new URL(uri, baseUrl);
  ...
  function bufferToString(buffer, contentType) {
    ...
    if (contentType) {
      ...
      const charset = parsed.charset && parsed.charset.toLowerCase();
      switch (charset) {
        case "utf-16":
          encoding = "utf16le";
          break;
