  ...
  class XMLHttpRequest extends XMLHttpRequestEventTarget.interface {
    ...
    get response() {
      ...
      switch (this.responseType) {
        ...
        case "blob": {
          ...
          const parsedContentType = finalMIMEType(this);
          res = Blob.create([
            [new Uint8Array(responseBuffer)],
            { type: (parsedContentType && parsedContentType.toString()) || "" }
          ]);
          break;
        }
        case "document": {
    ...
    get responseXML() {
      ...
      if (parsedContentType) {
        isHTML = parsedContentType.isHTML();
        isXML = parsedContentType.isXML();
        if (!isXML && !isHTML) {
          return null;
        }
      }
    ...
    overrideMimeType(mime) {
      ...
      const parsedContentType = firstMIMEType([mime]);
      if (parsedContentType) {
        this[xhrSymbols.flag].overrideMIMEType = parsedContentType.type + "/" + parsedContentType.subtype;

        const charset = parsedContentType.parameters.get("charset");
        if (charset) {
      ...
      try {
        ...
        if (body !== null) {
          ...
          if (mimeType !== null && existingContentType === null) {
            ...
            const parsedContentType = firstMIMEType([existingContentType]);
            if (parsedContentType) {
              for (const [key, value] of parsedContentType.parameters) {
                if (key && key.toLowerCase() === "charset") {
                  if (value.toLowerCase() !== encoding.toLowerCase()) {
                    parsedContentType.parameters.set(key, encoding);
                  }
                }
              }
              xhrUtils.updateRequestHeader(flag.requestHeaders, "content-type", parsedContentType.toString());
            }
          }
        }
      } finally {
...
function finalCharset(xhr) {
  ...
  const parsedContentType = firstMIMEType([getResponseHeader(xhr, "content-type")]);
  if (parsedContentType) {
