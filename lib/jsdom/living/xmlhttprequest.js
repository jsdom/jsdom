...
const { addConstants, firstMIMEType } = require("../utils");
...
module.exports = function createXMLHttpRequest(window) {
  ...
  class XMLHttpRequest extends XMLHttpRequestEventTarget.interface {
    ...
    get response() {
      ...
      switch (this.responseType) {
        ...
        case "blob": {
          ...
          const parsedMIMEType = finalMIMEType(this);
          res = Blob.create([
            [new Uint8Array(responseBuffer)],
            { type: (parsedMIMEType && parsedMIMEType.toString()) || "" }
          ]);
          break;
        }
        case "document": {
    ...
    get responseXML() {
      ...
      let isXML = false;
      if (parsedMIMEType) {
        isHTML = parsedMIMEType.isHTML();
        isXML = parsedMIMEType.isXML();
        if (!isXML && !isHTML) {
          return null;
        }
      }
    ...
    overrideMimeType(mime) {
      ...
      const parsedMIMEType = firstMIMEType([mime]);
      if (parsedMIMEType) {
        this[xhrSymbols.flag].overrideMIMEType = parsedMIMEType.type + "/" + parsedMIMEType.subtype;

        const charset = parsedMIMEType.parameters.get("charset");
        if (charset) {
      ...
      try {
        ...
        if (body !== null) {
          ...
          if (mimeType !== null && existingContentType === null) {
            ...
            const parsedMIMEType = firstMIMEType([existingContentType]);
            if (parsedMIMEType) {
              for (const [key, value] of parsedMIMEType.parameters) {
                if (key && key.toLowerCase() === "charset") {
                  if (value.toLowerCase() !== encoding.toLowerCase()) {
                    parsedMIMEType.parameters.set(key, encoding);
                  }
                }
              }
              xhrUtils.updateRequestHeader(flag.requestHeaders, "content-type", parsedMIMEType.toString());
            }
          }
        }
      } finally {
...
function finalMIMEType(xhr) {
  const flag = xhr[xhrSymbols.flag];
  return firstMIMEType([flag.overrideMIMEType, getResponseHeader(xhr, "content-type")]);
}
...
function finalCharset(xhr) {
  ...
  const parsedMIMEType = firstMIMEType([getResponseHeader(xhr, "content-type")]);
  if (parsedMIMEType) {
    return whatwgEncoding.labelToName(parsedMIMEType.parameters.get("charset"));
  }
  return null;
}
