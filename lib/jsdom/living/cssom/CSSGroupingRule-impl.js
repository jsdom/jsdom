...
const { parseRule, parseDeclarationsAsNestedRule } = require("../helpers/css-parser.js");
...
class CSSGroupingRuleImpl extends CSSRuleImpl {
  ...
  insertRule(rule, index) {
    ...
    let newRule;
    try {
      newRule = parseRule(rule, this._globalObject);
    } catch (e) {
      // In nesting context, declaration text creates CSSNestedDeclarations
      if (this._isInNestingContext()) {
        newRule = parseDeclarationsAsNestedRule(rule, this._globalObject);
        if (!newRule) {
          throw e;
        }
      } else {
    ...
    if (newRule.type === 5) {
      ...
      while (ancestor) {
        ...
        ancestor = ancestor.parentRule ? implForWrapper(ancestor.parentRule) : null;
      }
    }

    newRule.parentRule = wrapperForImpl(this);
