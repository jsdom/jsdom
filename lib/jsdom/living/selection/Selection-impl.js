...
class SelectionImpl {
  constructor(globalObject) {
    ...
    this._globalImpl = implForWrapper(globalObject);
  }
  ...
  addRange(range) {
    if (range._root === this._globalImpl.document && this.rangeCount === 0) {
  ...
  collapse(node, offset) {
    ...
    if (nodeRoot(node) !== this._globalImpl.document) {
      return;
    }
  ...
  setBaseAndExtent(anchorNode, anchorOffset, focusNode, focusOffset) {
    ...
    const { document } = this._globalImpl;
  ...
  containsNode(node, allowPartialContainment) {
    if (this._isEmpty() || nodeRoot(node) !== this._globalImpl.document) {
      return false;
    }
