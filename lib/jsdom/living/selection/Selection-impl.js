...
class SelectionImpl {
  ...
  getRangeAt(index) {
    if (index !== 0 || this._isEmpty()) {
      throw DOMException.create(this._sdGlobalObject, ["Invalid range index.", "IndexSizeError"]);
    }
  ...
  addRange(range) {
    if (range._root === implForWrapper(this._sdGlobalObject._document) && this.rangeCount === 0) {
  ...
  removeRange(range) {
    if (range !== this._range) {
      throw DOMException.create(this._sdGlobalObject, ["Invalid range.", "NotFoundError"]);
    }
  ...
  collapse(node, offset) {
    ...
    if (nodeRoot(node) !== implForWrapper(this._sdGlobalObject._document)) {
      return;
    }

    const newRange = Range.createImpl(this._sdGlobalObject, [], {
      start: { node, offset: 0 },
      end: { node, offset: 0 }
    });
  ...
  collapseToStart() {
    if (this._isEmpty()) {
      throw DOMException.create(this._sdGlobalObject, ["There is no selection to collapse.", "InvalidStateError"]);
    }
  ...
  extend(node, offset) {
    ...
    if (this._isEmpty()) {
      throw DOMException.create(this._sdGlobalObject, ["There is no selection to extend.", "InvalidStateError"]);
    }
  ...
  setBaseAndExtent(anchorNode, anchorOffset, focusNode, focusOffset) {
    if (anchorOffset > nodeLength(anchorNode) || focusOffset > nodeLength(focusNode)) {
      throw DOMException.create(this._sdGlobalObject, ["Invalid anchor or focus offset.", "IndexSizeError"]);
    }

    const document = implForWrapper(this._sdGlobalObject._document);
    ...
    if (compareBoundaryPointsPosition(anchor, focus) === -1) {
      newRange = Range.createImpl(this._sdGlobalObject, [], {
  ...
  containsNode(node, allowPartialContainment) {
    if (this._isEmpty() || nodeRoot(node) !== implForWrapper(this._sdGlobalObject._document)) {
      return false;
    }
  ...
  _associateRange(newRange) {
    ...
    if (didSelectionChange) {
      ...
      const document = this._sdGlobalObject._document;
      setTimeout(() => {
