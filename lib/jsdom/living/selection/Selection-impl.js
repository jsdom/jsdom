...
class SelectionImpl {
  ...
  addRange(range) {
    if (range._root === this._globalObject._document && this.rangeCount === 0) {
  ...
  collapse(node, offset) {
    ...
    if (node.getRootNode() !== this._globalObject._document) {
      return;
    }
  ...
  extend(node, offset) {
    ...
    if (node.getRootNode() !== this._range._root) {
  ...
  setBaseAndExtent(anchorNode, anchorOffset, focusNode, focusOffset) {
    if (anchorOffset > anchorNode._length || focusOffset > focusNode._length) {
    ...
    if (document !== anchorNode.getRootNode() || document !== focusNode.getRootNode()) {
      return;
    }
  ...
  selectAllChildren(node) {
    ...
    if (this._globalObject._document !== node.getRootNode()) {
      return;
    }

    const length = node._childCount;
  ...
  containsNode(node, allowPartialContainment) {
    if (this._isEmpty() || node.getRootNode() !== this._globalObject._document) {
      return false;
    }
    ...
    const endIsAfterNode = compareBoundaryPointsPosition(_end, { node, offset: node._length }) === 1;
  ...
  _associateRange(newRange) {
    ...
    if (didSelectionChange) {
      const document = this._globalObject._document;
      document._queueATask(() => fireAnEvent("selectionchange", document));
    }
  }
}
