...
class SelectionImpl {
  ...
  removeRange(range) {
    if (range !== this._range) {
      throw new DOMException("Invalid range.", "NotFoundError");
    }
  ...
  collapse(node, offset) {
    ...
    if (node.nodeType === NODE_TYPE.DOCUMENT_TYPE_NODE) {
      throw new DOMException("DocumentType Node can't be used as boundary point.", "InvalidNodeTypeError");
    }
  ...
  collapseToStart() {
    if (this._isEmpty()) {
      throw new DOMException("There is no selection to collapse.", "InvalidStateError");
    }
  ...
  extend(node, offset) {
    ...
    if (this._isEmpty()) {
      throw new DOMException("There is no selection to extend.", "InvalidStateError");
    }
  ...
  setBaseAndExtent(anchorNode, anchorOffset, focusNode, focusOffset) {
    if (anchorOffset > nodeLength(anchorNode) || focusOffset > nodeLength(focusNode)) {
      throw new DOMException("Invalid anchor or focus offset.", "IndexSizeError");
    }
