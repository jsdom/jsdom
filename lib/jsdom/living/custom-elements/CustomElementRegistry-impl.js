...
class CustomElementRegistryImpl {
  ...
  define(name, constructor, options) {
    ...
    if (!isValidCustomElementName(name)) {
      throw DOMException.create(_sdGlobalObject, ["Name argument is not a valid custom element name.", "SyntaxError"]);
    }

    const nameAlreadyRegistered = this._customElementDefinitions.some(entry => entry.name === name);
    if (nameAlreadyRegistered) {
      throw DOMException.create(_sdGlobalObject, [
    ...
    try {
      ...
      for (const callbackName of LIFECYCLE_CALLBACKS) {
        ...
        if (callbackValue !== undefined) {
          lifecycleCallbacks[callbackName] = IDLFunction.convert(_sdGlobalObject, callbackValue, {
    ...
    const document = idlUtils.implForWrapper(this._sdGlobalObject._document);
