...
class RangeImpl extends AbstractRangeImpl {
  ...
  get commonAncestorContainer() {
    ...
    for (const container of treeHelpers.ancestorsIterator(_start.node)) {
  ...
  deleteContents() {
    ...
    while (currentNode && currentNode !== endNode) {
      ...
      currentNode = treeHelpers.following(currentNode);
    }
  ...
  surroundContents(newParent) {
    ...
    while (node !== endNode) {
      ...
      node = treeHelpers.following(node);
    }
...
function cloneRange(range) {
  ...
  const containedChildren = treeHelpers.childrenToArray(commonAncestor)
