...
class RangeImpl extends AbstractRangeImpl {
  ...
  get commonAncestorContainer() {
    ...
    return _start.node._commonAncestor(_end.node);
  }
  ...
  deleteContents() {
    ...
    const endNode = this._end.node._nextAfterSubtree();
    while (currentNode && currentNode !== endNode) {
      if (
        isContained(currentNode, this) &&
        !isContained(currentNode.parentNode, this)
      ) {
    ...
    if (isInclusiveAncestor(originalStart.node, originalEnd.node)) {
      ...
      while (
        referenceNode &&
        !isInclusiveAncestor(referenceNode.parentNode, originalEnd.node)
      ) {
        referenceNode = referenceNode.parentNode;
      }

      newNode = referenceNode.parentNode;
      newOffset = referenceNode._treeIndex() + 1;
    }
  ...
  surroundContents(newParent) {
    ...
    const endNode = node._nextAfterSubtree();
    while (node !== endNode) {
  ...
  toString() {
    ...
    const endNode = _end.node._nextAfterSubtree();
    while (currentNode && currentNode !== endNode) {
...
function insertNodeInRange(node, range) {
  ...
  if (
    ...
    (startNode.nodeType === NODE_TYPE.TEXT_NODE && !startNode.parentNode) ||
    node === startNode
  ) {
  ...
  let newOffset = !referenceNode ? nodeLength(parent) : referenceNode._treeIndex();
...
function cloneRange(range) {
  ...
  const commonAncestor = originalStart.node._commonAncestor(originalEnd.node);
  ...
  const containedChildren = commonAncestor._childrenToArray(node => isContained(node, range));
