  ...
  surroundContents(newParent) {
    ...
    while (node !== endNode) {
      if (node.nodeType !== NODE_TYPE.TEXT_NODE && isPartiallyContained(node, this)) {
        throw new DOMException("The Range has partially contains a non-Text node.", "InvalidStateError");
      }
...
function cloneRange(range) {
  ...
  if (hasDoctypeChildren) {
    throw new DOMException("Invalid document type element.", "HierarchyRequestError");
  }
