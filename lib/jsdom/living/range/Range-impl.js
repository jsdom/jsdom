...
class RangeImpl extends AbstractRangeImpl {
  ...
  setStartBefore(node) {
    ...
    if (!parent) {
      throw DOMException.create(this._sdGlobalObject, ["The given Node has no parent.", "InvalidNodeTypeError"]);
    }
  ...
  compareBoundaryPoints(how, sourceRange) {
    if (
      ...
      const message = "The comparison method provided must be one of 'START_TO_START', 'START_TO_END', 'END_TO_END', " +
                      "or 'END_TO_START'.";
      throw DOMException.create(this._sdGlobalObject, [message, "NotSupportedError"]);
    }

    if (this._root !== sourceRange._root) {
      throw DOMException.create(this._sdGlobalObject, ["The two Ranges are not in the same tree.", "WrongDocumentError"]);
    }
  ...
  surroundContents(newParent) {
    ...
    if (
      ...
      throw DOMException.create(this._sdGlobalObject, ["Invalid element type.", "InvalidNodeTypeError"]);
    }
  ...
  cloneRange() {
    const { _start, _end, _sdGlobalObject } = this;

    return Range.createImpl(_sdGlobalObject, [], {
...
function validateSetBoundaryPoint(node, offset) {
  if (node.nodeType === NODE_TYPE.DOCUMENT_TYPE_NODE) {
    throw DOMException.create(node._sdGlobalObject, [
      "DocumentType Node can't be used as boundary point.",
      "InvalidNodeTypeError"
    ]);
  }
  ...
  if (offset > nodeLength(node)) {
    throw DOMException.create(node._sdGlobalObject, ["Offset out of bound.", "IndexSizeError"]);
  }
}
...
function selectNodeWithinRange(node, range) {
  ...
  if (!parent) {
    throw DOMException.create(node._sdGlobalObject, ["The given Node has no parent.", "InvalidNodeTypeError"]);
  }
...
function insertNodeInRange(node, range) {
  ...
  if (
    ...
    throw DOMException.create(node._sdGlobalObject, ["Invalid start node.", "HierarchyRequestError"]);
  }
...
function cloneRange(range) {
  const { _start: originalStart, _end: originalEnd, _sdGlobalObject } = range;

  const fragment = DocumentFragment.createImpl(_sdGlobalObject, [], {
  ...
  if (hasDoctypeChildren) {
    throw DOMException.create(range._sdGlobalObject, ["Invalid document type element.", "HierarchyRequestError"]);
  }
  ...
  if (
    ...
    const subrange = Range.createImpl(_sdGlobalObject, [], {
