...
class RangeImpl extends AbstractRangeImpl {
  ...
  surroundContents(newParent) {
    ...
    while (node !== endNode) {
      if (!Text.isImpl(node) && isPartiallyContained(node, this)) {
        throw DOMException.create(this._globalObject, [
  ...
  toString() {
    ...
    if (_start.node === _end.node && Text.isImpl(_start.node)) {
    ...
    while (currentNode && currentNode !== endNode) {
      if (Text.isImpl(currentNode) && isContained(currentNode, this)) {
        s += currentNode.data;
      }
...
function isPartiallyContained(node, range) {
  const { _start, _end } = range;
  return (
    (node.contains(_start.node) && !node.contains(_end.node)) ||
    (!node.contains(_start.node) && node.contains(_end.node))
  );
}
