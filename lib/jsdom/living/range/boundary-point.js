...
function compareBoundaryPointsPosition(bpA, bpB) {
  ...
  const position = treeHelpers.compareTreePosition(nodeA, nodeB);

  if (position & NODE_DOCUMENT_POSITION.DOCUMENT_POSITION_DISCONNECTED) {
  ...
  if (position & NODE_DOCUMENT_POSITION.DOCUMENT_POSITION_CONTAINED_BY) {
    let child = nodeB;
    while (child.parentNode !== nodeA) {
      child = child.parentNode;
    }

    return child._treeIndex() < offsetA ? 1 : -1;
  }

  // nodeB is an ancestor of nodeA, i.e. the mirror of the above with the
  // boundary points swapped, so the result is inverted.
  if (position & NODE_DOCUMENT_POSITION.DOCUMENT_POSITION_CONTAINS) {
    let child = nodeA;
    while (child.parentNode !== nodeB) {
      child = child.parentNode;
    }

    return child._treeIndex() < offsetB ? -1 : 1;
  }

  // Disjoint subtrees: tree order alone decides.
  return position & NODE_DOCUMENT_POSITION.DOCUMENT_POSITION_PRECEDING ? 1 : -1;
}
