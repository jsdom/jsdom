...
exports.implementation = class ReadableStreamImpl {
  constructor(globalObject, [underlyingSource, strategy]) {
    ...
    if (underlyingSourceDict.type === "bytes") {
      ...
      aos.SetUpReadableByteStreamControllerFromUnderlyingSource(globalObject, this, underlyingSource, underlyingSourceDict, highWaterMark);
    } else {
      assert(!("type" in underlyingSourceDict));
      const sizeAlgorithm = ExtractSizeAlgorithm(strategy);
      const highWaterMark = ExtractHighWaterMark(strategy, 1);
      aos.SetUpReadableStreamDefaultControllerFromUnderlyingSource(globalObject, this, underlyingSource, underlyingSourceDict, highWaterMark, sizeAlgorithm);
    }
  }
