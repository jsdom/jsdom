...
exports.implementation = class NodeIteratorImpl {
  ...
  _adjustNodePointer(referenceNode, pointerBeforeReferenceNode, toBeRemovedNode) {
    ...
    if (pointerBeforeReferenceNode) {
      const next = toBeRemovedNode._nextAfterSubtree(this.root);
      if (next !== null) {
        return [next, true];
      }
    }
    ...
    const newNode = previousSibling === null ?
      ...
      previousSibling._lastInclusiveDescendant();
    ...
    while (true) {
      if (direction === "next") {
        if (!this._candidatePointerBeforeReferenceNode) {
          const following = this._candidateReferenceNode._nextInTree(this.root);
        ...
        if (this._candidatePointerBeforeReferenceNode) {
          const preceding = this._candidateReferenceNode._previousInTree(this.root);
