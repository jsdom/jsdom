...
exports.implementation = class NodeIteratorImpl {
  ...
  // https://dom.spec.whatwg.org/#concept-nodeiterator-pre-remove
  _preRemoveSteps(toBeRemovedNode) {
  ...
  _adjustNodePointer(referenceNode, pointerBeforeReferenceNode, toBeRemovedNode) {
    ...
    if (pointerBeforeReferenceNode) {
      let next = null;
      let candidateForNext = treeHelpers.following(toBeRemovedNode, { skipChildren: true });
      while (candidateForNext !== null) {
        if (this.root.contains(candidateForNext)) {
          next = candidateForNext;
          break;
        }
        candidateForNext = treeHelpers.following(candidateForNext, { skipChildren: true });
      }
    ...
    const newNode = previousSibling === null ?
      ...
      treeHelpers.lastInclusiveDescendant(toBeRemovedNode.previousSibling);
    ...
    while (true) {
      if (direction === "next") {
        if (!this._candidatePointerBeforeReferenceNode) {
          const following = treeHelpers.following(this._candidateReferenceNode, { root: this.root });
        ...
        if (this._candidatePointerBeforeReferenceNode) {
          const preceding = treeHelpers.preceding(this._candidateReferenceNode, { root: this.root });
