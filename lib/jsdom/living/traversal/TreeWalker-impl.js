...
class TreeWalkerImpl {
	_filterNode(node) {
		...
		if (!((1 << n) & this.whatToShow)) return FILTER_SKIP;
		const filter = this.filter;
		if (isNull(filter)) return FILTER_ACCEPT;
		let result;

		if (typeof filter === "function") result = filter(idlUtils.wrapperForImpl(node));  //non-standard
		else result = filter.acceptNode(idlUtils.wrapperForImpl(node));

		if (result === true) result = FILTER_ACCEPT;  //non-standard
		return result;
	};
	...
	_traverseChildren(type) {
		...
		main: while (true) {
			...
			if (result === FILTER_ACCEPT) return (this._currentNode = node);
			...
			while (true) {
				...
				if (isNull(parent) || parent === this.root || parent === this._currentNode) return null;
				node = parent;
			}
		}
	};
	...
	_traverseSiblings(type) {
		...
		while (true) {
			...
			if (isNull(node) || node === this.root) return null;
			if (this._filterNode(node) === FILTER_ACCEPT) return null;
		}
	};
	...
	constructor(args, privateData) {
		...
		//this.expandEntityReferences = privateData.expandEntityReferences;  //removed from living
	...
	set currentNode(node) {
		if (isNull(node)) throw new DOMException(DOMException.NOT_SUPPORTED_ERR, "Cannot set currentNode to null");
	...
	parentNode() {
		...
		while (!isNull(node) && node !== this.root) {
			node = node.parentNode;
			if (!isNull(node) && this._filterNode(node) === FILTER_ACCEPT) return (this._currentNode = node);
		}
		return null;
	};
	...
	previousNode() {
		...
		while (node !== this.root) {
			...
			if (node === this.root || isNull(node.parentNode)) return null;
			node = node.parentNode;
			if (this._filterNode(node) === FILTER_ACCEPT) return (this._currentNode = node);
		}
	...
	nextNode() {
		...
		while (true) {
			...
			//if (isNull((node = idlUtils.wrapperForImpl(domSymbolTree.following(node, {root: this.root}))))) return null;
			do {
				if (node === this.root) return null;
