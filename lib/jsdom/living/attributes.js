...
exports.hasAttribute = function (element, A) {
  const attributesNNM = idlUtils.implForWrapper(element)._attributes;
...
exports.getAttributeByName = function (element, name) {
  ...
  const cache = idlUtils.implForWrapper(element)._attributes[INTERNAL].attributesByNameMap;
  const entry = cache.get(name);
  if (!entry) {
    return null;
  }
...
exports.getAttributeByNameNS = function (element, namespace, localName) {
  ...
  const attributeList = idlUtils.implForWrapper(element)._attributes[INTERNAL].attributeList;
  for (let i = 0; i < attributeList.length; ++i) {
...
exports.copyAttributeList = function (sourceElement, destElement) {
  ...
  for (const sourceAttr of idlUtils.implForWrapper(sourceElement)._attributes[INTERNAL].attributeList) {
...
exports.attributeListsEqual = function (elementA, elementB) {
  ...
  const listA = idlUtils.implForWrapper(elementA)._attributes[INTERNAL].attributeList;
  const listB = idlUtils.implForWrapper(elementB)._attributes[INTERNAL].attributeList;
...
exports.hasAttributes = function (element) {
  ...
  return idlUtils.implForWrapper(element)._attributes[INTERNAL].attributeList.length > 0;
};
