...
function invokeInlineListeners(object, event) {
  ...
  if (inlineListener) {
    const document = object._ownerDocument || object._document || (wrapper && wrapper._ownerDocument);
...
function invokeEventListeners(listeners, target, eventImpl) {
  ...
  const document = target._ownerDocument || target._document || (wrapper && wrapper._ownerDocument);
  // Will be falsy for windows that have closed
  if (!document) {
    return;
  }
  ...
  if (target._document) {
    target = target._document._defaultView;
  }
