...
class EventTargetImpl {
  ...
  removeEventListener(type, callback, options) {
    ...
    options = normalizeEventHandlerOptions(options, ["capture"], ["passive", "once"]);
  ...
  if (inlineListener) {
    ...
    if (document && (!object.nodeName || document.implementation._hasFeature("ProcessExternalResources", "script"))) {
      invokeEventListeners([{
        ...
        options: normalizeEventHandlerOptions(false, ["capture", "once", "passive"])
      }], object, event);
    }
  }
}
 ...
 * @param   {Array} keysBlackList    - options not allowed (optional)
 * @returns {Object} object containing at least the  "defaultBoolKeys"
 */
function normalizeEventHandlerOptions(options, defaultBoolKeys, keysBlackList) {
  // normalize the options argument in order to get always an object
  ...
  try {
    // add the default options properties
    defaultOptions = defaultBoolKeys.reduce((obj, key) => {
      obj[key] = Boolean(options[key]);
      return obj;
    }, {});

    // somehow according to the specs users could pass also other properties as options
    additionalOptions = Object.keys(options).filter(k => !defaultBoolKeys.includes(k)).reduce((obj, key) => {
      if (!keysBlackList.includes(key)) {
        obj[key] = options[key];
      }
    ...
    // should the errors be simply swallowed here?!
    defaultOptions = { capture: false };
  }
