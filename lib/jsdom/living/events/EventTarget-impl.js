...
const { retargetForPointerCapture, implicitlyReleaseAfterDispatch } = require("../helpers/pointer-capture");
...
class EventTargetImpl {
  ...
  dispatchEvent(eventImpl) {
    ...
    // Retargets to the pointer capture target override, if any, and processes pending pointer capture (firing
    // gotpointercapture/lostpointercapture as needed). This only ever does anything for pointer events, and bails
    // out immediately otherwise via a PointerEvent.isImpl() brand check, like the click activation behavior check
    // below already does for MouseEvent.
    const dispatchTargetImpl = retargetForPointerCapture(this, eventImpl);
    const result = dispatchTargetImpl._dispatch(eventImpl);

    implicitlyReleaseAfterDispatch(this, eventImpl);
...
function invokeEventListeners(struct, eventImpl, phase) {
  ...
  if (listeners === null || !listeners[eventImpl.type]) {
    return;
  }
...
function appendToEventPath(
  ...
  const itemInShadowTree = isNode(invocationTarget) && isShadowRoot(invocationTarget.getRootNode());
