...
exports.decodeString = function decodeString(buffer, options) {
  let encoding = options.contentType ? whatwgEncoding.labelToName(options.contentType.get("charset")) : null;

  const bomEncoding = whatwgEncoding.getBOMEncoding(buffer);
  if (bomEncoding) {
  ...
  return { data: whatwgEncoding.decode(buffer, encoding), encoding };
};
  ...
  for (let i = 0; i < l; i++) {
    ...
    if (c === 0x3C) {
      ...
      if (c1 === 0x21 && c2 === 0x2D && c3 === 0x2D) {
        ...
        charset = whatwgEncoding.labelToName(charset);
        if (!charset) {
          continue;
        }
        return charset;
