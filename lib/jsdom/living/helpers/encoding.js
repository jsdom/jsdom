...
function canonicalizeEncoding(enc) {
  return enc ? enc.toLowerCase() : null;
}

function normalizeEncoding(enc) {
  return enc ? (encodingsTable[enc] || null) : null;
}

exports.decodeString = function decodeString(buffer, contentType, defaultEncoding, detectMetaCharset) {
  defaultEncoding = normalizeEncoding(canonicalizeEncoding(defaultEncoding)) || "UTF-8";
  let charset = contentType ? normalizeEncoding(canonicalizeEncoding(contentType.get("charset"))) : null;
  if(buffer.length >= 2) {
