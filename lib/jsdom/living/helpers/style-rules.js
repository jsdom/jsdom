...
function getComputedStyleDeclaration(elementImpl) {
  ...
  // TODO: Remove parentElement check later.
  if (cachedDeclaration && elementImpl.parentElement) {
...
function prepareComputedStyleDeclaration(elementImpl, styleCache) {
  ...
  // Step 1: Apply stylesheet rules.
  applyStyleSheetRules(elementImpl, declaration);

  // Step 2: Apply inline styles.
  applyInlineStyles(elementImpl.style, declaration);

  // Step 3: Apply inherited styles.
  applyInheritedStyles(elementImpl, declaration, styleCache);

  // TODO: Step 4: Apply font size in pixel.
  // applyFontSize(elementImpl, declaration, styleCache);
...
function applyStyleSheetRules(elementImpl, declaration) {
  // Prepare parsedDefaultStyleSheet.
...
function applyInheritedStyles(elementImpl, declaration, styleCache) {
  // Prepare inheritedProperties.
  if (!inheritedProperties.size) {
    for (const [property, { inherited, initial }] of propertyDefinitions) {
      // Shorthand properties are omitted; their values are resolved by longhands.
      if (inherited === "yes" && initial && !/\s/.test(initial)) {
        if (/[A-Z]/.test(initial)) {
          inheritedProperties.set(property, asciiLowercase(initial));
        } else {
          inheritedProperties.set(property, initial);
        }
      }
    }
  }
  ...
  if (parentElement) {
    const parentDeclaration = styleCache.has(parentElement) ?
      styleCache.get(parentElement) :
      prepareComputedStyleDeclaration(parentElement, styleCache);
    for (let i = 0; i < parentDeclaration.length; i++) {
      const property = parentDeclaration.item(i);
      const currentValue = declaration.getPropertyValue(property);
      if (currentValue) {
        if (asciiLowercase(currentValue) === "inherit") {
          const value = parentDeclaration.getPropertyValue(property);
          declaration.setProperty(property, value);
        }
      } else if (inheritedProperties.has(property) || property.startsWith("--")) {
        const value = parentDeclaration.getPropertyValue(property);
        declaration.setProperty(property, value);
      }
    }
    ...
    for (const [property, value] of inheritedProperties) {
