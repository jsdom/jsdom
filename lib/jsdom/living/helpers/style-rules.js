...
function getComputedStyleDeclaration(elementImpl) {
  const cachedDeclaration = elementImpl._ownerDocument._styleCache.get(elementImpl);
  if (cachedDeclaration && cachedDeclaration._readonly) {
...
// Computes and caches the cascade (stylesheet + inline style) for an element.
// Returns the cached declaration directly; callers that need an independent copy
// (like getComputedStyleDeclaration's public return value) must clone it themselves.
function prepareComputedStyleDeclaration(elementImpl) {
  const styleCache = elementImpl._ownerDocument._styleCache;
  const existing = styleCache.get(elementImpl);
  if (existing) {
    return existing;
  }
...
function getCascadedPropertyValue(element, property) {
  return prepareComputedStyleDeclaration(element).getPropertyValue(property);
}
