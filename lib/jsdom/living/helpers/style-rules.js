...
function handleSheet(sheet, elementImpl, declaration, specificities) {
  const rootElementImpl = elementImpl._ownerDocument.documentElement;
  for (const rule of sheet.cssRules) {
      ...
      if (rule.constructor.name === "CSSImportRule") {
        ...
        if (rule.styleSheet !== null && (rule.media.length === 0 || hasMediaScreen)) {
          for (const innerRule of rule.styleSheet.cssRules) {
            if (innerRule.selectorText === ":root" && elementImpl !== rootElementImpl) {
              const { ast, match } = matches(rule, rootElementImpl);
              if (match) {
                handleRule(innerRule, declaration, specificities, ast, true);
              }
            } else {
    ...
    } else if (rule.selectorText === ":root" && elementImpl !== rootElementImpl) {
      const { ast, match } = matches(rule, rootElementImpl);
      if (match) {
        handleRule(rule, declaration, specificities, ast, true);
      }
    } else {
...
function handleRule(rule, declaration, specificities, ast, custom) {
  const { style } = rule;
