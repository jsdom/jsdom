...
function getDeclarationForElement(elementImpl) {
  ...
  if (cachedDeclaration) {
    ...
    // TODO: Remove when options are ready.
  ...
  forEachMatchingSheetRuleOfElement(elementImpl, rule => {
    for (let i = 0; i < rule.style.length; i++) {
      ...
      handleProperty(property, rule.style, declaration);
    }
  });

  for (let i = 0; i < elementImpl.style.length; i++) {
    const property = elementImpl.style[i];
    handleProperty(property, elementImpl.style, declaration);
  }
...
function forEachMatchingSheetRuleOfElement(elementImpl, handleRule) {
  ...
  handleSheet(parsedDefaultStyleSheet, elementImpl, handleRule);
  for (const sheet of elementImpl._ownerDocument.styleSheets._list) {
    handleSheet(sheet, elementImpl, handleRule);
  }
}

function handleSheet(sheet, elementImpl, handleRule) {
...
function handleProperty(property, style, declaration) {
