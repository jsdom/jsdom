...
function prepareComputedStyleDeclaration(elementImpl, { styleCache }) {
  ...
  applyStyleSheetRules(elementImpl, {
    declaration,
    specificities: new Map()
  });

  for (let i = 0; i < elementImpl.style.length; i++) {
    const property = elementImpl.style[i];
    handleProperty(property, elementImpl, {
      declaration,
      inline: true,
      style: elementImpl.style
    });
  }
...
function applyStyleSheetRules(elementImpl, opt) {
  if (!parsedDefaultStyleSheet) {
    parsedDefaultStyleSheet = cssom.parse(defaultStyleSheet);
  }

  handleSheet(parsedDefaultStyleSheet, elementImpl, opt);
  for (const sheet of elementImpl._ownerDocument.styleSheets._list) {
    handleSheet(sheet, elementImpl, opt);
  }
}

function handleSheet(sheet, elementImpl, opt) {
  for (const rule of sheet.cssRules) {
    if (rule.media) {
        ...
        if (rule.styleSheet !== null && (rule.media.length === 0 || hasMediaScreen)) {
          for (const innerRule of rule.styleSheet.cssRules) {
            if (matches(innerRule, elementImpl)) {
              opt.selectorText = innerRule.selectorText;
              handleRule(innerRule, elementImpl, opt);
            }
          }
        }
      ...
      opt.selectorText = rule.selectorText;
      handleRule(rule, elementImpl, opt);
    }
  }
}

function handleRule(rule, elementImpl, opt) {
  for (let i = 0; i < rule.style.length; i++) {
    const property = rule.style[i];
    opt.style = rule.style;
    handleProperty(property, elementImpl, opt);
  }
}

function handleProperty(property, elementImpl, { declaration, inline, selectorText, specificities, style }) {
  if (style.getPropertyPriority(property)) {
    setProperty(property, elementImpl, { declaration, style });
  } else if (!declaration.getPropertyPriority(property)) {
    if (inline) {
      setProperty(property, elementImpl, { declaration, style });
    } else if (selectorText) {
      const { value: specificity } = Specificity.max(...Specificity.calculate(selectorText));
...
function setProperty(property, elementImpl, { declaration, style }) {
  const value = style.getPropertyValue(property);
  // TODO: Add more CSS-wide keyword handlers, e.g. inherit, initial etc.
