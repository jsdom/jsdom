...
function handleRule(rule, elementImpl, declaration, specificities) {
  ...
  if (match) {
    handleStyle(rule.style, declaration, specificities, ast ?? rule.selectorText);
  }
}

function handleStyle(style, declaration, specificities, astOrSelector) {
  for (let i = 0; i < style.length; i++) {
    const property = style[i];
    handleProperty(property, declaration, style, specificities, astOrSelector);
  }
}

function handleProperty(property, declaration, style, specificities, astOrSelector) {
  const value = style.getPropertyValue(property);
  const priority = style.getPropertyPriority(property);
  if (priority) {
    declaration.setProperty(property, value, priority);
  } else if (!declaration.getPropertyPriority(property)) {
    const { value: specificity } = Specificity.max(...Specificity.calculate(astOrSelector));
