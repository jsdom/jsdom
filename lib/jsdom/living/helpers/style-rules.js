...
function getComputedValue(element, property) {
  ...
  if (computedValue === "as-specified") {
    ...
    let value = "";
    if (specifiedValue === "initial") {
      value = initial;
    } else if (specifiedValue === "inherit") {
      if (element?.parentElement?.nodeType === NODE_TYPE.ELEMENT_NODE) {
        return getComputedValue(element.parentElement, property);
      }
      value = initial;
    } else if (/^[a-z]+/i.test(specifiedValue)) {
      value = asciiLowercase(specifiedValue);
    }
    if (value && (systemColors.has(value) || deprecatedAliases.has(value))) {
      let key = value;
