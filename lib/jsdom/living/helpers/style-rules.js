...
function handleRule(rule, elementImpl, declaration, specificities) {
  ...
  if (cssText.includes("--")) {
    handleRuleForCustomProperty(rule, elementImpl, declaration, specificities);
  }
  const { ast, match } = matches(selectorText, elementImpl);
  if (match) {
    handleStyle(style, declaration, specificities, ast ?? selectorText);
  }
}

function handleRuleForCustomProperty(rule, elementImpl, declaration, specificities) {
  const { selectorText, style } = rule;
  const rootImpl = elementImpl._ownerDocument.documentElement;
  const ancestors = [];
  ...
  for (const ancestorImpl of ancestors.toReversed()) {
    if (selectorText === ":root") {
      if (ancestorImpl === rootImpl) {
        const { ast, match } = matches(selectorText, rootImpl);
        if (match) {
          handleStyle(style, declaration, specificities, ast ?? selectorText, true);
        }
      }
    } else {
      const { ast, match } = matches(selectorText, ancestorImpl);
      if (match) {
        handleStyle(style, declaration, specificities, ast ?? selectorText, true);
      }
    }
...
function handleStyle(style, declaration, specificities, ast, custom) {
  for (let i = 0; i < style.length; i++) {
