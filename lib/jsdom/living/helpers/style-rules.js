...
const { upgradeStyleDeclarations } = require("../cssom/helpers/upgrade-stylesheet");
...
function getComputedStyleDeclaration(elementImpl) {
  ...
  if (cachedDeclaration) {
    ...
    const clonedDeclaration = new CSSStyleDeclaration(elementImpl._globalObject, undefined, opts);
...
function prepareComputedStyleDeclaration(elementImpl, { styleCache }) {
  ...
  const opts = prepareCssstyleOpts(elementImpl);
  const declaration = new CSSStyleDeclaration(elementImpl._globalObject, undefined, opts);
...
function applyStyleSheetRules(elementImpl, declaration) {
  if (!parsedDefaultStyleSheet) {
    ...
    // TODO(cssom-upstream): Remove this conversion when @acemir/cssom allows
    // declaration-class injection and default stylesheet parse paths can opt into it.
    upgradeStyleDeclarations(parsedDefaultStyleSheet, elementImpl._globalObject);
  }
