...
function getCascadedPropertyValue(propertyRequest) {
  let style = new exports.ResolvedProperty();
...
function makeCssVarSubstitutions(propertyRequest, partiallyResolvedValue) {
  ...
  while(cssVar !== null) {
    ...
    if (cssVar) {
      ...
      const cssVarSubstitute = getSingleCssVarSubstitute(
        propertyRequest, cssVar);
...
function replaceKeywords(propertyRequest, resolvedProperty) {
  let value = resolvedProperty.value;

  const substituteDefaultForKeyword = (match, object, string) => {
    return getDefaultValue(propertyRequest, match).value;
  };
  
  value = value.replace(/\binitial\b/, substituteDefaultForKeyword);
  value = value.replace(/\binherit\b/, substituteDefaultForKeyword);
  value = value.replace(/\brevert\b/, substituteDefaultForKeyword);
  value = value.replace(/\bunset\b/, substituteDefaultForKeyword);
  return new exports.ResolvedProperty(value, resolvedProperty.priority);
}
