...
function handleSheet(sheetImpl, elementImpl, declaration, specificities) {
  for (const ruleImpl of sheetImpl._cssRules._list) {
    // CSSImportRule (type 3) — has media and a sub-stylesheet
    if (ruleImpl.type === 3) {
      let hasMediaScreen = false;
      for (let i = 0; i < ruleImpl._media._list.length; i++) {
        hasMediaScreen = ruleImpl._media._list[i] === "screen";
        if (hasMediaScreen) {
          break;
        }
      }
      if (ruleImpl._styleSheet !== null && (ruleImpl._media._list.length === 0 || hasMediaScreen)) {
        for (const innerRule of ruleImpl._styleSheet._cssRules._list) {
    ...
    // CSSMediaRule (type 4) — has media and nested rules
      ...
      if (hasMediaScreen) {
        for (const innerRule of ruleImpl._cssRules._list) {
          handleRule(innerRule, elementImpl, declaration, specificities);
        }
      }
    // CSSStyleRule (type 1)
...
function handleRule(ruleImpl, elementImpl, declaration, specificities) {
  const { ast, match } = matches(ruleImpl._selectorText, elementImpl);
  if (match) {
