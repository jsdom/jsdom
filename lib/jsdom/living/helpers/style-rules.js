...
function getComputedStyleDeclaration(elementImpl) {
  ...
  const declaration = prepareComputedStyleDeclaration(elementImpl, {
    styleCache
  });
...
function handleRule(rule, elementImpl, { declaration }) {
...
function handleProperty(property, elementImpl, { declaration, style }) {
  const stylePriority = style.getPropertyPriority(property);
  const declarationPriority = declaration.getPropertyPriority(property);
  // TODO: Compare specificity too.
  if (!declarationPriority || stylePriority) {
