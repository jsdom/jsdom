...
exports.isValidEmailAddress = (emailAddress, multiple = false) => {
  ...
  return this.regExpMatchValues(emailAddressRegexp, emailAddress, { split: multiple });
};
...
// https://html.spec.whatwg.org/#concept-fe-values
exports.regExpMatchValues = (regexp, testValue, { split = false }) => {
  if (split && testValue.indexOf(",") > 0) {
    return splitOnCommas(testValue).every(value => regexp.test(value));
  }
