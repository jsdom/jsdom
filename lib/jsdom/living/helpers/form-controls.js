...
exports.isValidEmailAddress = (emailAddress, multiple = false) => {
  const emailAddressRegex = new RegExp("^[a-zA-Z0-9.!#$%&'*+\\/=?^_`{|}~-]+@[a-zA-Z0-9]" +
    "(?:[a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?(?:\\.[a-zA-Z0-9](?:[a-zA-Z0-9-]{0,61}" +
    "[a-zA-Z0-9])?)*$", "i");
  if (multiple && emailAddress.indexOf(",") > 0) {
    return emailAddress.split(",").every(singleEmailAddress => emailAddressRegex.test(singleEmailAddress));
  }
