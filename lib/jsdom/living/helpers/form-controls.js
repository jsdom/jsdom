...
const treeHelpers = require("./dom-tree");
...
function getLabelAssociations(root) {
  ...
  for (const descendant of treeHelpers.treeIterator(root)) {
...
// Legacy approximation of form-owner changes caused by tree mutations below an ancestor form.
// `parent` is the insertion parent or the former removal parent. This does not assign form owners; `formOwner()`
// still computes them on demand. Replace this when parser-established form associations can be tracked.
exports.updateRadioButtonGroupsForTreeChange = (root, parent) => {
  if (closest(parent, "form") === null) {
    return;
  }

  for (const element of treeHelpers.treeIterator(root)) {
    if (element._namespaceURI === HTML_NS && element._localName === "input" && element._checkedness) {
      element._removeOtherRadioCheckedness();
    }
  }
};
