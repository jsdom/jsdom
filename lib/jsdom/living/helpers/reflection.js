...
function reflectLimitedToNonNegativeGreaterThanZero(prototype, idlAttr, contentAttr, defaultValue = 1) {
  mixin(prototype, {
    get [idlAttr]() {
      const attr = this.getAttributeNS(null, contentAttr);
      if (attr !== null) {
        const parsed = parseNonNegativeInteger(this.getAttributeNS(null, contentAttr));
        if (parsed !== null && parsed >= 1 && parsed <= 2147483647) {
          return parsed;
        }
      }
      return defaultValue;
    ...
    set [idlAttr](value) {
      ...
      this.setAttributeNS(null, contentAttr, String(n));
    }
  });
}
exports.reflectLimitedToNonNegativeGreaterThanZero = reflectLimitedToNonNegativeGreaterThanZero;

// https://html.spec.whatwg.org/#limited-to-only-non-negative-numbers-greater-than-zero-with-fallback
function reflectLimitedToNonNegativeGreaterThanZeroWithFallback(prototype, idlAttr, contentAttr, defaultValue) {
  mixin(prototype, {
    get [idlAttr]() {
      const attr = this.getAttributeNS(null, contentAttr);
      if (attr !== null) {
...
exports.reflectLimitedToNonNegativeGreaterThanZeroWithFallback = reflectLimitedToNonNegativeGreaterThanZeroWithFallback;
