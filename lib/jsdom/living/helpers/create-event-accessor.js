...
const Window = require("../generated/Window.js");
...
exports.appendHandler = function appendHandler(el, eventName) {
  el.addEventListener(eventName, event => {
    ...
    const specialError = ErrorEvent.isImpl(event) && event.type === "error" && Window.is(event.currentTarget);
  ...
  Object.defineProperty(obj, "on" + event, {
    ...
    get() { // https://html.spec.whatwg.org/#getting-the-current-value-of-the-event-handler
      ...
      if (value.body !== undefined) {
        const isWindow = Window.isImpl(this);
        let element;
        let document;
        if (isWindow) {
          element = null;
        ...
        const window = isWindow && this.document ? this : document.defaultView;
        ...
        let fn;
        const createFunction = idlUtils.wrapperForImpl(document.defaultView).Function;

        if (event === "error" && element === null) {
          const compileFunction = (window._vmContext && vm.compileFunction) || ((code, params) => {
            let wrapperBody = `with (window) { return function onerror(${params.join(",")}) {
  ${code}
}; }`;
            if (document) {
            ...
            return createFunction(
              "window",
              wrapperBody
            )(idlUtils.wrapperForImpl(window));
          });

          fn = compileFunction(body, ["event", "source", "lineno", "colno", "error"], {
            parsingContext: window._vmContext,
          ...
          if (!window._vmContext || !vm.compileFunction) {
            argNames.push("window");
            args.push(idlUtils.wrapperForImpl(window));
          }
...
return function on${event}(event) {
  ${body}
          ...
          fn = compileFunction(wrapperBody, argNames, {
            parsingContext: window._vmContext,
            filename: document && document.URL
          })(...args);
        }
