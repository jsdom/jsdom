...
function links(node) {
  return node[symbol] || emptyLinks;
}

function ensureLinks(node) {
  const data = node[symbol];
  if (data) {
    return data;
  }
  node[symbol] = new Links();
  return node[symbol];
}

function parent(node) {
  return links(node).parent;
}

function firstChild(node) {
  return links(node).firstChild;
}

function lastChild(node) {
  return links(node).lastChild;
}

function nextSibling(node) {
  return links(node).nextSibling;
}

function previousSibling(node) {
  return links(node).previousSibling;
}

function hasChildren(node) {
  return links(node).firstChild !== null;
}

function childrenCount(node) {
  return links(node).childCount;
}

function lastInclusiveDescendant(node) {
  let child;
  while ((child = lastChild(node))) {
    node = child;
  }
  return node;
}
...
function nextInTree(node, root) {
  return firstChild(node) || nextAfterSubtree(node, root);
}
...
class SiblingsIterator {
  ...
  next() {
    ...
    this.current = nextSibling(value);
...
class PreviousSiblingsIterator extends SiblingsIterator {
  next() {
    ...
    this.current = previousSibling(value);
    return { value, done: false };
  }
}
...
function childrenIterator(node, { reverse = false } = {}) {
  return reverse ? new PreviousSiblingsIterator(node, lastChild(node)) : new SiblingsIterator(node, firstChild(node));
}

function nextSiblingsIterator(node) {
  return new SiblingsIterator(node, nextSibling(node));
}

function previousSiblingsIterator(node) {
  return new PreviousSiblingsIterator(node, previousSibling(node));
}
...
function index(node) {
  ...
  let child = cached.length === 0 ? parentData.firstChild : nextSibling(cached.at(-1));
  while (child !== node) {
    links(child).cachedIndex = cached.length;
    cached.push(child);
    child = nextSibling(child);
  }
...
function compareTreePosition(left, right) {
  ...
  if (leftParent === rightParent) {
    ...
    return index(left) < index(right) ? TreePosition.FOLLOWING : TreePosition.PRECEDING;
  }
  ...
  while (a !== null || b !== null) {
    ...
    if (a === b) {
      return index(leftAncestors.at(-1)) < index(rightAncestors.at(-1)) ?
  ...
  return index(leftAncestors[i]) < index(rightAncestors[j]) ? TreePosition.FOLLOWING : TreePosition.PRECEDING;
}
...
module.exports = {
  symbol, TreePosition,
  parent, firstChild, lastChild, nextSibling, previousSibling, hasChildren, childrenCount,
  ...
  childrenToArray, treeToArray, index, compareTreePosition,
