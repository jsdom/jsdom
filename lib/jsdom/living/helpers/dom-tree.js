...
function compareTreePosition(left, right) {
  ...
  const { ancestor, leftChild, rightChild } = commonAncestor(left, right);
  if (ancestor === null) {
    return NODE_DOCUMENT_POSITION.DOCUMENT_POSITION_DISCONNECTED;
  }
  if (ancestor === left) {
...
// Also return the children where disjoint paths diverge, for tree-order comparisons.
...
module.exports = {
  ...
  compareTreePosition, commonAncestor,
