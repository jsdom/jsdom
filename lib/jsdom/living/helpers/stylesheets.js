...
exports.createStylesheet = (sheetText, elementImpl, baseURLString, parserOptions) => {
  ...
  const sheetImpl = parseStylesheet(sheetText, elementImpl._globalObject, {
    ...parserOptions,
    mediaText,
    // If we're given an existing styleSheet wrapper (from @import), get its impl
    styleSheet: parserOptions.styleSheet ? implForWrapper(parserOptions.styleSheet) : undefined,
    onError(err) {
  ...
  const sheet = wrapperForImpl(sheetImpl);
