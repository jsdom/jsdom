...
exports.createStylesheet = (sheetText, elementImpl, baseURLString, parserOptions) => {
  // Get the media text from the element's media attribute for the stylesheet
  let mediaText = "";
  if (parserOptions.ownerNode && elementImpl.hasAttributeNS && elementImpl.hasAttributeNS(null, "media")) {
    mediaText = elementImpl.getAttributeNS(null, "media");
  }

  const sheetImpl = parseStylesheet(sheetText, elementImpl._globalObject, {
    ...parserOptions,
    mediaText,
    onError(err) {
  ...
  scanForImportRules(elementImpl, sheetImpl.cssRules._list, baseURLString);
...
function scanForImportRules(elementImpl, ruleImpls, baseURLString) {
  for (const ruleImpl of ruleImpls) {
    if (ruleImpl.cssRules) {
      // @media rule: keep searching inside it.
      scanForImportRules(elementImpl, ruleImpl.cssRules._list, baseURLString);
      ...
      if (parsed) {
        exports.fetchStylesheet(elementImpl, parsed.href, ruleImpl);
      }
    }
  }
}
