...
function parseStylesheet(cssText, globalObject, options = {}) {
  ...
  for (const node of children) {
    ...
    if (ruleImpl) {
      sheetImpl._cssRules._list.push(ruleImpl);
    }
  }
...
function parseRule(ruleText, globalObject) {
  ...
  const rules = sheetImpl._cssRules._list;
  if (rules.length === 0) {
    throw DOMException.create(globalObject, [
      "Failed to parse the rule.",
      "SyntaxError"
...
function parseKeyframeRule(ruleText, globalObject) {
  ...
  const keyframesRule = sheetImpl._cssRules._list[0];
  if (!keyframesRule || !keyframesRule._cssRules || keyframesRule._cssRules._list.length === 0) {
    throw DOMException.create(globalObject, [
  ...
  const rule = keyframesRule._cssRules._list[0];
  rule._parentStyleSheet = null;
...
function translateStyleRule(node, globalObject, parentSheet, parentRule) {
  ...
  const { implForWrapper: implFor } = require("../../../generated/idl/utils.js");
  const parentIsScope = parentRule && "_start" in implFor(parentRule);
    ...
    try {
      ...
      for (const decl of blockAst.children.toArray()) {
        if (decl.type === "Declaration") {
          ruleImpl._style.setProperty(decl.property, generateFromNode(decl.value).trim(), decl.important ? "important" : "");
        }
      }
    } catch {
      // empty block is fine
  ...
  if (inNestingContext) {
    ...
    for (const child of block.children.toArray()) {
      if (child.type === "Declaration") {
        ...
        if (childRule) {
          ruleImpl._cssRules._list.push(childRule);
        }
        ...
        if (nestedRules.length > 0) {
          ...
          for (const nestedRule of nestedRules) {
            ruleImpl._cssRules._list.push(nestedRule);
          }
        }
      }
    }
  ...
  if (node.block && node.block.children) {
    ...
    for (const child of node.block.children.toArray()) {
      if (child.type === "Rule") {
        ...
        ruleImpl._cssRules._list.push(keyframeImpl);
      }
    }
  }
    ...
    if (child.type === "Declaration") {
      ...
      if (!child.property.startsWith("--") && containsTopLevelBrace(rawValue)) {
        ...
        if (split) {
          ...
          if (split.rule) {
            ruleImpl._cssRules._list.push(split.rule);
          }
...
function flushNestedDeclarations(declarations, globalObject, parentSheet, parentRule, parentRuleImpl) {
  ...
  parentRuleImpl._cssRules._list.push(nestedDeclImpl);
}
