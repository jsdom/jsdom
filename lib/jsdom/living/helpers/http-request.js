  ...
  _processResponse(response) {
    ...
    if (redirectAddress) {
      ...
      if (((statusCode === 301 || statusCode === 302) && this._options.method === "POST") ||
        ...
        this._requestBodyBuffers = [];
        // this.removeMatchingHeaders(/^content-length/i);
      }
      let previousHostName = this.removeMatchingHeaders(/^host$/i);
      if (!previousHostName) {
      ...
      if (redirectAddress !== 1) {
        ...
        if (nextURL.hostname !== previousHostName) {
          this.removeMatchingHeaders(/^authorization$/i);
        }
