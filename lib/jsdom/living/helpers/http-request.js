...
RedirectableRequest.prototype.setCookies = function (res) {
  ...
  if (jar && typeof jar.setCookieSync === "function" && Array.isArray(cookies)) {
    try {
...
RedirectableRequest.prototype._performRequest = function () {
  ...
  const request = nativeProtocol.request({ ...this._options, headers: {} }, this._onNativeResponse);
  this._currentUrl = format(this._options);
  this._currentRequest = request;
  Object.entries(this._options.headers).forEach(([k, v]) => {
    self._currentRequest.setHeader(k, v);
  });
  this.getCookie();
  if (this._sendingAuth) {
    // re-send the request with auth headers attached
    let authString = typeof this._options.xhrAuth === "string" ? this._options.xhrAuth : "";
    if (authString === "") {
      // Check user and pass options
...
RedirectableRequest.prototype._processResponse = function (response) {
  ...
  if (location && this._options.followRedirects !== false &&
    ...
    newRequest();
  } else if (statusCode === 401 && !this._sendingAuth) {
...
function urlToOptions(urlObject) {
  const { username, password, protocol, hash, search, href, pathname } = urlObject;
