...
class Request extends Writable {
  constructor(uriHref, options) {
    super();
    this._sanitizeOptions(options); /** @todo revise this method */
  ...
  _performRequest() {
    ...
    if (!(urlOptions.host || (urlOptions.hostname && urlOptions.port))) {
      this.abort();
      this.emit("error", new Error(`Invalid URI: ${urlOptions.href}`));
      return;
    }
