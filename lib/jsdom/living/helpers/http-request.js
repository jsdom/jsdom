...
module.exports = class Request extends Writable {
  ...
  _performRequest() {
    ...
    if (http.request.length === 2) {
      // The spread operator only spreads own propertise,
      // but href, origin, protocol, etc. all are setters and getters of prototype of urlOptions.
      // { ...urlOptions } produces empty object.
      const { href, origin, protocol, username, password, host, hostname, port, pathname, search } = urlOptions;
      const urlOptionsClone =
        { href, origin, protocol, username, password, host, hostname, port, pathname, search, path: pathname };
      this._currentRequest = request({ ...urlOptionsClone, ...this._requestOptions }, response => {
