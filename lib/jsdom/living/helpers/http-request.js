...
module.exports = class Request extends Writable {
  ...
  // Helper method for serializing instance data to JSON format
  // Without this method, the test suite will halt
  toJSON() {
    const { method, headers } = this._options;
    return { uri: new URL(this._currentURL), method, headers };
  }
  ...
  _performRequest() {
    ...
    const message = request(this._currentURL, this._options, response => {
    ...
    if (this._isRedirect) {
      ...
      const self = this;
      // Writes data to the request sequentially
    ...
    if (!this._areCookiesDisabled && jar && Array.isArray(cookies)) {
      try {
        cookies.forEach(cookie => {
          jar.setCookieSync(cookie, this._currentURL, { ignoreError: true });
        });
      } catch (e) {
        this.emit("error", e);
      }
