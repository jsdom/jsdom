...
RedirectableRequest.prototype._processResponse = function (response) {
  ...
  response.request = {};
  response.request.headers = this._headers;
  ...
  if (location && this._options.followRedirects !== false &&
    ...
    if (((statusCode === 301 || statusCode === 302) && this._options.method === "POST") ||
      ...
      this._requestBodyBuffers = [];
      removeMatchingHeaders(/^content-/i, this._headers);
    }

    // Drop the Host header, as the redirect might lead to a different host
    const previousHostName = removeMatchingHeaders(/^host$/i, this._headers) ||
    ...
    if (redirectUrlParts.hostname !== previousHostName) {
      removeMatchingHeaders(/^authorization$/i, this._headers);
    }
    ...
    const isCompressed = self._headers["Accept-Encoding"] === "gzip, deflate";
