...
function createElement(
  ...
  const { _sdGlobalObject } = document;
  ...
  if (definition !== null && definition.name !== localName) {
    ...
    result = elementInterface.createImpl(_sdGlobalObject, [], {
    ...
    if (synchronousCE) {
      try {
        ...
        if (result._attributeList.length !== 0) {
          throw DOMException.create(_sdGlobalObject, ["Unexpected attributes.", "NotSupportedError"]);
        }
        if (domSymbolTree.hasChildren(result)) {
          throw DOMException.create(_sdGlobalObject, ["Unexpected child nodes.", "NotSupportedError"]);
        }
        if (domSymbolTree.parent(result)) {
          throw DOMException.create(_sdGlobalObject, ["Unexpected element parent.", "NotSupportedError"]);
        }
        if (result._ownerDocument !== document) {
          throw DOMException.create(_sdGlobalObject, ["Unexpected element owner document.", "NotSupportedError"]);
        }
        if (result._namespaceURI !== namespace) {
          throw DOMException.create(_sdGlobalObject, ["Unexpected element namespace URI.", "NotSupportedError"]);
        }
        if (result._localName !== localName) {
          throw DOMException.create(_sdGlobalObject, ["Unexpected element local name.", "NotSupportedError"]);
        }
        ...
        result = interfaceWrapper.createImpl(_sdGlobalObject, [], {
          ownerDocument: document,
          localName,
          namespace: HTML_NS,
          prefix,
...
function internalCreateElementNSSteps(document, namespace, qualifiedName, options) {
  const extracted = validateAndExtract(document._sdGlobalObject, namespace, qualifiedName);
