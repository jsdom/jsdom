...
function notifyMutationObservers() {
  ...
  // There is a mismatch between the MutationObserver and the slotchange event handler invocations in the spec. For now,
  // instead of iterating over the list of all the MutationObserver, we only iterate over the MutationObserver
  // containing mutation records.
  // Revisit once resolved: https://github.com/whatwg/dom/issues/713
  const notifyList = mutationObserverList.filter(mo => {
    return mo._recordQueue.length;
  });

  const signalList = [...signalSlotList];

  signalSlotList.splice(0, signalList.length);
