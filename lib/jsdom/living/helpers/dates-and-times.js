...
function parseTimeString(str) {
  ...
  // The digits after the U+002E FULL STOP are a fraction of a second, so "1" is 100 milliseconds.
  const millisecond = matches[4] !== undefined ? Number(matches[4].padEnd(3, "0")) : 0;
...
// A normalized local date and time string wants its time "expressed as the shortest possible string
// for the given time", so unlike serializeTime this omits an all-zero fractional part and any
// trailing zeros within one.
function serializeShortestTime({ hour, minute, second, millisecond }) {
  const hourStr = `${hour}`.padStart(2, "0");
  const minuteStr = `${minute}`.padStart(2, "0");
  if (second === 0 && millisecond === 0) {
    return `${hourStr}:${minuteStr}`;
  ...
  const millisecondStr = `${millisecond}`.padStart(3, "0").replace(/0+$/, "");
  return `${hourStr}:${minuteStr}:${secondStr}.${millisecondStr}`;
}

function serializeNormalizedDateAndTime({ date, time }) {
  return `${serializeDate(date)}T${serializeShortestTime(time)}`;
}
