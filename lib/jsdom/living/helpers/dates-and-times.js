 ...
 * @param {*} type Check DATE_TIME_TYPES object for the types
 */
function parseDateByType(value, type) {
  switch (type) {
    case DATE_TIME_TYPES.DATE:
      return isValidDateString(value) ? new Date(value) : null;
    case DATE_TIME_TYPES.TIME:
      return isValidTimeString(value) ? new Date("1970-01-01 " + value) : null;
    case DATE_TIME_TYPES.MONTH:
      return isValidMonthString(value) ? new Date(value + "-01") : null;
    case DATE_TIME_TYPES.WEEK: {
      if (isValidWeekString(value)) {
        const { week, year } = parseWeekString(value);
...
function getDateOfWeek(week, year) {
  if (isValidWeekString(serializeWeek({ year, week }))) {
    const date = (week - 1) * 7; // 1st of January + 7 days for each week
