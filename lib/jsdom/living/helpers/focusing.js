...
exports.fixupFocus = (elImpl, opt = {}) => {
  const { attribute, event } = opt;
  const { _lastFocusedElement: lastFocused, body } = elImpl._ownerDocument;
  if (!lastFocused || lastFocused === body ||
      (event?.type === "click" && lastFocused === elImpl)) {
    return;
  }
  let fixup;
  if (event) {
    if (event.type === "click" && !focusableFormElements.has(elImpl._localName)) {
      fixup = true;
    }
  } else {
        ...
        case "style": {
          if (attribute.value) {
            ...
            if (styles.length) {
              ...
              const contentVisibility = styleMap.get("content-visibility");
              if (display === "none" || /^(collapse|hidden)$/.test(visibility)) {
                fixup = true;
              } else if (contentVisibility === "hidden" && lastFocused !== elImpl) {
                fixup = true;
              }
            }
          }
