...
exports.focusFixupRule = document => {
  const target = document._lastFocusedElement;
  if (!target || target === document) {
    return;
  }

  const isFocusable = target.isConnected && exports.isFocusableAreaElement(target);
  // if the focused area of doc is not a focusable area
  if (!isFocusable) {
    // run the focusing steps for doc's viewport
    const viewport = document.body || document.documentElement;
    if (viewport && typeof viewport.focus === "function") {
      viewport.focus();
    } else {
    ...
    // set doc's relevant global object's navigation API's focus changed during ongoing navigation to false.
    const globalObject = document._defaultView;
    if (globalObject) {
      // TODO: jsdom does not fully implement the Navigation API yet.
      // Set an internal flag for future compatibility.
      globalObject._navigationAPI ||= {};
      globalObject._navigationAPI.focusChangedDuringOngoingNavigation = false;
    }
  }
};
