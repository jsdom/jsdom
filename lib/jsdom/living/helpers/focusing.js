...
const { getComputedStyleDeclaration } = require("../css/helpers/computed-style.js");
const { isDisabled } = require("./form-controls.js");
const { firstChildWithLocalName } = require("./traversal.js");
const { createAnEvent } = require("./events.js");
const { HTML_NS, SVG_NS } = require("./namespaces.js");
const { flatTreeInclusiveAncestors } = require("./shadow-dom.js");
const { isRenderedElement } = require("./svg/render.js");
...
exports.isFocusableAreaElement = elImpl => {
  ...
  if (elImpl._namespaceURI === HTML_NS && !elImpl._ownerDocument._defaultView) {
    return false;
  }
  if (!elImpl._ownerDocument._defaultView || !elImpl.isConnected) {
    return false;
  }

  const ancestors = flatTreeInclusiveAncestors(elImpl);
  if (!ancestors) {
    return false;
  }

  for (const current of ancestors) {
    if (current.nodeType === NODE_TYPE.ELEMENT_NODE) {
      const declaration = getComputedStyleDeclaration(current);
      if (declaration.getPropertyValue("display") === "none") {
        return false;
      }
    }
  }
