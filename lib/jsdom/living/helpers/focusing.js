...
exports.focusFixupRule = activeElement => {
  if (!activeElement) {
    return;
  }

  const document = activeElement._ownerDocument;
  const { isConnected } = activeElement;
  const isFocusable = exports.isFocusableAreaElement(activeElement);

  if (isConnected && isFocusable) {
    return;
  }

  const fallbackTarget = document.body || document.documentElement || document;

  if (document._lastFocusedElement === fallbackTarget) {
    return;
  }

  if (isConnected) {
    exports.fireFocusEventWithTargetAdjustment("blur", activeElement, fallbackTarget);
    exports.fireFocusEventWithTargetAdjustment("focusout", activeElement, fallbackTarget, { bubbles: true });
  }

  document._lastFocusedElement = fallbackTarget;

  if (isConnected && fallbackTarget && fallbackTarget.nodeType === 1) {
    exports.fireFocusEventWithTargetAdjustment("focus", fallbackTarget, activeElement);
    exports.fireFocusEventWithTargetAdjustment("focusin", fallbackTarget, activeElement, { bubbles: true });
  }
};
