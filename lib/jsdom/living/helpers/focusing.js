...
function focusUpdateSteps(oldChain, newChain, newFocusTarget) {
  const oldTarget = oldChain.length > 0 ? oldChain[0] : null;
  const newTarget = newChain.length > 0 ? newChain[0] : null;

  const document = (oldTarget || newTarget)._ownerDocument;
  if (newFocusTarget) {
    document._lastFocusedElement = newFocusTarget;
  } else {
    document._lastFocusedElement = document.body || document.documentElement || document;
  }
  document._clearDOMSelector();

  if (oldTarget && oldTarget.nodeType === 1) {
    exports.fireFocusEventWithTargetAdjustment("blur", oldTarget, newTarget);
    exports.fireFocusEventWithTargetAdjustment("focusout", oldTarget, newTarget, { bubbles: true });
  }

  if (newTarget && newTarget.nodeType === 1) {
    exports.fireFocusEventWithTargetAdjustment("focus", newTarget, oldTarget);
    exports.fireFocusEventWithTargetAdjustment("focusin", newTarget, oldTarget, { bubbles: true });
  }
}
...
exports.focusingSteps = target => {
  ...
  const oldChain = (oldTarget && oldTarget !== document) ? [oldTarget] : [];
  const newChain = [target];

  focusUpdateSteps(oldChain, newChain, target);
...
exports.unfocusingSteps = target => {
  const document = target._ownerDocument;
  const oldChain = [target];
  const newChain = [];

  focusUpdateSteps(oldChain, newChain, null);
