...
exports.isFocusableAreaElement = elImpl => {
  ...
  if (elImpl._namespaceURI === HTML_NS) {
    ...
    while (current && current.nodeType === 1) {
      if (current.hasAttributeNS(null, "hidden") || current.hasAttributeNS(null, "inert")) {
        return false;
      }
...
exports.focusFixupRule = elImpl => {
  if (!elImpl) {
    return;
  }

  const { isConnected } = elImpl;
  const isFocusable = exports.isFocusableAreaElement(elImpl);
  if (isConnected && isFocusable) {
    return;
  }

  const fallbackTarget = elImpl._ownerDocument.body || elImpl._ownerDocument.documentElement;
  if (elImpl._ownerDocument._lastFocusedElement === fallbackTarget) {
    return;
  }

  if (isConnected) {
    const target = fallbackTarget || elImpl._ownerDocument;
    exports.fireFocusEventWithTargetAdjustment("blur", elImpl, target);
    exports.fireFocusEventWithTargetAdjustment("focusout", elImpl, target, { bubbles: true });
  }

  elImpl._ownerDocument._lastFocusedElement = fallbackTarget || elImpl._ownerDocument;
  elImpl._ownerDocument._clearDOMSelector();

  if (isConnected && fallbackTarget && fallbackTarget.nodeType === 1) {
    exports.fireFocusEventWithTargetAdjustment("focus", fallbackTarget, elImpl);
    exports.fireFocusEventWithTargetAdjustment("focusin", fallbackTarget, elImpl, { bubbles: true });
  }
};
