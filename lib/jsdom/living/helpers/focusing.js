...
exports.focusFixupRule = document => {
  ...
  if (target.isConnected && exports.isFocusableAreaElement(target)) {
    return;
  }
  ...
  if (target.isConnected) {
    exports.fireFocusEventWithTargetAdjustment("blur", target, fallbackTarget);
    exports.fireFocusEventWithTargetAdjustment("focusout", target, fallbackTarget, { bubbles: true });
  }

  document._lastFocusedElement = fallbackTarget;
  document._clearDOMSelector();

  if (target.isConnected && fallbackTarget.nodeType === 1) {
    exports.fireFocusEventWithTargetAdjustment("focus", fallbackTarget, target);
    exports.fireFocusEventWithTargetAdjustment("focusin", fallbackTarget, target, { bubbles: true });
  }
};
