...
module.exports = function reportException(window, error, filenameHint) {
  ...
  const fileName = (pieces && pieces[2]) || filenameHint || window.document.URL;
  const lineNumber = (pieces && parseInt(pieces[3])) || 0;
  const columnNumber = (pieces && parseInt(pieces[4])) || 0;

  // Window might already be an implementation instance, or it could be a wrapper
  const windowImpl = idlUtils.tryImplForWrapper(window);
  ...
  if (!handled) {
    ...
    windowImpl._virtualConsole.emit("jsdomError", jsdomError);
  }
};
