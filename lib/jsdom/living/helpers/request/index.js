...
Request.prototype.start = function () {
  ...
  const { agent, headers, rejectUnauthorized, localAddress, method, host, path, port } = self;
  try {
    self.req = self.httpModule.request({ agent, headers, rejectUnauthorized, localAddress, method, host, path, port });
  } catch (err) {
    self.emit("error", err);
    return;
  }
