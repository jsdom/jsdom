...
function getParent(eventTarget, event) {
  // Handle ShadowRoot
  if (isShadowRoot(eventTarget)) {
    const shadowRoot = eventTarget;

    if (!event.composed && shadowRoot === getRoot(event._path[0].item)) {
      return null;
    }
  ...
  // Handle Document
  if (eventTarget.nodeType === NODE_TYPE.DOCUMENT_NODE) {
    if (event.type === "load" || !eventTarget._defaultView) {
      return null;
    }

    return idlUtils.implForWrapper(eventTarget._defaultView);
  }

  // Handle Node
  if (eventTarget._assignedSlot) {
    return eventTarget._assignedSlot;
  }
  return domSymbolTree.parent(eventTarget);
}
