...
function assignSlotableForTree(root) {
  for (const slot of treeHelpers.treeIterator(root)) {
    if (isSlot(slot)) {
      assignSlotable(slot);
    }
  }
...
function findSlotable(slot) {
  ...
  for (const slotable of treeHelpers.treeIterator(root.host)) {
...
function findFlattenedSlotables(slot) {
  ...
  if (slotables.length === 0) {
    for (const child of treeHelpers.childrenIterator(slot)) {
      if (isSlotable(child)) {
...
function findSlot(slotable, openFlag) {
  ...
  for (const child of treeHelpers.treeIterator(shadow)) {
