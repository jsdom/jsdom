...
function assignSlotableForTree(root) {
  for (const slot of root._inclusiveDescendants()) {
    if (isSlot(slot)) {
      assignSlotable(slot);
    }
  }
...
function findSlotable(slot) {
  ...
  for (const slotable of root.host._inclusiveDescendants()) {
...
function findFlattenedSlotables(slot) {
  ...
  if (slotables.length === 0) {
    for (const child of slot._children()) {
      if (isSlotable(child)) {
...
function findSlot(slotable, openFlag) {
  ...
  for (const child of shadow._inclusiveDescendants()) {
...
// https://drafts.csswg.org/css-shadow/#flat-tree
// Returns inclusive element ancestors, nearest first, or null if the element
// is absent from its document's flat tree.
function flatTreeInclusiveAncestors(element) {
  const root = element._ownerDocument.documentElement;
  const ancestors = [];

  for (let current = element; current !== null; current = flatTreeParentElement(current)) {
...
function flatTreeParentElement(element) {
  ...
  if (parent === null || parent.nodeType !== NODE_TYPE.ELEMENT_NODE) {
    return null;
  }

  if (parent._shadowRoot) {
    // Unassigned light children are excluded. `findSlot()` includes closed
    // roots, unlike the public `assignedSlot` getter.
    return findSlot(element);
  }

  if (isSlot(parent) &&
      isShadowRoot(parent.getRootNode()) &&
      parent._assignedNodes.length > 0) {
    // Assigned nodes replace the slot's fallback children.
    return null;
  }
