...
function nodeLength(node) {
  switch (node.nodeType) {
    ...
    default:
      return node._treeChildrenCount();
  }
}
...
function nodeRoot(node) {
  ...
  //
  // We only cache when the root is a Document. `_remove()` in `Node-impl.js` invalidates descendant caches before
  // removing steps, gated on the old parent's connectivity. That gate is true exactly when the tree is rooted at a
  // Document (via `shadowIncludingRoot`). So Document-rooted caches are always properly invalidated on removal.
  //
