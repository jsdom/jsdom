...
const { parseURL, serializeURL, serializeURLOrigin, URL: WHATWGURL } = require("whatwg-url");
...
class WebSocketImpl extends EventTargetImpl {
  constructor(globalObject, [url, protocols]) {
    ...
    const wsOptions = {
      protocols,
      headers: {
        "user-agent": globalObject._userAgent,
        "origin": globalObject._origin
      }
    };

    // Add Authorization header for credentials in URL (workaround for https://github.com/nodejs/undici/issues/4744)
    if (urlRecord.username || urlRecord.password) {
      const credentials = `${urlRecord.username}:${urlRecord.password}`;
      wsOptions.headers.authorization = "Basic " + btoa(credentials);
    }

    // Add cookies from the cookie jar and set up cookie handling for response
    const parsedURLForCookies = new WHATWGURL(url);
    const cookieJar = idlUtils.implForWrapper(globalObject._document)._cookieJar;
    const cookieString = cookieJar.getCookieStringSync(parsedURLForCookies, { http: true });
    if (cookieString) {
      wsOptions.headers.cookie = cookieString;
    }

    // Set up dispatcher with cookie-capturing interceptor
    // Use the window's dispatcher if available, otherwise create a new Agent
    const baseDispatcher = globalObject._dispatcher ?? new Agent();
    wsOptions.dispatcher = baseDispatcher.compose(
      createCookieCapturingInterceptor(headers => {
        // Process Set-Cookie headers from the WebSocket upgrade response
        if (cookieJar && headers["set-cookie"]) {
          ...
          for (const cookie of cookies) {
            try {
              cookieJar.setCookieSync(cookie, parsedURLForCookies, { http: true, ignoreError: true });
            } catch {
    ...
    // Track open sockets for cleanup - only after successful creation
...
// Create an interceptor that captures Set-Cookie headers from WebSocket upgrade responses.
// Uses the "new" handler API (onRequestUpgrade) which provides parsed headers.
function createCookieCapturingInterceptor(onUpgradeHeaders) {
  return dispatch => (opts, handler) => {
    const originalOnRequestUpgrade = handler.onRequestUpgrade;
    if (originalOnRequestUpgrade) {
      handler.onRequestUpgrade = (controller, statusCode, headers, socket) => {
        onUpgradeHeaders(headers);
        return originalOnRequestUpgrade.call(handler, controller, statusCode, headers, socket);
      };
    }
    return dispatch(opts, handler);
  };
