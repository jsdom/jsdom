...
class WebSocketImpl extends EventTargetImpl {
  constructor(globalObject, [url, protocols]) {
    ...
    const wsOptions = {
      dispatcher: globalObject._settings.dispatcher,
      protocols,
      headers: {
    ...
    this._ws = wrapAndRethrowNodeDOMExceptions(() => {
      // Avoid loading the full undici entry point for documents that never create a WebSocket.
    ...
    // Queue event forwarding to work around https://github.com/nodejs/undici/issues/4741 where
    // undici fires events synchronously during `close()`, but the spec requires them to fire asynchronously.
    const document = globalObject._document;
    this._ws.addEventListener("open", () => {
      document._queueATask(() => fireAnEvent("open", this));
    });

    this._ws.addEventListener("message", event => {
      // Capture readyState now, because undici may transition to CLOSED before our task runs,
    ...
    this._ws.addEventListener("error", () => {
      document._queueATask(() => fireAnEvent("error", this));
    });
