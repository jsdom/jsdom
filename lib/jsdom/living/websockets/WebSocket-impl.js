...
class WebSocketImpl extends EventTargetImpl {
  constructor(globalObject, args, privateData) {
    ...
    if (urlRecord === null) {
      throw DOMException.create(this._sdGlobalObject, [`The URL '${url}' is invalid.`, "SyntaxError"]);
    }
    ...
    for (const protocol of protocols) {
      if (!verifySecWebSocketProtocol(protocol)) {
        throw DOMException.create(this._sdGlobalObject, [`The subprotocol '${protocol}' is invalid.`, "SyntaxError"]);
      }
  ...
  _onMessageReceived(data, isBinary) {
    ...
    if (!isBinary) {
      ...
      if (isArrayBuffer(data)) {
        ...
        dataForEvent = copyToArrayBufferInNewRealm(Buffer.concat(data), this._sdGlobalObject);
      } else {
        dataForEvent = copyToArrayBufferInNewRealm(data, this._sdGlobalObject);
      }
    } else { // this.binaryType === "blob"
      if (!Array.isArray(data)) {
        data = [data];
      }
      dataForEvent = Blob.create(this._sdGlobalObject, [data, { type: "" }]);
    }
  ...
  send(data) {
    if (this.readyState === CONNECTING) {
      throw DOMException.create(this._sdGlobalObject, ["Still in CONNECTING state.", "InvalidStateError"]);
    }
    if (this.readyState !== OPEN) {
      return;
    }
