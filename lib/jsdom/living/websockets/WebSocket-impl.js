...
class WebSocketImpl extends EventTargetImpl {
  constructor(globalObject, args, privateData) {
    ...
    // Separately use whatwg-url's `URL` constructor to parse the URL in a way that the `tough-cookie` library can use.
    // Don't just pass the string, or use the Node.js `URL` constructor, as that could lead to inconsistencies.
    const parsedURLForToughCookie = new WHATWGURL(this.url);
    ...
    openingQueues.set(this._ownerDocument, openingQueues.get(this._ownerDocument).then(() => new Promise(resolve => {
      ...
      this._ws = new WebSocket(this.url, protocols, {
        headers: {
          ...
          "cookie": this._ownerDocument._cookieJar.getCookieStringSync(parsedURLForToughCookie, { http: true }),
  ...
  _onMessageReceived(data, isBinary) {
    ...
    // `data` from the internal `this._ws` is always a node `Buffer` if `isBinary` is false, or an `ArrayBuffer` if
    // `isBinary` is true (since we set `this._ws.binaryType = "arraybuffer"`).
    let dataForEvent;
    if (!isBinary) {
      ...
      dataForEvent = Blob.create(this._globalObject, [undefined, { type: "" }], { fastPathArrayBufferToWrap: data });
    }
  ...
  close(code = undefined, reason = undefined) {
    ...
    if (reason !== undefined && utf8Encode(reason).byteLength > 123) {
      throw DOMException.create(this._globalObject, [
  ...
  send(data) {
    ...
    let length;
    if (Blob.isImpl(data)) {
      // Blob already copies data and keeps it immutably, so we can use its buffer directly
      data = data._bytes;
      length = data.byteLength;
    } else if (typeof data === "string") {
      length = utf8Encode(data).byteLength;
    } else if (isArrayBuffer(data)) {
      // Copy to ensure data isolation
      data = (new Uint8Array(data)).slice(0);
      length = data.byteLength;
    } else if (ArrayBuffer.isView(data)) {
      // Copy to ensure data isolation
      data = (new Uint8Array(data.buffer, data.byteOffset, data.byteLength)).slice(0);
      length = data.byteLength;
    } else {
      length = data.byteLength;
    }
