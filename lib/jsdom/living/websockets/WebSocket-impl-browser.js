...
class WebSocketImpl extends EventTargetImpl {
  constructor(_globalObject, constructorArgs, privateData) {
    super(_globalObject, constructorArgs, privateData);

    this.windowImpl = idlUtils.implForWrapper(globalObject);
    this._ownerDocument = this.windowImpl.document;
    ...
    let openSocketsForWindow = openSockets.get(this.windowImpl._globalProxy);
    if (openSocketsForWindow === undefined) {
      openSocketsForWindow = new Set();
      openSockets.set(this.windowImpl._globalProxy, openSocketsForWindow);
    }
