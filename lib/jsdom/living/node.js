...
module.exports.clone = function (core, node, document, cloneChildren) {
  ...
  switch (node.nodeType) {
    ...
    case core.Node.ELEMENT_NODE:
      copy = document._createElementNoTagNameValidation(node.tagName);
  ...
  if (node._cloningSteps) {
    node._cloningSteps(copy, node, document, cloneChildren);
  }

  if (cloneChildren) {
    for (let i = 0; i < node._childNodes.length; ++i) {
      const childCopy = module.exports.clone(core, node._childNodes[i], document, true);
