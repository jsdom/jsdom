"use strict";
const { appendAttribute } = require("./elements.js");
...
module.exports.clone = function (node, document, cloneChildren) {
  ...
  switch (node.nodeType) {
    ...
    case NODE_TYPE.ELEMENT_NODE:
      ...
      copyAttributeList(node, copy, document);
      break;
...
function copyAttributeList(sourceElement, destElement, document) {
  // Needed by https://dom.spec.whatwg.org/#concept-node-clone

  for (const sourceAttr of sourceElement._attributeList) {
    const destAttr = document._createAttribute(
      sourceAttr._localName,
      sourceAttr._namespace,
      sourceAttr._namespacePrefix,
      sourceAttr._value
    );

    appendAttribute(destElement, destAttr);
  }
}
