    ...
    setImmediate(() => {
      ...
      setImmediate(() => {
        ...
        switch (format) {
          ...
          case "dataURL": {
            let dataUrl = "data:";
            const parsedMIMEType = firstMIMEType([file.type]);
            if (parsedMIMEType && parsedMIMEType.isText()) {
              const fallbackEncoding = whatwgEncoding.getBOMEncoding(data) ||
                whatwgEncoding.labelToName(parsedMIMEType.parameters.get("charset")) || "UTF-8";
              const decoded = whatwgEncoding.decode(data, fallbackEncoding);

              parsedMIMEType.parameters.set("charset", encoding);
              dataUrl += parsedMIMEType.toString();
              dataUrl += ",";
