...
const { firstMIMEType } = require("../../utils.js");
    ...
    setImmediate(() => {
      ...
      setImmediate(() => {
        ...
        switch (format) {
          ...
          case "dataURL": {
            let dataUrl = "data:";
            const parsedContentType = firstMIMEType([file.type]);
            if (parsedContentType && parsedContentType.isText()) {
              const fallbackEncoding = whatwgEncoding.getBOMEncoding(data) ||
                whatwgEncoding.labelToName(parsedContentType.parameters.get("charset")) || "UTF-8";
              const decoded = whatwgEncoding.decode(data, fallbackEncoding);

              parsedContentType.parameters.set("charset", encoding);
              dataUrl += parsedContentType.toString();
              dataUrl += ",";
