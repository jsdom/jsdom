      ...
      setImmediate(() => {
        ...
        switch (format) {
          case "binaryString": {
            ...
            let binaryString = "";
            for (let i = 0; i < file._buffer.length; i++) {
              binaryString += String.fromCharCode(file._buffer[i]);
            }
          ...
          case "dataURL": {
            ...
            this.result = `data:${contentType};base64,${toBase64(file._buffer)}`;
            break;
          }
          case "text": {
            this.result = legacyHookDecode(file._buffer, encodingLabel);
            break;
          }
          case "buffer":
          default: {
            this.result = copyToArrayBufferInTargetRealm(file._buffer.buffer, this._globalObject);
            break;
          }
        }
        this.readyState = READY_STATES.DONE;
