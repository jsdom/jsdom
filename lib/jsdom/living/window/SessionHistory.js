...
const HashChangeEvent = require("../../../generated/idl/HashChangeEvent.js");
const PopStateEvent = require("../../../generated/idl/PopStateEvent.js");
...
class SessionHistory {
  constructor(initialEntry, window) {
    ...
    this._historyTraversalController = new AbortController();
    this._entries = [initialEntry];
    this._currentIndex = 0;
  }

  _queueHistoryTraversalTask(fn) {
    this._window._document._queueATask(fn, { signal: this._historyTraversalController.signal });
  }

  clearHistoryTraversalTasks() {
    this._historyTraversalController.abort();
    this._historyTraversalController = new AbortController();
  }
    ...
    this._queueHistoryTraversalTask(() => {
      ...
      this._queueHistoryTraversalTask(() => {
        ...
        if (specifiedEntry.document !== this.currentEntry.document) {
          ...
          notImplemented(this._window, "traversing history to another Document");
        }
  ...
  traverseHistory(specifiedEntry, flags = {}) {
    if (!specifiedEntry.document) {
      ...
      notImplemented(this._window, "traversing the history to an entry that no longer holds a Document");
    }
    // Not spec compliant, just minimal. Lots of missing steps.

    const nonBlockingEvents = Boolean(flags.nonBlockingEvents);

    const document = this._window._document;
    ...
    if (specifiedEntry.document !== currentEntry.document) {
      ...
      notImplemented(this._window, "traversing the history to an entry with a different Document");
    }
    ...
    if (nonBlockingEvents) {
      document._queueATask(fireEvents);
    } else {
      fireEvents();
    }
  }
