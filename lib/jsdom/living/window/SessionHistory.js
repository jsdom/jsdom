...
class SessionHistory {
  ...
  traverseHistory(specifiedEntry, flags = {}) {
    ...
    specifiedEntry.document._history._state = state;
  ...
  _fireEvents(stateChanged, hashChanged, state, oldURL, newURL) {
    if (stateChanged) {
      this._window.dispatchEvent(PopStateEvent.create(["popstate", {
        bubbles: true,
        state
      }]));
    }
    ...
    if (hashChanged) {
      this._window.dispatchEvent(HashChangeEvent.create(["hashchange", {
        bubbles: true,
  ...
  traverseByDelta(delta) {
    this._queueHistoryTraversalTask(() => {
      ...
      this._queueHistoryTraversalTask(() => {
        ...
        // then cancel that attempt to navigate the browsing context.
        this.clearHistoryTraversalTasks();
        if (specifiedEntry.document && specifiedEntry.document !== this.currentEntry.document) {
          // unload the active document with the recycle parameter set to false
          this.unloadCurrentDocument();
        }
        this.traverseHistory(specifiedEntry);
      });
    });
  ...
  unloadCurrentDocument() {
    const oldDocument = this.currentEntry.document;
    if (oldDocument) {
      oldDocument._defaultView.close();
      for (const e of this._entries) {
        if (e.document === oldDocument) {
          e.document = null;
        }
      }
    }
  ...
  traverseHistory(specifiedEntry, flags = {}) {
    if (!specifiedEntry.document) {
      ...
      // to perform an entry update of entry, and abort these steps.
      this.updateCurrentEntry(specifiedEntry);
      navigate(this._window, specifiedEntry.url, {replacement: true});
      return;
    }
    // Not spec compliant, just minimal. Lots of missing steps.
    ...
    // If the current entry's title was not set by the pushState() or replaceState() methods, then set its title
    // to the value returned by the document.title IDL attribute.
    if (currentEntry.title === undefined && document) {
      currentEntry.title = document.title;
    }
    ...
    if (specifiedEntry.document !== currentEntry.document) {
      this.clearHistoryTraversalTasks();
      specifiedEntry.document._defaultView._sessionHistory = this;
      this._window._onNavigationStart(specifiedEntry.document._defaultView);
      this._window = specifiedEntry.document._defaultView;
      // NOT IMPLEMENTED: some extra steps if the origin is not changing.
    }

    specifiedEntry.document._URL = specifiedEntry.url;
