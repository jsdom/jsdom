...
exports.traverseHistory = (window, specifiedEntry, flags = {}) => {
  ...
  const currentEntry = window._sessionHistory.currentEntry;
  ...
  window._sessionHistory.currentIndex = window._sessionHistory.entries.indexOf(specifiedEntry);
};
...
function navigateToFragment(window, newURL, flags) {
  ...
  if (!flags.replace) {
    ...
    window._sessionHistory.entries.push(newEntry);
