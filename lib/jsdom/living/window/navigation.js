...
exports.chooseBrowsingContext = (currentWindow, name) => {
  ...
  if (name.toLowerCase() === "_self") {
    chosen = currentWindow;
  } else if (name.toLowerCase() === "_parent") {
    chosen = currentWindow._parent;
  } else if (name.toLowerCase() === "_top") {
    chosen = currentWindow._top;
  } else {
    isNew = true;
    notImplemented('target=_blank', currentWindow);
  }
  return {chosen, isNew};
};
...
function navigateFetch(window, newURL, flags) {
  const document = idlUtils.implForWrapper(window._document);
  let newEntry = null;
  const newWindow = loadPage(document, newURL, url => {
    // url may not match newURL due to redirects
    newEntry.url = url;
    newEntry.document._URL = url;
  });
...
function loadPage(oldDocument, newURL, onFinalURL) {
  ...
  if (serializedURL === "about:blank") {
    ...
    resourceLoader.loadBase(
      ...
      (err, html, response) => {
        ...
        onFinalURL(whatwgURL.parseURL(response.request.uri.href + (newURL.hash || '')));
        if (response) {
