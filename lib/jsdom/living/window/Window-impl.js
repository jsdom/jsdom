...
const createXMLHttpRequest = require("../xmlhttprequest");
...
class WindowImpl extends EventTargetImpl {
  constructor(args, options) {
    ...
    this._globalProxy = options.wrapper;

    this.__timers = Object.create(null);

    // Set up the window as if it's a top level window.
    // If it's not, then references will be corrected by frame/iframe code.
    this._parent = this._top = this;
    ...
    this.navigator = Navigator.create([], { userAgent: options.userAgent });
    ...
    this.Image = function (width, height) {
      ...
      // To fully reflect browser behavior we shall only set width / height if they are
      // actually specified.
  ...
  setTimeout(fn, ms) {
    ...
    return startTimer(this, setTimeout, clearTimeout, this.latestTimerId++, fn, ms, args);
  }
  ...
  setInterval(fn, ms) {
    ...
    return startTimer(this, setInterval, clearInterval, this.latestTimerId++, fn, ms, args);
  }
...
module.exports = { implementation: WindowImpl };
