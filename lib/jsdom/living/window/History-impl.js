...
exports.implementation = class HistoryImpl {
  ...
  go(delta) {
    ...
    if (delta === 0) {
      ...
      this._queueHistoryTraversalTask(() => {
        const newIndex = this._window._sessionHistory.currentIndex + delta;
        if (newIndex < 0 || newIndex >= this._window._sessionHistory.length) {
          return;
        }

        const specifiedEntry = this._window._sessionHistory.entries[newIndex];
  ...
  _sharedPushAndReplaceState(data, title, url, methodName) {
    ...
    if (methodName === "pushState") {
      ...
      this._window._sessionHistory.entries.push({
        document: this._document,
        stateObject: data,
        title,
        url: newURL
      });
      this._window._sessionHistory.currentIndex = this._window._sessionHistory.length - 1;
    } else {
    ...
    this._latestEntry = this._window._sessionHistory.currentEntry;
  }
