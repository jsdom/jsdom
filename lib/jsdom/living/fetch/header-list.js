...
class HeaderList {
  ...
  sortAndCombine() {
    ...
    for (const lowerName of names) {
      const { name, values } = this._headers.get(lowerName);
  ...
  * [Symbol.iterator]() {
  ...
  /**
   * Creates a HeaderList from undici's raw headers format.
   * Raw headers are a flat array: [name, value, name, value, ...]
   * where names and values are Buffer objects decoded as Latin-1 per HTTP spec.
   */
  static fromUndiciRawHeaders(rawHeaders) {
    const list = new HeaderList();
