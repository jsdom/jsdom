...
const { isFocusableAreaElement, fireFocusEventWithTargetAdjustment } = require("../helpers/focusing");
...
class HTMLOrSVGElementImpl {
  ...
  get inert() {
    return this.hasAttributeNS(null, "inert");
  }

  set inert(value) {
    if (value) {
      this.setAttributeNS(null, "inert", "");
    } else {
      this.removeAttributeNS(null, "inert");
    }
  }

  focus() {
    if (!isFocusableAreaElement(this)) {
      return;
    }
    ...
    if (blurTarget) {
      fireFocusEventWithTargetAdjustment("blur", blurTarget, this);
      if (ownerDocument._lastFocusedElement !== ownerDocument) {
        return;
      }

      fireFocusEventWithTargetAdjustment("focusout", blurTarget, this, { bubbles: true });
      if (ownerDocument._lastFocusedElement !== ownerDocument) {
        return;
      }
    }
    ...
    if (frameElement) {
      ...
      if (framePrevious !== frameElement) {
        ...
        if (frameBlurTarget) {
          fireFocusEventWithTargetAdjustment("blur", frameBlurTarget, null);
          if (frameOwnerDoc._lastFocusedElement !== frameOwnerDoc) {
            return;
          }

          fireFocusEventWithTargetAdjustment("focusout", frameBlurTarget, null, { bubbles: true });
          if (frameOwnerDoc._lastFocusedElement !== frameOwnerDoc) {
            return;
          }
        }
    ...
    fireFocusEventWithTargetAdjustment("focus", this, relatedTargetForFocus);
    if (ownerDocument._lastFocusedElement !== this) {
      return;
    }

    fireFocusEventWithTargetAdjustment("focusin", this, relatedTargetForFocus, { bubbles: true });
    ownerDocument.getSelection().collapse(this, 0);
  }

  blur() {
    if (this._ownerDocument._lastFocusedElement !== this || !isFocusableAreaElement(this)) {
      return;
    }
    ...
    fireFocusEventWithTargetAdjustment("blur", this, null);
    if (this._ownerDocument._lastFocusedElement !== this._ownerDocument) {
      return;
    }

    fireFocusEventWithTargetAdjustment("focusout", this, null, { bubbles: true });
