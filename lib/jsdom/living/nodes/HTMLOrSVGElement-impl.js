...
class HTMLOrSVGElementImpl {
  ...
  focus() {
    ...
    // The focus update steps remove the shared Document from both chains before dispatching events.
    // https://html.spec.whatwg.org/multipage/interaction.html#focus-update-steps
    if (previous && previous !== ownerDocument) {
    ...
    const relatedTarget = previous === ownerDocument ? null : previous;
    focusing.fireFocusEventWithTargetAdjustment("focus", this, relatedTarget);
    focusing.fireFocusEventWithTargetAdjustment("focusin", this, relatedTarget, { bubbles: true });
