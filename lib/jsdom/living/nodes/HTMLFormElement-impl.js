...
class HTMLFormElementImpl extends HTMLElementImpl {
  ...
  _getElementNodes() {
    const nodes = domSymbolTree.treeToArray(this, {
      filter: node => this._filterElement(node)
    });

    if (!this.hasAttributeNS(null, "id")) {
      return nodes;
    }

    const documentNodes = domSymbolTree.treeToArray(this.ownerDocument, {
      filter: node => this._filterExternalElement(node)
    });

    return nodes.concat(documentNodes);
  }

  _filterExternalElement(node) {
    const id = this.getAttributeNS(null, "id");

    if (
      this._filterElement(node) &&
      !this.contains(node) &&
      node.hasAttributeNS(null, "form") &&
      node.getAttributeNS(null, "form") === id
    ) {
      return true;
    }
