...
class HTMLFormElementImpl extends HTMLElementImpl {
  ...
  checkValidity() {
    const controls = descendantsByHTMLLocalNames(this, new Set(["input", "select", "textarea"]));
    return !controls.some(element => !element.checkValidity());
  }

  reportValidity() {
    const controls = descendantsByHTMLLocalNames(this, new Set(["input", "select", "textarea"]));
    const invalidControls = controls.filter(element => !element.checkValidity());
