...
class HTMLFormElementImpl extends HTMLElementImpl {
  ...
  checkValidity() {
    // TODO: This should include only descendants that are not part of a fieldset,
    // separately fieldsets need to be processed as well. 
    const descendants = descendantsByHTMLLocalNames(this,
      new Set(["input", "select", "textarea"]));
    return !descendants.some(element => element.checkValidity && !element.checkValidity());
  }
