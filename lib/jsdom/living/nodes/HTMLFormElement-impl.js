...
class HTMLFormElementImpl extends HTMLElementImpl {
  ...
  get elements() {
    const baseQuery = node => isListed(node) && (node._localName !== "input" || node.type !== "image");
    ...
    return HTMLCollection.createImpl(this._globalObject, [], {
      ...
      query: () => {
        ...
        const documentNodes = domSymbolTree.treeToArray(this.ownerDocument, {
          filter: node => baseQuery(node) && !this.contains(node) && node.hasAttributeNS(null, "form") && node.getAttributeNS(null, "form") === id
        });
