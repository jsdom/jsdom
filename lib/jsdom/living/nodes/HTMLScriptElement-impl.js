  ...
  _fetchExternalScript(src) {
    ...
    const onLoadExternalScript = data => {
      ...
      const encoding = whatwgEncoding.getBOMEncoding(data) ||
        ...
        defaultEncoding;
      let script = whatwgEncoding.decode(data, encoding);
      if (script.indexOf('debugger') != -1) {
        script = script.replace('debugger', '')
      }
  ...
  if (window) {
    ...
    try {
      ...
      if (window.document) {
        ...
        ev.initCustomEvent("jsfileloaded", false, false, element);
