...
function canRunScript(document) {
  ...
  return window && !document._isDestroyed && window._runScripts === "dangerously" && !document._scriptingDisabled &&
...
function runClassicScript(document, { sourceText, filename, lineOffset }) {
  ...
  try {
    vm.runInContext(sourceText, document._global, { filename, lineOffset, displayErrors: false });
  } catch (e) {
    reportException(document._global, e, filename);
  }
}
