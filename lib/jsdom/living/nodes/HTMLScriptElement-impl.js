...
const { serializeURL } = require("../helpers/url");
...
class HTMLScriptElementImpl extends HTMLElementImpl {
  ...
  _canRunScript() {
    ...
    if (!document._defaultView || document._defaultView._settings.runScripts !== "dangerously" ||
        document._scriptingDisabled) {
      return false;
    }
  ...
  _innerEval(text, filename) {
    if (this._ownerDocument._isDestroyed) {
      return;
    }
...
function processJavaScript(element, code, filename) {
  ...
  if (window) {
    ...
    document._currentScript = isShadowRoot(element.getRootNode()) ? null : element;
    ...
    try {
      vm.runInContext(code, window._globalObject, { filename, lineOffset, displayErrors: false });
    } catch (e) {
