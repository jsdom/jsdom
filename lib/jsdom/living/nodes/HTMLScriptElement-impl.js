...
class HTMLScriptElementImpl extends HTMLElementImpl {
  ...
  _canRunScript() {
    ...
    if (!document._defaultView || document._defaultView._runScripts !== "dangerously") {
      return false;
    }
  ...
  _canBeFetch() {
    const documentImpl = this._ownerDocument.implementation;

    if (!documentImpl._hasFeature("FetchExternalResources", this.tagName.toLowerCase())) {
      return false;
    }
