...
class HTMLScriptElementImpl extends HTMLElementImpl {
  ...
  _childrenChangedSteps(options) {
    super._childrenChangedSteps(options);

    // Removing a child never triggers execution.
    if (options?.isRemoval) {
      return;
    }

    // The parser fills in the text before it has seen the closing tag, and runs the script from
    // _poppedOffStackOfOpenElements() once it has.
