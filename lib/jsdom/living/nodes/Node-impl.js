...
class NodeImpl extends EventTargetImpl {
  ...
  * _liveRanges() {
  ...
  getRootNode(options) {
    return options && options.composed ? shadowIncludingRoot(this) : nodeRoot(this);
  }
  ...
  _insert(nodeImpl, childImpl, suppressObservers) {
    ...
    for (const node of nodesImpl) {
      ...
      const rootIsShadow = isShadowRoot(root);
      const isAttached = this._attached || (rootIsShadow && root._host._attached);
      if (rootIsShadow) {
        assignSlotableForTree(root);
      }

      if (isAttached && nodeImpl._attach) {
        node._attach();
        if (node._shadowRoot) {
          if (node._shadowRoot._attach) {
            node._shadowRoot._attach();
          }
        }
      }
