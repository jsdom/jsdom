...
function nodeEquals(a, b) {
  ...
  for (const nodes of simultaneousIterators(treeHelpers.childrenIterator(a), treeHelpers.childrenIterator(b))) {
    if (!nodes[0] || !nodes[1]) {
...
function addSubtreeToDocumentCaches(root) {
  ...
  for (const node of treeHelpers.treeIterator(root)) {
...
class NodeImpl extends EventTargetImpl {
  ...
  * _liveRanges() {
  ...
  _treeIndex() {
    ...
    if (cached[data.cachedIndex] === this) {
      return data.cachedIndex;
    }
  ...
  get childNodes() {
    if (!this._childNodesList) {
      this._childNodesList = NodeList.createImpl(this._globalObject, [], {
        element: this,
        query: () => treeHelpers.childrenToArray(this)
      });
    } else {
  ...
  _invalidateCaches() {
    this._version++;
    this.#memoizedQueries = null;
    for (const ancestor of treeHelpers.ancestorsIterator(this)) {
      ancestor._version++;
  ...
  normalize() {
    ...
    for (const node of treeHelpers.treeToArray(this)) {
      ...
      for (const currentNode of treeHelpers.previousSiblingsIterator(node)) {
      ...
      for (const currentNode of treeHelpers.nextSiblingsIterator(node)) {
        if (currentNode.nodeType !== NODE_TYPE.TEXT_NODE) {
          break;
        }
  ...
  get textContent() {
    switch (this.nodeType) {
      ...
      case NODE_TYPE.ELEMENT_NODE: {
        let text = "";
        for (const child of treeHelpers.treeIterator(this)) {
  ...
  _preInsertValidity(nodeImpl, childImpl, childrenToExclude = new Set()) {
    ...
    if (nodeType === NODE_TYPE.DOCUMENT_FRAGMENT_NODE) {
      ...
      for (const child of treeHelpers.childrenIterator(nodeImpl)) {
        if (
    ...
    if (nodeType === NODE_TYPE.DOCUMENT_FRAGMENT_NODE || nodeType === NODE_TYPE.ELEMENT_NODE) {
      ...
      for (const child of treeHelpers.childrenIterator(this)) {
  ...
  _insert(nodeImpl, childImpl, suppressObservers = false) {
    ...
    for (const node of nodesImpl) {
      ...
      for (const inclusiveDescendant of treeHelpers.shadowIncludingInclusiveDescendantsIterator(node)) {
  ...
  _replaceAll(nodeImpl) {
    const removedNodesImpl = treeHelpers.childrenToArray(this);

    let addedNodesImpl;
    if (nodeImpl === null) {
      addedNodesImpl = [];
    } else if (nodeImpl.nodeType === NODE_TYPE.DOCUMENT_FRAGMENT_NODE) {
      addedNodesImpl = treeHelpers.childrenToArray(nodeImpl);
    } else {
  ...
  _remove(nodeImpl, suppressObservers) {
    ...
    for (const descendant of treeHelpers.treeIterator(nodeImpl)) {
    ...
    if (wasParentConnected) {
      for (const descendantImpl of treeHelpers.shadowIncludingDescendantsIterator(nodeImpl)) {
