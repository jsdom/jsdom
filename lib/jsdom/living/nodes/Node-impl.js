...
class NodeImpl extends EventTargetImpl {
  ...
  * _liveRanges() {
  ...
  _preInsertValidity(nodeImpl, childImpl, childrenToExclude = emptySet) {
  ...
  _replace(nodeImpl, childImpl) {
    const childrenToExclude = this.nodeType === NODE_TYPE.DOCUMENT_NODE ? new Set([childImpl]) : undefined;
    this._preInsertValidity(nodeImpl, childImpl, childrenToExclude);
  ...
  _replaceAll(nodeImpl, removedNodesImpl = domSymbolTree.childrenToArray(this)) {
    let addedNodesImpl;
    if (nodeImpl === null) {
      addedNodesImpl = [];
