...
class NodeImpl extends EventTargetImpl {
  constructor(args, privateData) {
    ...
    this.ownerDocument = privateData.ownerDocument;
    this.readonly = false;
  ...
  get ownerDocument() {
    ...
    return this.nodeType === NODE_TYPE.DOCUMENT_NODE ? null : this.ownerDocument;
  }
  ...
  insertBefore(newChild, refChild) {
    ...
    if (this.nodeName !== "#document" && newChild.ownerDocument !== this.ownerDocument) {
  ...
  _attrModified(name, value, oldValue) {
    ...
    if (name.length > 2 && name[0] === "o" && name[1] === "n") {
      if (value) {
        const w = this.ownerDocument._global;
        const self = proxiedWindowEventHandlers.has(name) && this._localName === "body" ? w : this;
        const vmOptions = { filename: this.ownerDocument._URL, displayErrors: false };
    ...
    if (value !== oldValue && this.ownerDocument &&
      this.ownerDocument.implementation._hasFeature("MutationEvents")) {
      const ev = this.ownerDocument.createEvent("MutationEvents");
  ...
  _attach() {
    ...
    if (this.id) {
      attachId(this.id, this, this.ownerDocument);
    }
  ...
  _detach() {
    ...
    if (this.id) {
      detachId(this.id, this, this.ownerDocument);
    }
  ...
  removeChild(/* Node */ oldChild) {
    ...
    if (this.ownerDocument) {
      this.ownerDocument._runRemovingSteps(oldChild, this, oldPreviousSibling);
    }
    return oldChild;
  } // raises(DOMException);
