  ...
  * _liveRanges() {
  ...
  _preInsertValidity(nodeImpl, childImpl, childrenToExclude) {
    ...
    if (nodeType === NODE_TYPE.DOCUMENT_FRAGMENT_NODE) {
      ...
      const elementChildren = nodeChildren.filter(child => child.nodeType === NODE_TYPE.ELEMENT_NODE);
      const hasTextChild = nodeChildren.some(child => {
        return child.nodeType === NODE_TYPE.TEXT_NODE || child.nodeType === NODE_TYPE.CDATA_SECTION_NODE;
      });

      if (elementChildren.length > 1 || hasTextChild) {
        throw DOMException.create(this._globalObject, [
    ...
    const hasDoctypeChild = parentChildren.some(child => {
    ...
    const hasElementPrecedingChild = childImpl !== null &&
      Array.from(domSymbolTree.previousSiblingsIterator(childImpl)).some(child => {
        return child.nodeType === NODE_TYPE.ELEMENT_NODE;
      });
    const hasElementChild = childImpl === null && parentChildren.some(child => {
      return child.nodeType === NODE_TYPE.ELEMENT_NODE && !childrenToExclude.has(child);
    });

    if (hasDoctypeChild || hasElementPrecedingChild || hasElementChild) {
      throw DOMException.create(this._globalObject, [
        `Invalid insertion of ${nodeImpl.nodeName} node in ${this.nodeName} node.`,
        "HierarchyRequestError"
      ]);
  ...
  _insert(nodeImpl, childImpl, suppressObservers = false) {
    ...
    for (const node of nodesImpl) {
      ...
      for (const inclusiveDescendant of shadowIncludingInclusiveDescendantsIterator(node)) {
        ...
        if (inclusiveDescendant.isConnected && inclusiveDescendant.nodeType === NODE_TYPE.ELEMENT_NODE) {
    ...
    if (nodesImpl.some(node => {
      return node.nodeType === NODE_TYPE.TEXT_NODE || node.nodeType === NODE_TYPE.CDATA_SECTION_NODE;
    })) {
    ...
    for (const node of nodesImpl) {
      for (const inclusiveDescendant of shadowIncludingInclusiveDescendantsIterator(node)) {
        staticNodeList.push(inclusiveDescendant);
      }
    }

    for (const node of staticNodeList) {
      if (node.isConnected && node._postConnectionSteps) {
