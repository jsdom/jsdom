...
class NodeImpl extends EventTargetImpl {
  ...
  insertBefore(newChildImpl, refChildImpl) {
    ...
    if (refChildImpl && refChildImpl.parentNode !== this) {
      throw new DOMException("The object can not be found here.", "NotFoundError");
    }

    // https://dom.spec.whatwg.org/#concept-node-insert

    const nodesImpl = newChildImpl.nodeType === NODE_TYPE.DOCUMENT_FRAGMENT_NODE ?
      domSymbolTree.childrenToArray(newChildImpl) :
      [newChildImpl];
    ...
    for (const nodeImpl of nodesImpl) {
      ...
      if (refChildImpl === null) {
        domSymbolTree.appendChild(this, nodeImpl);
      } else {
        domSymbolTree.insertBefore(refChildImpl, nodeImpl);
      }

      if (
        (this.nodeType === NODE_TYPE.ELEMENT_NODE && this._shadowRoot !== null) &&
        (nodeImpl.nodeType === NODE_TYPE.ELEMENT_NODE || nodeImpl.nodeType === NODE_TYPE.TEXT_NODE)
      ) {
        assignSlot(nodeImpl);
      }
      ...
      this._descendantAdded(this, nodeImpl);
    }
  ...
  removeChild(oldChildImpl) {
    ...
    if (oldChildImpl._assignedSlot) {
      assignSlotable(oldChildImpl._assignedSlot);
    }

    let hasSlotDescendant = oldChildImpl._localName === "slot";
    if (!hasSlotDescendant) {
      for (const child of domSymbolTree.treeIterator(oldChildImpl)) {
    ...
    if (hasSlotDescendant) {
      ...
      assignSlotableForTree(oldChildImpl);
    }
