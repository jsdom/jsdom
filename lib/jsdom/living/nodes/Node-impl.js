...
// TODO: Make this helper respect the spec
...
class NodeImpl extends EventTargetImpl {
      ...
      switch (nodeType) {
        case NODE_TYPE.DOCUMENT_FRAGMENT_NODE: {
          ...
          if (
            ...
            (
              ...
              (childImpl && domSymbolTree.nextSibling(childImpl).nodeType === NODE_TYPE.DOCUMENT_TYPE_NODE)
            )
          ) {
            throw new DOMException(
              `Invalid insertion of ${nodeName} node in ${parentName} node.`,
        ...
        case NODE_TYPE.DOCUMENT_TYPE_NODE:
          if (
            ...
            (childImpl && domSymbolTree.previousSibling(childImpl) === NODE_TYPE.ELEMENT_NODE) ||
  ...
  _preInsert(nodeImpl, childImpl) {
    ...
    this._insert(nodeImpl, childImpl);
