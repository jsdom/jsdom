...
class HTMLElementImpl extends ElementImpl {
  ...
  dispatchEvent(event) {
    ...
    // 3:06 PM Domenic> jeffcarp: yes, although in theory per current spec I think
    // dispatchEvent should not be calling those, so add a comment to that effect
    // saying that this matches browser behavior but not the specs,
    // and linking to https://www.w3.org/Bugs/Public/show_bug.cgi?id=12230
  ...
  click() {
    ...
    // 3:07 PM <Domenic> jeffcarp: hmm that will also require a comment in your
    // implementation of https://html.spec.whatwg.org/multipage/interaction.html#run-synthetic-click-activation-steps
    // saying that the pre-click activation steps and click behavior have been moved to dispatchEvent
