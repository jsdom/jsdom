...
class HTMLElementImpl extends ElementImpl {
  ...
  dispatchEvent(event) {
    ...
    if (event.type === "click") {
      ...
      callHandler(event, '_preClickActivationSteps');

      // TODO: Figure out outcome vs. event.defaultPrevented
      if (event.defaultPrevented) {
        callHandler(event, '_canceledActivationSteps');
      } else {
        callHandler(event, '_activationBehavior');
      }
    }
  ...
  click() {
    ...
    if (idlUtils.wrapperForImpl(this).disabled) {
      return false;
    }
...
function callHandler(event, name) {
  if (event && event.target && event.target[name] && typeof event.target[name] === "function") {
