...
const { labelToName, legacyHookDecode, getBOMEncoding } = require("@exodus/bytes/encoding.js");
...
function fetchFrame(serializedURL, frame, document, contentDoc) {
  ...
  function onFrameLoaded(data) {
    ...
    if (contentDoc._parsingMode === "xml") {
      contentDoc._encoding = labelToName(getBOMEncoding(data) || transportLayerEncodingLabel || "utf-8");
    } else {
      const encoding = labelToName(sniffHTMLEncoding(data, {
