...
async function fetchFrame(serializedURL, frame, document, contentDoc) {
  if (!document._defaultView._loadSubresources) {
    return;
  }
  ...
  try {
    ...
    const finishParsing = contentDoc._startParsing();
    try {
...
function loadFrame(frame, attaching) {
  ...
  if (serializedURL === "about:blank" || url.scheme === "javascript") {
    const finishParsing = noQueue ? null : contentDoc._startParsing();
    ...
    if (noQueue) {
      ...
      fireAnEvent("DOMContentLoaded", contentDoc, undefined, { bubbles: true });
      fireAnEvent("load", contentDoc);
