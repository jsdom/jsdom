...
const { defineGetter, reflectURLAttribute, firstMimeType } = require("../../utils");
  ...
  if (serializedURL === "about:blank") {
    ...
    resourceLoader.load(
      ...
      (html, responseURL, response) => {
        if (response) {
          const parsedMIMEType = firstMimeType([response.headers["content-type"]]);
          if (parsedMIMEType) {
            if (parsedMIMEType.isXML()) {
              contentDoc._parsingMode = "xml";
            }
            contentDoc._encoding = parsedMIMEType.parameters.get("charset");
          }
        }
