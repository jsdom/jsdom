...
function loadFrame(frame, attaching) {
  ...
  const srcdocAttribute = getAttributeValue(frame, "srcdoc");

  if (srcdocAttribute !== "") {
    url = parseURL("about:srcdoc");
  ...
  if (serializedURL === "about:blank" || serializedURL === "about:srcdoc") {
    ...
    if (srcdocAttribute === "") {
      ...
      contentDoc.write(srcdocAttribute);
    }
  ...
  function frameURL() {
    if (url.scheme === "javascript" || serializedURL === "about:blank" || serializedURL === "about:srcdoc") {
      return parentDoc.URL;
    }
