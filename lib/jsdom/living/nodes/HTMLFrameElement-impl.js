...
const xmlTypeRegexp = /^(?:text\/xml|application\/xml|[^;]*?\+xml)(?:;.*)*$/;
...
function loadFrame(frame) {
  ...
  if (url.protocol === "data:") {
    const dataType = url.pathname.match(/^(.+?),.*$/);
    if (dataType) {
      const splittedDataType = dataType[1].split(";");
      isXml = splittedDataType[0].match(xmlTypeRegexp);
    }
  }
  ...
  if (url.href === "about:blank") {
    ...
    resourceLoader.load(frame, url.href, (html, uri, response) => {
      if (response) {
        const type = response.headers["content-type"];
        if (type) {
          const splittedDataType = type.split(";");
          const isXml = splittedDataType[0].match(xmlTypeRegexp);
          const impl = idlUtils.implForWrapper(contentDoc);
          impl._parsingMode = isXml ? "xml" : "html";
        }
      }
