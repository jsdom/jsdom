...
const { parseURL, serializeURL, matchesAboutBlank } = require("../helpers/url");
...
function fetchFrame(url, frame, document, contentDoc) {
  ...
  resourceLoader.fetch(serializeURL(url), {
    urlRecord: url,
    element: frame,
...
function loadFrame(frame, attaching) {
  ...
  // Parse before unloading: the old document might own the blob URL being navigated to.
  ...
  const inheritsOrigin = matchesAboutBlank(url) || url.scheme === "javascript";
  const wnd = window.createWindow({
    parsingMode: "html",
    url: url.scheme === "javascript" ? parentDoc.URL : serializedURL,
    urlRecord: url.scheme === "javascript" ? undefined : url,
    inheritedOrigin: inheritsOrigin ? parentDoc._origin : undefined,
    inheritedOriginIdentity: inheritsOrigin ? parentDoc._originIdentity : undefined,
    ...
    storageQuota: parentDoc._defaultView._storageQuota
  });
  ...
  if (matchesAboutBlank(url)) {
    ...
    fetchFrame(url, frame, parentDoc, contentDoc);
  }
}
