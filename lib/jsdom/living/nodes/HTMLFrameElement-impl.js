...
const { sniffMIMEType } = require("../helpers/mimesniff");
...
function fetchFrame(serializedURL, frame, document, contentDoc) {
  ...
  function onFrameLoaded(data, response) {
    let contentType = MIMEType.parse(response.headers.get("content-type"));
    if (!contentType) {
      // No content-type header - use MIME sniffing
      const sniffedType = sniffMIMEType(data, { sniffScriptable: true });
      contentType = new MIMEType(sniffedType);
    }
