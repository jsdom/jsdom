...
const windows = require("../../browser/windows.js");
...
function loadFrame(frame, attaching) {
  ...
  contentWindow.frameElement = frame;
...
function refreshAccessors(document) {
  ...
  const { length, _globalProxy } = window;

  // delete accessors for all frames
  for (let i = 0; i < length; ++i) {
    delete _globalProxy[i];
  }

  window.length = frames.length;
  Array.prototype.forEach.call(frames, (frame, i) => {
    Object.defineProperty(_globalProxy, i, {
      configurable: true,
      enumerable: true,
      get() {
        return idlUtils.wrapperForImpl(frame.contentWindow);
      }
    });
  });
}
