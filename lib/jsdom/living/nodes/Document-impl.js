...
class DocumentImpl extends NodeImpl {
  ...
  adoptNode(node) {
    ...
    if (DocumentFragment.isImpl(node) && node._host) {
      return node;
    }
  ...
  _adoptNode(node, doItAnyway = false) {
    ...
    if (oldDocument !== newDocument) {
      for (const inclusiveDescendant of shadowIncludingInclusiveDescendantsIterator(node)) {
        if (!doItAnyway && DocumentFragment.isImpl(inclusiveDescendant) && DocumentFragment.isImpl(node) && node._host) {
          // This assumes all _adoptNode callers that can reach this step will remove the children
          // of this DocumentFragment
          continue;
        }
