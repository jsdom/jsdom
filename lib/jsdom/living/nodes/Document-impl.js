...
class DocumentImpl extends NodeImpl {
  ...
  constructor(globalObject, args, privateData) {
    ...
    const parsed = privateData.options.urlRecord ?? whatwgURL.parseURL(urlOption);
    if (parsed === null) {
      throw new TypeError(`Could not parse "${urlOption}" as a URL`);
    }

    this._URL = parsed;
    this._origin = whatwgURL.matchesAboutBlank(parsed) && privateData.options.parentOrigin ?
    ...
    } else if (whatwgURL.matchesAboutBlank(parsed) && privateData.options.parentOriginIdentity !== undefined) {
      this._originIdentity = privateData.options.parentOriginIdentity;
    }
