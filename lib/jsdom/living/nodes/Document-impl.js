...
class DocumentImpl extends NodeImpl {
  constructor(args, privateData) {
    ...
    this._URL = whatwgURL.parseURL(privateData.options.url === undefined ? "about:blank" : privateData.options.url);
    ...
    this._scriptsExecuteASAP = [];
    this._scriptsExecuteAtParseEnd = [];
  ...
  close() {
    ...
    const enqueue = () => {
      this._parserFinishedPromise = null;
    ...
    if (this._parserFinishedPromise) { // parse5 doms
      this._parserFinishedPromise.then(enqueue);
