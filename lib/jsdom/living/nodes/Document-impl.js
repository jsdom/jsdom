...
const whatwgURL = require("../helpers/url");
...
class DocumentImpl extends NodeImpl {
  ...
  constructor(globalObject, args, {
    ...
    const parsed = urlRecord ?? whatwgURL.parseURL(urlOption);
    if (parsed === null) {
      throw new TypeError(`Could not parse "${urlOption}" as a URL`);
    }

    this._URL = parsed;
    this._origin = inheritedOrigin ?? whatwgURL.serializeURLOrigin(this._URL);
    // Keep opaque origins distinct even though their serialization is always "null".
    if (inheritedOriginIdentity !== undefined) {
      this._originIdentity = inheritedOriginIdentity;
    } else if (parsed.blobURLEntry) {
      this._originIdentity = parsed.blobURLEntry.environment._originIdentity;
    } else {
      this._originIdentity = this._origin === "null" ? {} : this._origin;
    }
  ...
  _fallbackBaseURL() {
    if (whatwgURL.matchesAboutBlank(this._URL) && this._defaultView &&
  ...
  _cloneDocument() {
    ...
    copy._originIdentity = this._originIdentity;
    return copy;
  }
}
