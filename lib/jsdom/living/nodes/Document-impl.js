...
class DocumentImpl extends NodeImpl {
  ...
  #parser = null;
  #pendingParsingBlockingScript = null;
  ...
  constructor(globalObject, args, {
    ...
    // The script-blocking style sheet set, with a completion promise for each contributing element.
    this._scriptBlockingStyleSheets = new Map();
  ...
  write(...args) {
    ...
    // `_writeAfterElement` stands in for the parser's insertion point until we have a pausable parser.
    if (!this._writeAfterElement && this._ignoreDestructiveWritesCounter > 0) {
      return;
    }
  ...
  open() {
    ...
    this._startParsing({ scriptCreated: true });
  ...
  close() {
    ...
    if (this.#parser !== null && this.#parser.scriptCreated) {
      this.#parser.finish();
    }
  }

  // Callers must call the returned finish function after parsing their input.
  _startParsing({ scriptCreated = false } = {}) {
    if (this.#parser !== null) {
      this._abort();
    }
    this.#pendingParsingBlockingScript = null;
    this._scriptsToExecuteAfterParsing = [];
    ...
    const parser = {
      ...
      finish: () => {
        if (completion === undefined) {
          // Publish the completion before `readystatechange` listeners can reenter `close()`.
          completion = Promise.withResolvers();
          this.#finishParsing(controller.signal).then(completion.resolve, completion.reject).finally(() => {
  ...
  async #finishParsing(signal) {
    // Our synchronous parsers have already consumed EOF, but parsing-blocking scripts can still be pending.
    while (!signal.aborted && this.#pendingParsingBlockingScript !== null) {
      await this.#pendingParsingBlockingScript;
    }
  ...
  async #waitForStyleSheets(signal) {
    while (!signal.aborted && this._scriptBlockingStyleSheets.size > 0) {
      await Promise.all(this._scriptBlockingStyleSheets.values());
    }
  }

  // The spec has a single pending parsing-blocking script. Since our parsers cannot pause, chain these scripts
  // in parser order.
  _queueParsingBlockingScript(ready, execute) {
    const predecessor = this.#pendingParsingBlockingScript;
    const styleSheets = [...this._scriptBlockingStyleSheets.values()];
    if (ready === null && predecessor === null && styleSheets.length === 0) {
      execute();
      return;
    }
    const completion = Promise.all([predecessor, ready, ...styleSheets]).then(execute).then(() => {
      if (this.#pendingParsingBlockingScript === completion) {
        this.#pendingParsingBlockingScript = null;
      }
    });
    this.#pendingParsingBlockingScript = completion;
  }
