...
function isHTMLElementWithLocalName(node, localName) {
...
/**
 * An embed or object element is exposed when it has no exposed object ancestor. Since jsdom has no
 * plugins and object elements have no content navigable, that reduces to having no object ancestor.
 *
 * https://html.spec.whatwg.org/multipage/dom.html#exposed
 */
function isExposed(element) {
  for (let ancestor = domSymbolTree.parent(element); ancestor; ancestor = domSymbolTree.parent(ancestor)) {
    if (isHTMLElementWithLocalName(ancestor, "object")) {
      return false;
    }
  }
...
function isNamedPropertyElement(node) {
  ...
  return (node._localName !== "embed" && node._localName !== "object") || isExposed(node);
}
...
class DocumentImpl extends NodeImpl {
   ...
   * https://html.spec.whatwg.org/multipage/dom.html#dom-document-nameditem-filter
   */
  get _namedPropertyElements() {
    if (this._namedPropertyElementsCache?.version !== this._version) {
      const map = new Map();
      ...
      for (const element of domSymbolTree.treeToArray(this, { filter: isNamedPropertyElement })) {
      ...
      this._namedPropertyElementsCache = { version: this._version, map };
    }

    return this._namedPropertyElementsCache.map;
  }

  get [idlUtils.supportedPropertyNames]() {
    return this._namedPropertyElements.keys();
  }

  [idlUtils.supportsPropertyName](name) {
    return this._namedPropertyElements.has(name);
  }

  // https://html.spec.whatwg.org/multipage/dom.html#dom-document-nameditem
  [idlUtils.namedGet](name) {
    const elements = this._namedPropertyElements.get(name);
    ...
    this._namedPropertyCollections ??= new WeakValueMap();
    let collection = this._namedPropertyCollections.get(name);
    if (collection === undefined) {
      collection = HTMLCollection.createImpl(this._globalObject, [], {
        element: this,
        query: () => this._namedPropertyElements.get(name) ?? []
      });
      this._namedPropertyCollections.set(name, collection);
    }
