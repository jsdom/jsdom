...
const ResourceQueue = require("../../browser/resource-queue");
...
class DocumentImpl extends NodeImpl {
  ...
  close() {
    const documentImpl = this;
    if (documentImpl.readyState === "complete") {
      return;
    }

    documentImpl._queue.push(() => {
      documentImpl.readyState = "interactive";
      const ev = documentImpl.createEvent("HTMLEvents");
      ev.initEvent("DOMContentLoaded", false, false);
      documentImpl.dispatchEvent(ev);
    })();

    documentImpl._queue.resume(() => {
      documentImpl.readyState = "complete";
