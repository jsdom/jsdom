...
class ElementImpl extends NodeImpl {
  ...
  _attach() {
    ...
    if (this.hasAttributeNS(null, "autofocus")) {
      this._autoFocus();
    }
  ...
  _autoFocus() {
    if (this._ownerDocument.hasFocus()) {
      return;
    }
    ...
    // step 3
    // if (!target._browsingContext) {
    //   return;
    // }

    // step 4
    // if (target._activeSandboxFlags.has(SANDBOXED_AUTOMATIC_FEATURES_BROWSING_CONTEXT_FLAG)) {
    //   return;
    // }
    ...
    let topDoc = target;
    while (topDoc !== topDoc._ownerDocument) {
      topDoc = topDoc._ownerDocument;
    }

    if (!topDoc._autoFocusProcessed) {
      const at = topDoc._autoFocusCandidates.indexOf(this);

      if (at !== -1) {
        topDoc._autoFocusCandidates.splice(at, 1);
      }

      topDoc._autoFocusCandidates.push(this);
    }
  }
