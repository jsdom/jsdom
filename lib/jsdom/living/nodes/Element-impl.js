...
const { focusFixupRule, isFocusableAreaElement } = require("../helpers/focusing");
...
const focusRelatedAttributes = new Set(["disabled", "tabindex", "hidden", "contenteditable"]);
...
class ElementImpl extends NodeImpl {
  ...
  // Overrides call `super` first so document-dependent caches are cleared before further adoption work.
  _adoptingSteps() {
    // `tagName` depends on whether the node document is HTML, even before the element is inserted again.
    this.#cachedTagName = null;
  }

  // https://dom.spec.whatwg.org/#concept-element-attributes-change-ext
  // The attribute data has already changed. Overrides call `super` first so caches and bookkeeping reflect that
  // change before element-specific side effects, which can reenter through e.g. resource request interceptors.
  _attributeChangeSteps(localName, oldValue, value, namespace) {
    this._invalidateCaches();

    windowProperties.elementAttributeModified(this, localName, value, oldValue, namespace);

    if (namespace === null && localName === "id" && this._isInDocumentTree) {
    ...
    // update classList
    if (namespace === null && localName === "class" && this._classList !== undefined) {
    ...
    if (namespace === null) {
      this._attrModifiedSlotableMixin(localName, value, oldValue);
    }

    const isFocusAttributeChanged = namespace === null && focusRelatedAttributes.has(localName);
    const isStyleAttributeChanged = namespace === null && localName === "style";
    if (isFocusAttributeChanged || isStyleAttributeChanged) {
      const activeElement = this._ownerDocument._lastFocusedElement;
      if (activeElement && (activeElement === this || this.contains(activeElement))) {
        if (!isFocusableAreaElement(activeElement)) {
          this._ownerDocument._defaultView.queueMicrotask(() => {
            if (this._ownerDocument._lastFocusedElement === activeElement && !isFocusableAreaElement(activeElement)) {
              focusFixupRule(activeElement);
            }
          });
        }
      }
  ...
  get tagName() {
    ...
    if (this.#cachedTagName === null) {
      if (this._namespaceURI === HTML_NS && this._ownerDocument._parsingMode === "html") {
        this.#cachedTagName = asciiUppercase(this._qualifiedName);
      } else {
        this.#cachedTagName = this._qualifiedName;
      }
    }
  ...
  set outerHTML(markup) {
    ...
    const contextObjectParent = this.parentNode;
