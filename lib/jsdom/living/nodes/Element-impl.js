...
const { createWindow } = require("../../browser/Window");
...
class ElementImpl extends NodeImpl {
  ...
  checkVisibility(options = {}) {
    ...
    const computedStyles = window.createWindow().getComputedStyle(this);
    // 1. The element does not have an associated box
    if (
      computedStyles.display === "none" ||
      computedStyles.display === "contents"
    ) {
      return false;
    }
    // 2. The element `content-visibility` property has (or inherits) a value of `hidden`.
    if (computedStyles.contentVisibility === "hidden") {
      return false;
    }
    // 3. `opacityProperty` (or `checkOpacity`) is `true` and the element `opacity` property has (or inherits) a value of `0`.
    if ((opacityProperty || checkOpacity) && computedStyles.opacity === 0) {
      return false;
    }
    // 4. `visibilityProperty` (or `checkVisibilityCSS`) is `true ` and the element is invisible due to the value of its `visibility` property.
    if (
    ...
    // 5. `contentVisibilityAuto` is `true`, the `content-visibility` property has (or inherits) a value of `auto`, and element rendering is being skipped.
    if (contentVisibilityAuto && computedStyles.contentVisibility === "auto") {
      return false;
    }
    return true;
  }
