...
const { createStyleSheet, removeStyleSheet, scanForImportRules } = require("../css/stylesheets");
...
class HTMLStyleElementImpl extends HTMLElementImpl {
  ...
  _attrModified(name, value, oldValue) {
    ...
    if (name === "media" && this.sheet) {
      this.sheet.media.mediaText = value;
    }
  }
  ...
  _updateAStyleBlock() {
    ...
    const sheet = createStyleSheet({
      media: this.getAttributeNS(null, "media"),
      ownerNode: this,
      rules: childTextContent(this),
      title: this._ownerDocument ? this.getAttributeNS(null, "title") : ""
    });
    scanForImportRules(sheet, this).then(() => fireAnEvent("load", this));
  }
}
