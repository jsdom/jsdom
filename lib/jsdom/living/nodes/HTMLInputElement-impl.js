...
class HTMLInputElementImpl extends HTMLElementImpl {
  ...
  get validity() {
    if (!this._validity) {
      ...
      this._validity = ValidityState.createImpl({
        ...
        rangeOverflow: () => supportsRange() && this._maximum !== null && value() > this._maximum,
        ...
        // datetime-local, month, time and week.
        rangeUnderflow: () => supportsRange() && this._minimum !== null && value() < this._minimum,
        ...
        stepMismatch: () => {
          if (supportsRange() && this.hasAttribute("step")) {
            ...
            if (!isNaN(step)) {
              if (this.type === "week") {
                return (parseWeekString(this.value).week - 1) % step !== 0;
              } else if (this.type === "month") {
                return (parseMonthString(this.value).month - 1) % step !== 0;
              } else if (this.type === "date") {
                return (parseDateString(this.value).day - 1) % step !== 0;
              } else if (this.type === "datetime-local" || this.type === "datetime" ||
                this.type == "time") {
                return number % (step * 1000) !== 0;
              }
        ...
        typeMismatch: () => {
          ...
          if (this.type === "email") {
            ...
            const emailRegex = new RegExp("^[a-zA-Z0-9.!#$%&'*+\\/=?^_`{|}~-]+@[a-zA-Z0-9]" +
              ...
              "[a-zA-Z0-9])?)*$", "i");
            if (this.hasAttribute("multiple") && email.indexOf(",") > 0) {
              return email.split(",").some(oneEmail => !emailRegex.test(oneEmail));
            }
            return !emailRegex.test(email);
          } else if (this.type === "url") {
            const url = this.value;
            return url.length >= 2083 || whatwgURL.parseURL(url) === null;
          }
          return false;
        }
      });
  ...
  checkValidity() {
    ...
    const { validity } = this;
    if (!validity.valid) {
      this.dispatchEvent(Event.createImpl(["invalid", { bubbles: true, cancelable: true }], {}));
    }
