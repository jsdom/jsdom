...
class HTMLInputElementImpl extends HTMLElementImpl {
  ...
  set type(type) {
    ...
    let previousState = valueAttributeMode(this.type);
    let previousSelectabel = allowVariableLengthSelection(this.type)
    this.setAttribute("type", type);
    let currentState = valueAttributeMode(this.type);
    if (previousState != currentState) {
      if (previousState === "value" && (currentState === "default" || currentState === "default/on")) {
        this.setAttribute("value", previousValue);
      } else if (previousState !== "value" && currentState === "value") {
        this.value = previousValue;
      } else if (previousState !== "filename" && currentState === "filename") {
        this._value = "";
        this.setAttribute("value", "");
        this.files.length = 0;
      }

      if (!previousSelectabel && allowVariableLengthSelection(this.type)) {
        this._selectionStart = 0;
        this._selectionEnd = 0;
        this._selectionDirection = "none";
      }
