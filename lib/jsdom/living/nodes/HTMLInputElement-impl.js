...
class HTMLInputElementImpl extends HTMLElementImpl {
  ...
  get validity() {
    if (!this._validity) {
      const value = () => {
        const converter = convertStringToNumberByTypeMap.get(this.type);
        if (converter) {
      ...
      this._validity = ValidityState.createImpl({
        ...
        valueMissing: () => {
          if (this.hasAttribute("required")) {
            ...
            return (isCheckedType && !this.checked) || (!isCheckedType && !value());
          }
          return false;
        },
        ...
        rangeOverflow: () => supportsRange() && value() > this._maximum,
        ...
        // datetime-local, month, time and week.
        rangeUnderflow: () => supportsRange() && value() < this._minimum,
        ...
        typeMismatch: () => {
          if (this.type === "email") {
            ...
            if (this.hasAttribute("multiple") && email.indexOf(",") > 0) {
              return email.split(",").some(email => !emailRegex.test(email));
            }
