...
const serializeDateByTypeMap = new Map([
  ...
  ["week", input => serializeWeek(parseDateAsWeek(input))],
  [
    "time", input => {
...
class HTMLInputElementImpl extends HTMLElementImpl {
  ...
  set valueAsDate(v) {
    ...
    const isDateObj = v !== null && isDate(v);
  ...
  get _stepBase() {
    const stringToNumber = this._convertStringToNumber;
    const parseAttribute = attributeName => stringToNumber(this.getAttributeNS(null, attributeName));
