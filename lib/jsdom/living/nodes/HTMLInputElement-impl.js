...
class HTMLInputElementImpl extends HTMLElementImpl {
  ...
  get willValidate() {
    ...
    const typeHidden = this.type === "hidden";

    return !(typeHidden || disabled || readonly);
  }

  get validity() {
    if (!this._validity) {
      const value = () => convertStringToNumberByTypeMap.get(this.type)(this.value);
      const supportsRange = () => this.type === "date" || this.type === "datetime" ||
          this.type === "datetime-local" || this.type === "month" ||
          this.type === "time" || this.type === "week" || this.type === "number" ||
          this.type === "range";
      this._validity = ValidityState.createImpl({
        ...
        customError: () => this.willValidate && this._customValidityMessage &&
              this._customValidityMessage !== "",
        // When a control has incomplete input and the user agent does not think the
        //  user ought to be able to submit the form in its current state.
        badInput: () => false,
        // Is a Boolean indicating the element has a required attribute, but no value.
        valueMissing: () => {
          if (this.willValidate && this.hasAttribute("required")) {
            const isCheckedType = this.type === "checkbox" || this.type === "radio";
            return (isCheckedType && !this.checked) || (!isCheckedType && !this.value);
          }
          return false;
        },
        // Gecko never allows the value to exceed the maxLength and minLength values,
        // it would be simpler to do something like that here as well
        tooLong: () => false,
        tooShort: () => false,
        // Is a Boolean indicating the value is greater than the maximum specified by the max attribute.
        // The max attribute works with the following types:  number, range, date, datetime,
        // datetime-local, month, time and week.
        rangeOverflow: () => this.willValidate && supportsRange() && value() > this._maximum,
        // Is a Boolean indicating the value is less than the minimum specified by the min attribute.
        // The min attribute works with the following types:  number, range, date, datetime,
        // datetime-local, month, time and week.
        rangeUnderflow: () => this.willValidate && supportsRange() && value() < this._minimum,
        // Is a Boolean indicating the value does not match the specified pattern.
        patternMismatch: () => this.willValidate && this.hasAttribute("pattern") &&
            !new RegExp(this.getAttribute("pattern")).test(this.value),
        // Is a Boolean indicating the value does not fit the rules determined by the step
        // attribute(that is, it's not evenly divisible by the step value).
        stepMismatch: () => this.willValidate && this.type === "number" &&
            value() % parseFloat(this.getAttribute("step")) !== 0,
        // Is a Boolean indicating the value is not in the required syntax (when type is email or url).
        // Setting to always be false as there would otherwise be significant complexity added
        // to check for correctness of email and url values.
        typeMismatch: () => false
      });
    }
