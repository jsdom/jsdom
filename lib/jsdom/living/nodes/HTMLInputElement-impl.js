...
class HTMLInputElementImpl extends HTMLElementImpl {
  ...
  _preClickActivationSteps(event) {
    ...
    if (target.type === 'checkbox') {
      target.checked = !target.checked;
      // Not implementing input.indeterminate
      // TODO: cancelled activation steps
    }
      
  }
  _activationBehavior(event) {
    const target = event.target;
    console.log('--- _activationBehavior ---')

    if (target.type === 'checkbox') {
      // fire a simple event that bubbles named input at the element and then fire a simple event that bubbles named change at the element.

      // TODO: test input and then change is fired
