...
class HTMLInputElementImpl extends HTMLElementImpl {
  ...
  #legacyActivationBehaviorPreviousCheckedRadio = null;
  ...
  _legacyPreActivationBehavior() {
    if (this.type === "checkbox") {
      ...
      this.#legacyActivationBehaviorPreviousCheckedRadio = this.checked ?
        this :
        this.#getOtherRadioGroupElements(true)[0] || null;
      this.checked = true;
    }
  }

  _legacyCanceledActivationBehavior() {
    const previousCheckedRadio = this.#legacyActivationBehaviorPreviousCheckedRadio;
    this.#legacyActivationBehaviorPreviousCheckedRadio = null;
    if (this.type === "checkbox") {
      this.checked = !this.checked;
      this.indeterminate = this._legacyActivationBehaviorPreviousIndeterminateState;
    } else if (this.type === "radio") {
      if (previousCheckedRadio !== null &&
          (previousCheckedRadio === this || this.#getOtherRadioGroupElements().includes(previousCheckedRadio))) {
        previousCheckedRadio._checkedness = true;
      } else {
  ...
  _attributeChangeSteps(localName, oldValue, value, namespace) {
    ...
    if (namespace === null && !this.#dirtyValue && localName === "value") {
      this.#value = sanitizeValueByType(this, wrapper.defaultValue);
    }
    if (namespace === null && !this.#dirtyCheckedness && localName === "checked") {
      this._checkedness = wrapper.defaultChecked;
    }

    // Until form owners are tracked, a `form` attribute change approximates an owner reset here.
    if (namespace === null && (localName === "name" || localName === "form")) {
      if (this._checkedness) {
        this._removeOtherRadioCheckedness();
      }
    }

    if (namespace === null && localName === "type") {
      const prevType = getTypeFromAttribute(oldValue);
      ...
      if (prevType !== curType) {
        ...
        if (prevValueMode === "value" && this.#value !== "" &&
            (curValueMode === "default" || curValueMode === "default/on")) {
          this.setAttributeNS(null, "value", this.#value);
        } else if (prevValueMode !== "value" && curValueMode === "value") {
          this.#value = this.getAttributeNS(null, "value") || "";
        ...
        this.#value = sanitizeValueByType(this, this.#value);
  ...
  _insertionSteps() {
    ...
    if (this._checkedness && this.isConnected) {
      this._removeOtherRadioCheckedness();
    }
  }
  ...
  #getOtherRadioGroupElements(checkedOnly = false) {
  ...
  _removeOtherRadioCheckedness() {
    for (const radioGroupElement of this.#getOtherRadioGroupElements(true)) {
  ...
  _someInRadioGroup(name) {
    ...
    return this.#getOtherRadioGroupElements().some(radioGroupElement => radioGroupElement[name]);
  }
  ...
  // Setting checkedness, including internally during reset or activation, updates the radio group.
  set _checkedness(checkedness) {
    this.#checkedness = checkedness;
    if (checkedness) {
      this._removeOtherRadioCheckedness();
    }
  }
  ...
  set value(val) {
    switch (valueAttributeMode(this.type)) {
      ...
      case "value": {
        ...
        this.#value = sanitizeValueByType(this, val);
  ...
  get valueAsDate() {
    ...
    const convertedValue = this._convertStringToDate(this.#value);
  ...
  set valueAsDate(v) {
    ...
    this.#value = this._convertDateToString(v);
  }
  ...
  get valueAsNumber() {
    ...
    const parsedValue = this._convertStringToNumber(this.#value);
  ...
  set valueAsNumber(v) {
    ...
    if (isNaN(v)) {
      ...
      this.#value = this._convertNumberToString(v);
    }
  }
  ...
  _stepUpdate(n, isUp) {
    ...
    this.#value = this._convertNumberToString(value.toNumber());
  }
  ...
  _dispatchSelectEvent() {
    this._globalObject._document._queueATask(() => {
      fireAnEvent("select", this, undefined, { bubbles: true, cancelable: false });
    });
  }
    ...
    if (!this._validity) {
      ...
      const state = {
        ...
        valueMissing: () => {
          ...
          if (this._required && valueAttributeMode(this.type) === "value" && this._mutable && this.#value === "") {
            return true;
          }
        ...
        patternMismatch: () => {
          ...
          if (this.#value === "" || !this._hasAttributeAndApplies("pattern")) {
            return false;
          }
          let regExp;
          try {
        ...
        stepMismatch: () => {
          ...
          const number = this._convertStringToNumber(this.#value);
        ...
        typeMismatch: () => {
          switch (this.type) {
            ...
            case "url":
              if (this.#value !== "" && !isValidAbsoluteURL(this.#value)) {
                return true;
              }
              break;
            ...
            case "email":
              if (this.#value !== "" && !isValidEmailAddress(this._getValue(), this.hasAttributeNS(null, "multiple"))) {
                return true;
              }
              break;
          }
  ...
  // https://html.spec.whatwg.org/multipage/input.html#the-input-element:concept-node-clone-ext
  _cloningSteps(copy) {
    copy.#value = this.#value;
    copy._checkedness = this._checkedness;
    copy.#dirtyValue = this.#dirtyValue;
    copy.#dirtyCheckedness = this.#dirtyCheckedness;
    copy.indeterminate = this.indeterminate;
  }
}
