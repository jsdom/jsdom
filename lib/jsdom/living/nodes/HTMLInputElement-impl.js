...
class HTMLInputElementImpl extends HTMLElementImpl {
  ...
  get _stepBase() {
    const parseNumberAttribute =
      attributeName => parseFloatingPointNumber(this.getAttributeNS(null, attributeName));
    if (this.hasAttributeNS(null, "min")) {
      const min = parseNumberAttribute("min");
      if (!isNaN(min)) {
        return min;
      }
    }
    if (this.hasAttributeNS(null, "value")) {
      const value = parseNumberAttribute("value");
      if (!isNaN(value)) {
        return value;
      }
    }
    ...
    if (!this._validity) {
      this._validity = ValidityState.createImpl(this, {
        ...
        stepMismatch: () => {
          ...
          const normalizedNumber = this._stepBase - number;
          const scaledStep = this._step * this._stepScaleFactor;

          const isIntegralMultiple = new Decimal(normalizedNumber).modulo(scaledStep).isZero();
