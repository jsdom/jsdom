...
const { regexpMatches } = require("../helpers/strings");
const {
  isDisabled,
  isValidEmailAddress,
  isValidAbsoluteUrl,
...
const allowedTypesForAttribute = {
  readonly: new Set([
    "text", "search", "url", "tel", "email", "password", "date",
    "month", "week", "time", "number", "datetime-local",
    // The below are NOT correct per spec, but that's what the '-willValidate' web platform test expects
    // An issue needs to be filed against the tests
    "color", "file", "submit"
  ]),
  max: maxMinStepTypes,
  min: maxMinStepTypes,
...
class HTMLInputElementImpl extends HTMLElementImpl {
  ...
  _removeOtherRadioCheckedness() {
    this._otherRadioGroupElements.forEach(radioGroupElement => {
  ...
  get _readOnly() {
    if (allowedTypesForAttribute.readonly.has(this.type)) {
  ...
  get _step() {
    ...
    if (this.hasAttribute("step") && this.getAttribute("step").toLowerCase() !== "any") {
  ...
  get willValidate() {
    // https://html.spec.whatwg.org/#hidden-state-(type=hidden)
    // https://html.spec.whatwg.org/#reset-button-state-(type=reset)
    // https://html.spec.whatwg.org/#button-state-(type=button)
    const willNotValidateTypes = new Set(["hidden", "reset", "button"]);
    // https://html.spec.whatwg.org/multipage/input.html#attr-input-readonly
    const readOnly = this._readOnly;

    // https:// html.spec.whatwg.org/multipage/form-control-infrastructure.html#attr-fe-disabled
    if (willNotValidateTypes.has(this.type) || isDisabled(this) || readOnly) {
      return false;
    }
  ...
  get validity() {
    if (!this._validity) {
      this._validity = ValidityState.createImpl(this, {
        ...
        rangeOverflow: () => allowedTypesForAttribute.max.has(this.type) && this._maximum !== null &&
        ...
        rangeUnderflow: () => allowedTypesForAttribute.min.has(this.type) && this._minimum !== null &&
          this._parsedValue < this._minimum,

        // https://html.spec.whatwg.org/#suffering-from-a-pattern-mismatch
        patternMismatch: () => {
          if (!this.hasAttribute("pattern") || !allowedTypesForAttribute.pattern.has(this.type) || this.value === "") {
            return false;
          }
          let regexp;
          try {
          ...
          const multiple = allowedTypesForAttribute.multiple.has(this.type) && this.hasAttribute("multiple");
          return !regexpMatches({ regexp, testValue: this.value, split: multiple });
        },
        ...
        stepMismatch: () => {
          ...
          if (!allowedTypesForAttribute.step.has(this.type)) {
            return false;
          }
