...
function dispatchPhase(event, iterator) {
  ...
  while (target && !event._stopPropagation) {
    ...
    if (inlineListener && (event._eventPhase === event.AT_TARGET || event._eventPhase === event.BUBBLING_PHASE)) {
      const document = target._ownerDocument || target._document;
      // Will be falsy for windows that have closed
      if (document && (!target.nodeName || document.implementation._hasFeature("ProcessExternalResources", "script"))) {
