...
class XMLHttpRequestImpl extends XMLHttpRequestEventTargetImpl {
  constructor(window) {
    ...
    this.flag = {
      ...
      method: undefined,
      responseType: "",
      // We use a plain object instead of Headers because XHR preserves header name casing,
      // unlike the rest of the platform where Headers lowercases names.
      requestHeaders: {},
  ...
  send(body) {
    ...
    if (flag.synchronous) {
      ...
      // Convert responseHeaders back to Headers object
      if (resProp.responseHeaders) {
        const headers = new Headers();
        for (const [key, value] of Object.entries(resProp.responseHeaders)) {
          headers.set(key, value);
        }
        resProp.responseHeaders = headers;
      }
      ...
      // Create the client and set up event handlers BEFORE firing loadstart,
      // so that abort() called from the loadstart handler can properly abort the client.
      ...
      client.on("redirect", (response, requestHeaders, sourceURL, targetURL) => {
        const sourceUrlObj = new URL(sourceURL);
        const targetUrlObj = new URL(targetURL);
        const isCrossOriginRedirect = sourceUrlObj.origin !== targetUrlObj.origin;

        // Per https://fetch.spec.whatwg.org/#http-redirect-fetch:
        ...
        if (methodWillChange) {
          // Remove request-body-header names per https://fetch.spec.whatwg.org/#request-body-header-name
          delete requestHeaders["Content-Type"];
          delete requestHeaders["Content-Length"];
          delete requestHeaders["Content-Encoding"];
          delete requestHeaders["Content-Language"];
          delete requestHeaders["Content-Location"];
        }

        // Step 14: Set origin to "null" if the redirect crosses origins and the source is cross-origin
        // from the original request origin. This ensures origin becomes "null" when redirecting
        // away from a cross-origin server, but not when redirecting from the same-origin server.
        if (isCrossOriginRedirect && sourceUrlObj.origin !== flag.origin) {
          properties.origin = "null";
        }
        requestHeaders.Origin = properties.origin;

        // Step 15: If cross-origin redirect, remove CORS non-wildcard request-header names
        // per https://fetch.spec.whatwg.org/#cors-non-wildcard-request-header-name
        // Currently this is only "Authorization"
        // See also: https://github.com/whatwg/fetch/pull/1544
        if (isCrossOriginRedirect) {
          delete requestHeaders.Authorization;
        }

        // If the redirect source was cross-origin, validate CORS headers on the redirect response
  ...
  setRequestHeader(header, value) {
    ...
    const { flag } = this;
    const existingValue = xhrUtils.getRequestHeader(flag.requestHeaders, header);
    if (existingValue !== null) {
      xhrUtils.updateRequestHeader(flag.requestHeaders, header, existingValue + ", " + value);
    } else {
...
function getResponseHeader(xhr, lcHeader) {
  ...
  try {
    ...
    // Invalid header names should return null per XHR spec
    return null;
  }
}
