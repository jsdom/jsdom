...
class XMLHttpRequestImpl extends XMLHttpRequestEventTargetImpl {
  ...
  send(requestBody) {
    ...
    if (flag.synchronous) {
      ...
      client.on("redirect", (response, meta) => {
        const { referrer, url } = meta;

        const destUrlObj = new URL(referrer);
...
function receiveResponse(xhr, response) {
  ...
  const destUrlObj = new URL(url);
  const { origin, protocol } = destUrlObj;
  // const isHTTP = protocol !== "data:" && protocol !== "file:";
  ...
  const headerMap = {};
  const n = Number(responseHeaders.length);
  for (let i = 0; i < n; i += 2) {
    const k = responseHeaders[i];
    const kl = k.toLowerCase();
    const v = responseHeaders[i + 1];
  ...
  if (properties.origin !== origin && protocol !== "data:") {
    if (xhrUtils.validCORSHeaders(xhr, response, flag, properties, properties.origin)) {
      const acehStr = responseHeaders["access-control-expose-headers"];
  ...
  xhr.status = status;
  xhr.statusText = statusText || HTTP_STATUS_CODES[status] || "";
  ...
  // Content-Length).
  getResponseBody(response).on("data", chunk => {
