...
const { READY_STATES, headerListSeparatorRegexp, simpleHeaders, performFetch, decodeBodyStream, isNetworkError } =
  require("./xhr-utils");
...
class XMLHttpRequestImpl extends XMLHttpRequestEventTargetImpl {
  ...
  open(method, url, asynchronous, user, password) {
    ...
    let urlObj;
    try {
      urlObj = new URL(url, _ownerDocument.baseURLSerialized());
    } catch {
      throw DOMException.create(this._globalObject, [
        "The string did not match the expected pattern.",
        "SyntaxError"
      ...
      (async () => {
        try {
          const response = await performFetch(
            ...
            {
              processPreflightResponse: preflightResponse => {
                // CORS preflight validation - return error message string on failure, true on success
                ...
                if (forbiddenHeaders.length > 0) {
                  return "Headers " + forbiddenHeaders + " forbidden";
                }
                return true;
              },

              processRedirect: ({ response: redirectResponse, requestHeaders, sourceURL, targetURL }) => {
                // Return error message string on failure, true on success
                ...
                if (this._origin !== sourceUrlObj.origin &&
                  ...
                  const acaoStr = redirectResponse.headers.get("access-control-allow-origin");
                  const acao = acaoStr ? acaoStr.trim() : null;
                  if (acao !== "*" && acao !== this._origin) {
                    return "Cross origin " + this._origin + " forbidden";
                  }
                  if (this._withCredentials) {
                    const acacStr = redirectResponse.headers.get("access-control-allow-credentials");
                    const acac = acacStr ? acacStr.trim() : null;
                    if (acac !== "true") {
                      return "Credentials forbidden";
                    }
                  ...
                  if (targetUrlObj.username || targetUrlObj.password) {
                    return "Userinfo forbidden in cors redirect";
                  }
                }
                return true;
              }
          ...
          if (response.body) {
            ...
            const decodedStream = decodeBodyStream(response.body, headers);
