...
const { parseURL, serializeURL } = require("../helpers/url");
const { samePartition } = require("../file-api/blob-url-store");
...
class XMLHttpRequestImpl extends XMLHttpRequestEventTargetImpl {
  constructor(window) {
    ...
    this._dispatcher = window._settings.dispatcher;
  ...
  open(method, url, asynchronous, user, password) {
    const { _ownerDocument } = this;
    // TODO: Check whether the document is fully active, not just whether it has been destroyed.
    if (_ownerDocument._isDestroyed) {
      throw DOMException.create(this._globalObject, ["The object is in an invalid state.", "InvalidStateError"]);
    }
  ...
  send(body) {
    ...
    if (upload._eventListeners !== null && Object.keys(upload._eventListeners).length > 0) {
      ...
      (async () => {
        try {
          const response = await performFetch(
            ...
            {
              url: this._url,
              urlRecord: this._urlRecord,
              environment: this._ownerDocument,
              method: this._method,
          ...
          // An aborted request can finish after `open()` and `send()` have started a replacement request.
          if (this._controller === abortController) {
            clearTimeout(this._timeoutId);
            this._timeoutFn = null;
            this._timeoutStart = 0;
            this._controller = null;
  ...
  _serializeRequest() {
    const entry = this._urlRecord.blobURLEntry;
    return {
      method: this._method,
      url: this._url,
      // The sync worker cannot consult the main thread's store. Clone the retained blob without transferring its
      // backing buffer, which remains usable by the original Blob and other requests.
      blob: entry && samePartition(entry, this._ownerDocument) ?
        { _bytes: entry.object._bytes, type: entry.object.type } :
        null,
      body: this._body,
  ...
  _adoptSerializedRequest(config) {
    ...
    this._urlRecord = parseURL(config.url);
    if (this._urlRecord.scheme === "blob") {
      this._urlRecord.blobURLEntry = config.blob === null ?
        null :
        { object: config.blob, environment: this._ownerDocument };
    }
