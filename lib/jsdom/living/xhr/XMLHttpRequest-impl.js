...
const { URL, serializeURL } = require("whatwg-url");
...
class XMLHttpRequestImpl extends XMLHttpRequestEventTargetImpl {
  ...
  send(body = null) {
    ...
    if (!this._synchronousFlag) {
      ...
      const processResponse = response => {
        ...
        const processBodyChunk = bytes => {
          ...
          fireProgressEvent(this, "progress", this._receivedBytes.length, length);
        };
    ...
    const syncResponse = JSON.parse(res.stdout.toString());
...
function handleResponseEndOfBody(xhr) {
  ...
  const transmitted = xhr._receivedBytes.length;
...
function applySyncResponse(xhr, response) {
  ...
  if (response.response && response.response.url) {
    response.response.url = new URL(response.response.url);
  }
