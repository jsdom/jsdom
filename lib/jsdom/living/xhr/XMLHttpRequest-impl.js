...
const SYNC_XHR_DIAGNOSTIC_PREFIX = "[jsdom][sync-xhr]";
...
let syncWorker = null;
let nextSyncWorkerRequestId = 1;
const pendingSyncWorkerRequests = new Map();
const syncWorkerRecentEvents = [];

function emitSyncXhrDiagnostic(event, details = {}) {
  // eslint-disable-next-line no-console
  console.error(`${SYNC_XHR_DIAGNOSTIC_PREFIX} ${event} ${inspect(details, { depth: 4, breakLength: Infinity })}`);
}

function recordSyncWorkerEvent(event, details = {}) {
  syncWorkerRecentEvents.push({ timestamp: Date.now(), event, details });
  if (syncWorkerRecentEvents.length > 30) {
    syncWorkerRecentEvents.shift();
  }
}
...
function getSyncWorker() {
  if (!syncWorker) {
    ...
    syncWorker.unref();
    recordSyncWorkerEvent("worker-created", { threadId: syncWorker.threadId });

    const worker = syncWorker;

    worker.on("messageerror", error => {
      recordSyncWorkerEvent("worker-messageerror", { threadId: worker.threadId, error });
      emitSyncXhrDiagnostic("worker-messageerror", {
        threadId: worker.threadId,
        pendingRequests: pendingSyncWorkerRequests.size,
        error
      });
    });

    worker.on("error", error => {
      recordSyncWorkerEvent("worker-error", { threadId: worker.threadId, error });
      emitSyncXhrDiagnostic("worker-error", {
        threadId: worker.threadId,
        pendingRequests: pendingSyncWorkerRequests.size,
        error
      });
    ...
    worker.on("exit", code => {
      recordSyncWorkerEvent("worker-exit", { threadId: worker.threadId, code });
      if (pendingSyncWorkerRequests.size > 0) {
        emitSyncXhrDiagnostic("worker-exit-with-pending-requests", {
          threadId: worker.threadId,
          code,
          pendingRequests: Array.from(pendingSyncWorkerRequests.values()).map(request => ({
...
function sendSyncWorkerRequest(config) {
  const requestId = nextSyncWorkerRequestId++;
  ...
  pendingSyncWorkerRequests.set(requestId, requestInfo);
  ...
  if (waitResult === "timed-out") {
    emitSyncXhrDiagnostic("timeout", {
      ...
      recentEvents: syncWorkerRecentEvents
    });
  ...
  if (!msg) {
    emitSyncXhrDiagnostic("missing-response-message", {
      requestId,
      method: config.method,
      url: config.url,
      threadId: worker.threadId,
  ...
  const responseRequestId = msg.message?.requestId;
  if (responseRequestId !== undefined && responseRequestId !== requestId) {
    emitSyncXhrDiagnostic("request-id-mismatch", {
