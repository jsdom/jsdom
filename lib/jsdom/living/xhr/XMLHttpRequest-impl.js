...
class XMLHttpRequestImpl extends XMLHttpRequestEventTargetImpl {
  ...
  constructor(window) {
    ...
    this.#associatedDocument = _ownerDocument;
  ...
  send(body) {
    ...
    if (this._synchronous) {
      ...
      const unlink = linkAbortController(abortController, this.#associatedDocument._fetchSignal);
      ...
      if (this.readyState !== READY_STATES.OPENED || !this._send || signal.aborted) {
      ...
      (async () => {
        try {
          ...
          signal.removeEventListener("abort", onAbort);
          unlink();
          // Event handlers can call `open()` and `send()` again. An older request must not clear the new controller.
          if (this._controller === abortController) {
            this._controller = null;
            clearTimeout(this._timeoutId);
            this._timeoutFn = null;
