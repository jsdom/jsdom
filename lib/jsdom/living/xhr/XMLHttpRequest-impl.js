...
const { utf8fromString, utf8fromStringLoose } = require("@exodus/bytes/utf8.js");
  ...
  get response() {
    ...
    switch (this.responseType) {
      ...
      case "arraybuffer": {
        ...
        res = copyToArrayBufferInTargetRealmDestructively(responseBuffer.buffer, this._globalObject);
        break;
      }
      case "blob": {
        if (!responseBuffer) {
  ...
  send(body) {
    ...
    try {
      ...
      if (body !== null) {
        ...
        if (Document.isImpl(body)) {
          ...
          // We use utf8fromStringLoose to represent the spec's convert to USVString + UTF-8 encode operations.
          flag.body = utf8fromStringLoose(fragmentSerialization(body, { requireWellFormed: false }));
        } else {
    ...
    if (flag.synchronous) {
      ...
      if (resProp.responseBuffer && resProp.responseBuffer.data) {
        resProp.responseBuffer = new Uint8Array(resProp.responseBuffer.data);
      }
...
function receiveResponse(xhr, response, currentURL) {
  ...
  // pre-allocate buffer.
  properties.responseBuffer = new Uint8Array(properties.bufferStepSize);
  ...
  properties.client.on("data", chunk => {
    ...
    if (properties.totalReceivedChunkSize >= properties.bufferStepSize) {
      ...
      tmpBuf.set(properties.responseBuffer);
      properties.responseBuffer = tmpBuf;
    }
    properties.responseBuffer.set(chunk, properties.totalReceivedChunkSize - chunk.length);
    properties.responseCache = null;
    properties.responseTextCache = null;
    properties.responseXMLCache = null;
...
function extractBody(bodyInit) {
  ...
  if (Blob.isImpl(bodyInit)) {
    ...
    return {
      buffer: new Uint8Array(bodyInit).slice(0),
      contentType: null
    };
  } else if (ArrayBuffer.isView(bodyInit)) {
    return {
      buffer: new Uint8Array(bodyInit),
      contentType: null
    };
    ...
    return {
      buffer: concatTypedArrays(outputChunks),
  ...
  // Must be a string
  return {
    buffer: utf8fromString(bodyInit),
