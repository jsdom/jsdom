...
class XMLHttpRequestImpl extends XMLHttpRequestEventTargetImpl {
  ...
  set responseType(responseType) {
    ...
    if (this.readyState === READY_STATES.LOADING || this.readyState === READY_STATES.DONE) {
      throw DOMException.create(this._sdGlobalObject, ["The object is in an invalid state.", "InvalidStateError"]);
    }
  ...
  get response() {
    ...
    switch (this.responseType) {
      ...
      case "arraybuffer": {
        ...
        res = copyToArrayBufferInNewRealm(responseBuffer, this._sdGlobalObject);
        break;
      }
      case "blob": {
        if (!responseBuffer) {
        ...
        res = Blob.createImpl(this._sdGlobalObject, [
  ...
  get responseXML() {
    ...
    const res = Document.createImpl(this._sdGlobalObject, [], {
      options: {
        url: flag.uri,
  ...
  open(method, uri, asynchronous, user, password) {
    ...
    if (forbiddenRequestMethods.has(upperCaseMethod)) {
      throw DOMException.create(this._sdGlobalObject, ["The operation is insecure.", "SecurityError"]);
    }
  ...
  send(body) {
    ...
    if (flag.synchronous) {
      ...
      if (resProp.error) {
        ...
        throw DOMException.create(this._sdGlobalObject, [resProp.error, "NetworkError"]);
      } else {
