...
module.exports = function createXMLHttpRequest(window) {
  // TODO: Wrapping the XMLHttpRequest interface into a factory is not needed anymore with
  // the webidl2js constructor reform. This code can be cleaned up.
  class XMLHttpRequest extends window.XMLHttpRequestEventTarget {
    constructor() { // eslint-disable-line constructor-super
      const theThis = Object.create(new.target.prototype);
      theThis._globalObject = window;
      XMLHttpRequestEventTarget.setup(theThis, [], { globalObject: window });
      theThis.upload = XMLHttpRequestUpload.create(window);
    ...
    get response() {
      ...
      switch (this.responseType) {
        ...
        case "blob": {
          ...
          res = Blob.create(this._globalObject, [
    ...
    send(body) {
      ...
      if (flag.synchronous) {
        ...
        if (response.properties.error) {
          ...
          throw DOMException.create(this._globalObject, [response.properties.error, "NetworkError"]);
        } else {
          ...
          properties.timeoutFn = () => {
            ...
            if (!(this.readyState === XMLHttpRequest.UNSENT ||
              ...
              if (!properties.uploadComplete) {
                const uploadImpl = idlUtils.implForWrapper(this.upload);
                fireAnEvent("progress", uploadImpl, ProgressEvent);
                readyStateChange(this, XMLHttpRequest.DONE);
                fireAnEvent("timeout", uploadImpl, ProgressEvent);
                fireAnEvent("loadend", uploadImpl, ProgressEvent);
                stateChanged = true;
              }
  ...
  function setDispatchProgressEvents(xhr) {
    ...
    if (properties.uploadListener) {
      fireAnEvent("loadstart", uploadImpl, ProgressEvent, initProgress);
    }
    ...
    client.on("request", req => {
      req.on("response", () => {
        ...
        fireAnEvent("progress", uploadImpl, ProgressEvent, progress);
        fireAnEvent("load", uploadImpl, ProgressEvent, progress);
        fireAnEvent("loadend", uploadImpl, ProgressEvent, progress);
      });
    });
  }
