...
class XMLHttpRequestImpl extends XMLHttpRequestEventTargetImpl {
  constructor(window) {
    ...
    // These will only be used for async XHRs. xhr-sync-worker.js will override them from the serialized options.
    this._httpAgent = window._resourceLoader._httpAgent;
    this._httpsAgent = window._resourceLoader._httpsAgent;
    ...
    this.flag = {
      ...
      // These will only be used for sync XHRs.
      httpAgentOptions: xhrUtils.serializeAgentToOptions(this._httpAgent),
      httpsAgentOptions: xhrUtils.serializeAgentToOptions(this._httpsAgent)
    };
