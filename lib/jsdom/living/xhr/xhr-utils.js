...
// Convert a fetch Response headers to the old object format expected by XHR code
...
// Convert a fetch Response headers to raw headers array format [key, value, key, value, ...]
function headersToRawArray(headers) {
  const arr = [];
  for (const [key, value] of headers) {
    arr.push(key, value);
  }
...
async function decompressBody(response, reader) {
  ...
  if (contentEncoding === "gzip" || contentEncoding === "deflate") {
    ...
    const decompressor = contentEncoding === "gzip" ?
      zlib.createGunzip({ flush: zlib.constants.Z_SYNC_FLUSH, finishFlush: zlib.constants.Z_SYNC_FLUSH }) :
    ...
    return {
      async *[Symbol.asyncIterator]() {
        ...
        // Feed data from the response body into the decompressor
        (async () => {
          try {
            while (true) {
        ...
        // Yield decompressed chunks
        while (!done && !error) {
          if (chunks.length > 0) {
            yield { value: chunks.shift(), done: false };
          } else {
  ...
  // No compression, return the reader as-is wrapped in an async iterable
  return {
    async *[Symbol.asyncIterator]() {
      while (true) {
        const { value, done: readerDone } = await reader.read();
...
function createClient(xhr) {
  ...
  // HTTP/HTTPS requests - use fetch() with optional dispatcher from resourceLoader
  const client = new EventEmitter();
  ...
  async function doFetch(url, method, headers, bodyData) {
    ...
    if (flag.resourceLoader?._dispatcher) {
      fetchOptions.dispatcher = flag.resourceLoader._dispatcher;
    }
  ...
  (async () => {
    try {
      ...
      if (needsPreflight) {
        ...
        // Convert preflight response headers to old format for validation
        const preflightHeadersObj = headersToObject(preflightResponse.headers);
        const preflightResponseAdapted = {
          statusCode: preflightResponse.status,
          headers: preflightHeadersObj
        };

        if (!validCORSPreflightHeaders(xhr, preflightResponseAdapted, flag, properties)) {
          setResponseToNetworkError(xhr);
          return;
        }
      }
      ...
      while (redirectCount < MAX_REDIRECTS) {
        ...
        if (response.status >= 300 && response.status < 400) {
          ...
          if (location) {
            ...
            // Convert response for CORS validation
            ...
            client.emit("redirect", responseAdapted, currentHeaders, currentURL);
      ...
      // Convert final response headers to old format
      const responseHeaders = headersToObject(finalResponse.headers);
      const rawHeaders = headersToRawArray(finalResponse.headers);
      ...
      responseObj.rawHeaders = rawHeaders;
      ...
      if (finalResponse.body) {
        ...
        const decompressed = await decompressBody(finalResponse, reader);

        for await (const { value } of decompressed) {
          if (aborted) {
            return;
          }
          if (value) {
