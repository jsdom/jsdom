...
async function performFetch(config, signal, hooks = {}) {
  ...
  // For data: and file: URLs, use the dispatcher (which includes the jsdom interceptor)
  if (urlObj.protocol === "data:" || urlObj.protocol === "file:") {
    try {
      const response = await dispatchAndCollectForDataFile(dispatcher, url, signal, {
        cookieJar,
        globalObject
      });
