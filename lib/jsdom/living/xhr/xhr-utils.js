...
const wrapCookieJarForRequest = require("../helpers/request/wrap-cookie-jar-for-request");
...
function createClient(xhr) {
  ...
  if (flag.auth) {
    options.auth = {
      user: flag.auth.user || "",
      pass: flag.auth.pass || "",
      sendImmediately: false /** @warning , always false if defined in the options */
    };
  }
  ...
  function doRequest() {
    try {
      const client = new Request(options);
  ...
  if (crossOrigin && (!simpleMethods.has(ucMethod) || nonSimpleHeaders.length > 0 || properties.uploadListener)) {
    ...
    const preflightClient = new Request(preflightOptions);
