  ...
  function doRequest(sendAuthImmediately = false) {
    try {
      ...
      clientRequest.on("response", res => {
        if (hasCookies) {
          cookieHandlerFactory(
            cookie => flag.cookieJar.setCookieSync(cookie, clientRequest._currentUrl, { ignoreError: true }),
            client
          )(res);
        }
        const code = res.statusCode;
        if (!sendAuthImmediately && code === 401 && /^Basic /i.test(res.headers["www-authenticate"] || "")) {
          ...
          response.once("end", () => client.emit("end"));
          response.on("error", err => client.emit("error", err));
        }
      });
