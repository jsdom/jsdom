  ...
  async function doRequestWithClient(client, sendAuthImmediately) {
    try {
      ...
      if (cookieJar && (!crossOrigin || flag.withCredentials)) {
        response = await fetchCookie(fetch, cookieJar)(uri, fetchOptions);
      } else {
      ...
      if (!sendAuthImmediately &&
        ...
        if (response.redirected) {
          // then emit redirect with response and a request object as arguments
          client.emit(
            "redirect",
            response,
            { ...getResponseHeaders(response), origin: headers.Origin, referrer: headers.Referer }
          );
        }
      }
    } catch (e) {
      if (e.name === "AbortError") {
        process.nextTick(() => client.emit("abort"));
      } else {
