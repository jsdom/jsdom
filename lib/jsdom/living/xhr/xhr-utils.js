...
function createClient(xhr) {
  ...
  const agent = agentFactory(uri, flag.proxy, flag.strictSSL);
  const options = {
    method: flag.method,
  ...
  function doRequest() {
    try {
      const client = request.stream(uri, options);
  ...
  function getRequestProperty(lcHeaders, requestUri) {
    const { origin, referer } = lcHeaders;
    return { headers: { Origin: origin, Referer: referer }, uri: requestUri };
  }
  ...
  if (crossOrigin && (!simpleMethods.has(ucMethod) || nonSimpleHeaders.length > 0 || properties.uploadListener)) {
    ...
    const preflightClient = request.stream(uri, preflightOptions);
    ...
    preflightClient.on("response", resp => {
      ...
      realClient.on("response", res => {
        const { headers, url } = res.request.options;
        res.request = getRequestProperty(headers, url);
      ...
      realClient.on("redirect", (response, nextOptions) => {
        client.response = response;
        client.response.request = getRequestProperty(nextOptions.headers, nextOptions.url);
