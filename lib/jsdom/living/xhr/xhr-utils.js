...
function createClient(xhr) {
  ...
  // HTTP STUFF HERE //
  if (!(urlObj.host || (urlObj.hostname && urlObj.port))) {
    client.emit("error", new Error(`Invalid URI "${urlObj.toString()}"`));
    client.emit("abort");
    return client;
  }
  ...
  if (hasBody && flag.formData) {
    ...
    requestHeaders["Content-Length"] = form.getLengthSync();
  } else if (hasBody && getRequestHeader(flag.requestHeaders, "content-type") === null) {
    ...
    try {
      ...
      clientRequest.on("response", res => {
        ...
        if (!sendAuthImmediately && code === 401 && /^Basic /i.test(res.headers["www-authenticate"] || "")) {
          ...
          options.headers.Authorization = xhrAuth.sendAuth();
          res.destroy();
          doRequest(true);
        } else {
          let response = res;
      ...
      setImmediate(() => clientRequest.end());
    } catch (e) {
  ...
  if (crossOrigin && (!simpleMethods.has(ucMethod) || nonSimpleHeaders.length > 0 || properties.uploadListener)) {
    ...
    if (flag.pool === "object" && flag.pool !== null) {
      preflightOptions.agent = flag.pool;
    }
