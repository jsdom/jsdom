...
const { agentFactory, getRequest, cookieHandlerFactory } = require("../helpers/http-request");
...
function createClient(xhr) {
  ...
  // HTTP STUFF HERE //
  client.headers = Object.create(null);
  ...
  const xhrAuth = {
    ...
    pass: "",
    sendAuth() {
      const thisToString = `${this.user}:${this.pass}`;
      this.sentAuth = true;
      return `Basic ${Buffer.from(thisToString, "utf8").toString("base64")}`;
    }
  };
  ...
  const request = getRequest(uri, flag.strictSSL);
  let hasCookies = true;
  if (flag.cookieJar && (!crossOrigin || flag.withCredentials)) {
    requestHeaders.Cookie = flag.cookieJar.getCookieStringSync(uri);
  } else {
    hasCookies = false;
  }
  function beforeRedirect(reqOptions, response) {
    if (hasCookies) {
      cookieHandlerFactory(
        cookie => flag.cookieJar.setCookieSync(cookie, reqOptions.href, { ignoreError: true }),
        client
      )(response);
    }
    // We use this hook to adjust the request options upon redirecting
    // and to inspect the latest response headers
    // const { header } = response; - the only property on response lul
    Object.assign(client.headers, reqOptions.headers);

    const destUrlObj = new URL(reqOptions.headers.Referer);
    const urlObject = new URL(reqOptions.href);
    if (destUrlObj.origin !== urlObject.origin && destUrlObj.origin !== flag.origin) {
      properties.origin = "null";
    }
    // Alter request headers
    reqOptions.headers.Origin = properties.origin;

    if (flag.origin !== destUrlObj.origin && destUrlObj.protocol !== "data:") {
      if (!validCORSHeaders(xhr, response, flag, properties, flag.origin)) {
        return;
      }
  ...
  function doRequest(sendAuthImmediately = false) {
    try {
      // set authentication header if sendAuthImmediately is set
      const clientRequest = request(uri, options);
      if (hasBody && flag.formData && (!xhrAuth.hasAuth || (xhrAuth.hasAuth && xhrAuth.sentAuth))) {
        ...
        form.pipe(clientRequest._currentRequest);
      } else if (hasBody && !flag.formData) {
        clientRequest.write(requestBody);
      }
      clientRequest.on("response", res => {
        if (hasCookies) {
          cookieHandlerFactory(
            cookie => flag.cookieJar.setCookieSync(cookie, clientRequest._currentUrl.href, { ignoreError: true }),
            client
          )(res);
        }
        const code = res.statusCode;
        if (!sendAuthImmediately && code === 401 && /^Basic /i.test(res["WWW-Authenticate"] || "")) {
          requestHeaders.Authorization = xhrAuth.sendAuth();
          Object.assign(options.headers, requestHeaders);
          doRequest(true);
        } else {
          let response = res;
          res.responseUrl = clientRequest._currentUrl;
          if (clientRequest._options.method === "HEAD" || (code >= 100 && code < 200) || code === 204 || code === 304) {
            const zlibOptions = {
          ...
          response.on("data", chunk => client.emit("data", chunk));
          response.on("end", () => client.emit("end"));
          response.on("error", () => client.emit("error"));
        }
      });
      clientRequest.on("abort", () => client.emit("abort"));
      clientRequest.on("error", err => client.emit("error", err));
      client.abort = () => {
        clientRequest.abort();
      };
      client.headers = clientRequest._options.headers;
      client.emit("request", clientRequest._currentRequest);
      clientRequest.end();
    } catch (e) {
  ...
  if (crossOrigin && (!simpleMethods.has(ucMethod) || nonSimpleHeaders.length > 0 || properties.uploadListener)) {
    ...
    preflightRequestHeaders["Accept-Encoding"] = "gzip, deflate";
    ...
    const preflightClient = request(uri, preflightOptions);
