...
function createClient(xhr) {
  ...
  function beforeRedirect(reqOptions, response) {
    ...
    Object.assign(client.response, response);
    client.response.request = {};
    Object.assign(client.response.request, reqOptions);
  ...
  function doRequest(sendAuthImmediately = false) {
    try {
      ...
      const clientRequest = request(urlObj, options);
      if (hasBody && flag.formData) {
      ...
      clientRequest.on("response", res => {
        ...
        if (!sendAuthImmediately && code === 401 && /^Basic /i.test(res.headers["www-authenticate"] || "")) {
          ...
          response.once("end", endResponse);
          response.once("close", endResponse);
      ...
      process.nextTick(() => clientRequest.end());
    } catch (e) {
  ...
  if (crossOrigin && (!simpleMethods.has(ucMethod) || nonSimpleHeaders.length > 0 || properties.uploadListener)) {
    ...
    const preflightClient = request(urlObj, preflightOptions);
    ...
    process.nextTick(() => preflightClient.end());
  } else {
    doRequest();
  }
