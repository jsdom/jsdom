"use strict";
const { parseURL, serializeURL, serializeURLOrigin, serializePath } = require("../helpers/url");
...
async function performFetch(dispatcher, config, signal) {
  ...
  const isCrossOriginResponse = origin !== serializeURLOrigin(destUrlRecord) &&
    destUrlRecord.scheme !== "data" && destUrlRecord.scheme !== "blob";
...
async function dispatchMainRequest(dispatcher, opts, signal) {
  ...
  try {
    ...
    return {
      ...
      body: response.body,
      url: serializeURL(response.context.finalURL, true)
    };
  } catch {
    return makeNetworkError();
  }
