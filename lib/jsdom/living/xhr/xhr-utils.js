...
async function performFetch(config, signal, hooks = {}) {
  ...
  if (urlObj.protocol === "data:" || urlObj.protocol === "file:") {
    try {
      const resourceResponse = await resourceLoader.fetch(url);

      if (aborted) {
        return makeNetworkError();
      }

      const responseHeaders = HeaderList.fromJSON(resourceResponse.headers);
      return {
        status: resourceResponse.status,
        statusText: resourceResponse.statusText,
        headers: responseHeaders,
        body: resourceResponse.body ? Readable.fromWeb(resourceResponse.body) : null,
        url: resourceResponse.url || url
      };
    } catch (err) {
      if (aborted) {
        return makeNetworkError();
  ...
  try {
    ...
    while (redirectCount < MAX_REDIRECTS) {
      ...
      // Final response - attach URL and return
