...
const { abortRequest, agentFactory, getRequest, cookieHandlerFactory } = require("../helpers/http-request");
  ...
  function doRequest(sendAuthImmediately = false) {
    try {
      ...
      clientRequest.on("response", res => {
        ...
        if (!sendAuthImmediately && res.statusCode === 401 && /^Basic /i.test(res["WWW-Authenticate"] || "")) {
      ...
      clientRequest.on("data", chunk => client.emit("data", chunk));
      clientRequest.on("end", () => client.emit("end"));
