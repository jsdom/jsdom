...
function createClient(xhr) {
  ...
  function doFetch(url, method, headers, bodyData) {
    return new Promise((resolve, reject) => {
      ...
      const bodyState = { complete: false, waiting: null, error: null };
      ...
      dispatcher.dispatch({
        ...
        onHeaders(statusCode, rawHeaders, resume, statusText) {
          ...
          // Header values must be decoded as Latin-1 (ISO-8859-1) per HTTP spec
          for (let i = 0; i < rawHeaders.length; i += 2) {
            const name = rawHeaders[i].toString("latin1").toLowerCase();
        ...
        onError(err) {
          ...
          bodyState.complete = true;
          bodyReject(err);
          // Notify any waiting iterator so it can throw the error
          if (bodyState.waiting) {
            bodyState.waiting();
          }
          reject(err);
