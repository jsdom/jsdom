...
function getAgent(uri, proxy, rejectUnauthorized) {
  ...
  if (proxy) {
    ...
    const proxyOpts = { ...url.parse(proxy), ...agentOpts };
...
function createClient(xhr) {
   ...
   */
  function doRequestWithClient(client, sendAuthImmediately) {
    try {
      const headers = { ...requestHeaders };
      if (flag.auth && sendAuthImmediately) {
        headers["Authorization"] = "Basic " + Buffer.from(username + ":" + password).toString("base64");
      }
      let targetFetch;
      if (flag.cookieJar && (!crossOrigin || flag.withCredentials)) {
        targetFetch = fetchCookie(myFetch, cookieJar);
      } else {
        targetFetch = fetch;
      }

      const hasBody = flag.body !== undefined &&
                      flag.body !== null &&
                      flag.body !== "" &&
                      !(ucMethod === "HEAD" || ucMethod === "GET");

      // const body = hasBody && !flag.formData ? flag.body : undefined;
      let body = null;
      if (hasBody) {
        // if flag.formData is truthy, `flag.body` is an iterable collection containing objects with keys name, value, options.
        // if it's false, `flag.body` should be some kind of non-empty buffer, blob or string.
        body = flag.formData ? flag.body : flag.body;
      }
      ...
      const response = fetch(uri, {
        ...
        follow: 21,
        redirect: "follow",
        agent: getAgent(uri, flag.proxy, flag.strictSSL),
        body,
      });
      ...
      && /^Basic /i.test(response.headers["WWW-Authenticate"] || "")) {
        return closeResponse(response).then(() => doRequestWithClient(client, true));
      }
  ...
  if (crossOrigin && (!simpleMethods.has(ucMethod) || nonSimpleHeaders.length > 0 || properties.uploadListener)) {
    ...
    fetch(uri, {
      ...
      if (maybeStop !== stop) {
        return doRequestWithClient(client, false);
      }
    }).catch((error) => {
      client.emit("error", error);
    });
