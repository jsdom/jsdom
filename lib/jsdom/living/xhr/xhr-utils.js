...
function createClient(xhr) {
  ...
  // For async XHRs, we have access to the original XHR object, so we can assign the agents directly. For sync XHRs, the
  // XHR instance was freshly-created in `xhr-sync-worker.js` and has not inherited any customizations from the
