...
/**
 * Create a decompression stream based on Content-Encoding header.
 * Returns the original stream if no decompression is needed.
 */
function decodeBodyStream(body, headers) {
  ...
  // pipeline() handles errors from the source by calling the callback,
  // but doesn't propagate them to the destination stream. We need to
  // destroy the destination with the error so that for-await-of throws.
  const decodedStream = pipeline(body, ...decoders, err => {
    if (err) {
      decodedStream.destroy(err);
    }
  });
...
/**
 * Handle data: URLs directly without a dispatcher.
 * Returns a response object with status, statusText, headers, body, url.
 */
async function handleDataURLDirectly(url, signal) {
  ...
  return {
    ...
    statusText: "OK",
    headers,
    body: body.length > 0 ? Readable.from([body]) : null,
    url
  };
}

/**
 * Handle file: URLs directly without a dispatcher.
 * Returns a response object with status, statusText, headers, body, url.
 */
async function handleFileURLDirectly(url, signal) {
...
/**
 * Dispatch a request for data: or file: URLs and collect the response.
 * This goes through the jsdom interceptor.
 */
function dispatchAndCollectForDataFile(dispatcher, url, signal, { cookieJar, globalObject }) {
...
async function performFetch(config, signal, hooks = {}) {
  ...
  // For data: and file: URLs, use the dispatcher if available, otherwise handle directly
  if (urlObj.protocol === "data:" || urlObj.protocol === "file:") {
    try {
      let response;
      if (dispatcher) {
        // Use the dispatcher (which includes the jsdom interceptor)
        response = await dispatchAndCollectForDataFile(dispatcher, url, signal, {
          cookieJar,
          globalObject
        });
      } else if (urlObj.protocol === "data:") {
        // No dispatcher available, handle data: URL directly
        response = await handleDataURLDirectly(url, signal);
      } else {
        // No dispatcher available, handle file: URL directly
        response = await handleFileURLDirectly(url, signal);
      }
  ...
  // Use global dispatcher as fallback if no dispatcher was configured
  const effectiveDispatcher = dispatcher ?? getGlobalDispatcher();
    ...
    return new Promise((resolve, reject) => {
      ...
      effectiveDispatcher.dispatch({
        ...
        onError(err) {
          signal?.removeEventListener("abort", onAbort);
          if (resolved) {
            // Already resolved (aborted), just destroy the stream silently
...
exports.decodeBodyStream = decodeBodyStream;
