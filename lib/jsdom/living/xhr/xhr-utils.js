...
function createClient(xhr) {
  ...
  const options = {
    // For async XHRs, we have access to the original XHR object, so we can assign the agents directly. For sync XHRs,
    // the XHR instance was freshly-created in `xhr-sync-worker.js` and has not inherited any customizations from the
    // `ResourceLoader`. So, create new agents that attempt to copy over as much configuration as possible.
    agents: {
      http: flag.synchronous ? createAgentFromSerializedOptions(flag.httpAgentOptions) : xhr._httpAgent,
      https: flag.synchronous ? createAgentFromSerializedOptions(flag.httpsAgentOptions) : xhr._httpsAgent
    },
  ...
  if (crossOrigin && (!simpleMethods.has(ucMethod) || nonSimpleHeaders.length > 0 || properties.uploadListener)) {
    ...
    const preflightClient = new Request(
      uri,
      { agents: flag.agents, followRedirects: false },
      { method: "OPTIONS", headers: preflightRequestHeaders }
    );
...
function serializeAgentToOptions(agent) {
  return {
    ...
    maxTotalSockets: agent.maxTotalSockets,
    options: agent.options
  };
}

function createAgentFromSerializedOptions(options) {
  // Filter out null or undefined options since Node.js does not follow the principle that those
  // will cause defaults to be used.
  ...
  return new (options.protocol === "http:" ? http.Agent : https.Agent)(filteredOptions);
}
...
exports.serializeAgentToOptions = serializeAgentToOptions;
exports.createAgentFromSerializedOptions = createAgentFromSerializedOptions;
