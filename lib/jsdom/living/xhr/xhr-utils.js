...
const getRequest = require("../helpers/http-request");
...
function createClient(xhr) {
  ...
  if (flag.auth) {
    ...
    options.xhrAuth = `${username}:${password}`;
    /**
    options.authentication = {
      user: user || "",
      password: pass || "",
      // sendImmediately: false // not really required
      @todo Test out http.clientRequest's auth option, then revert to request's patterns
    }; */
  }
  ...
  function doRequest() {
    try {
      const requestClient = request(urlObj, options);
      if (hasBody) {
        ...
        if (flag.formData) {
          ...
          requestClient.attachListenerToForm(requestBody);
          requestClient.setHeader("Content-Type", `multipart/form-data; boundary=${requestBody.getBoundary()}`);
          requestBody.pipe(requestClient._currentRequest);
        } else {
        ...
        requestClient.setHeader("Content-Length", len);
      }
      requestClient.setHeader("Accept-Encoding", "gzip, deflate");
  ...
  if (crossOrigin && (!simpleMethods.has(ucMethod) || nonSimpleHeaders.length > 0 || properties.uploadListener)) {
    ...
    preflightClient.on("response", resp => {
      ...
      realClient.on("request", req => {
        client.headers = realClient._options.headers;
