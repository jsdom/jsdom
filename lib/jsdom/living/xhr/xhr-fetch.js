...
function applyResponseHeaderFiltering(request, response) {
  ...
  if (crossOrigin && response.url?.protocol !== "data:") {
...
async function runPreflight(request, unsafeHeaders, dispatcher, abortController) {
  ...
  if (request.credentialsMode === "include" && allowCredentials !== "true") {
    return makeNetworkErrorResponse();
  }
...
async function dispatchRequest(url, method, headerList, body, dispatcher, abortController) {
...
async function fetchWithResourceLoader(request, resourceLoader, abortController) {
  ...
  return {
    ...
    url: new URL(response.url || request.url.href)
  };
}

async function performFetch(request, abortController) {
  ...
  while (redirectCount < 20) {
    ...
    if (!headerListContains(requestHeaders, "Authorization") &&
        (currentURL.username || currentURL.password)) {
      const authString = `${currentURL.username}:${currentURL.password}`;
      headerListAppend(requestHeaders, "Authorization", "Basic " + toBase64(utf8Encode(authString)));
    }

    const response = await dispatchRequest(currentURL, currentMethod, requestHeaders, currentBody, dispatcher,
      abortController);
    ...
    if (response.status >= 300 && response.status < 400) {
      ...
      if (!isSameOrigin(previousURL.origin, currentURL.origin) && previousURL.origin !== origin) {
        origin = "null";
      }
      headerListDelete(requestHeaders, "Origin");
...
function fetching(request, {
  ...
  (async () => {
    try {
      ...
      let response = await performFetch(request, abortController);
