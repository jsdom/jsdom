// Import the Level 3 core object
...
/**
 * Living Standard implementation of compareDocumentPosition.
 * Contained in a closure to isolate all the helper functions.
 */
(function () {
  ...
  /**
   * Traverse the node tree starting at current. Return DOCUMENT_POSITION_FOLLOWING if otherNode follows node. Return DOCUMENT_POSITION_PRECEDING if otherNode precedes node
   * @param  {Node} current
   * @param  {Node} node
   * @param  {Node} otherNode
   * @return {Number}
  ...
  /**
   * Returns a bitmask Number composed of DOCUMENT_POSITION constants based upon the rules defined in http://dom.spec.whatwg.org/#dom-node-comparedocumentposition
   * @param  {Node} other
   * @return {Number}
   */
...
// Export the living dom implementation
exports.dom = {
  living : {
    core: core
  }
