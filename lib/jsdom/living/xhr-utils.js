...
exports.createClient = function createClient(xhr) {
  ...
  const customLoader = xhr._ownerDocument._customResourceLoader;
  const requester = customLoader ? function(options) {
    return customLoader(Object.assign({}, options, {
      element: xhr,
  ...
  function doRequest() {
    try {
      const client = requester(options);
  ...
  if (crossOrigin && (!simpleMethods.has(ucMethod) || nonSimpleHeaders.length > 0)) {
    ...
    const preflightClient = requester(preflightOptions);
