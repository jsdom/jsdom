...
const simpleHeaders = new Set(["user-agent", "accept", "accept-language", "content-language", "content-type"]);
    ...
    preflightClient.on("response", resp => {
      if (resp.statusCode >= 200 && resp.statusCode <= 299) {
        ...
        realClient.on("response", res => {
          if (resp.headers["access-control-expose-headers"]) {
            res.headers["access-control-expose-headers"] += "," + resp.headers["access-control-expose-headers"];
          }
          if (resp.headers["access-control-allow-headers"]) {
            res.headers["access-control-allow-headers"] += "," + resp.headers["access-control-allow-headers"];
          }
          if (!res.headers["access-control-allow-credentials"]) {
            res.headers["access-control-allow-credentials"] = resp.headers["access-control-allow-credentials"];
          }
          if (!res.headers["access-control-allow-origin"]) {
            res.headers["access-control-allow-origin"] = resp.headers["access-control-allow-origin"];
          }
