...
class CSSImpl {
  ...
  static #supportsConditionText(conditionText) {
    try {
      // 1. Fast path for single declarations (e.g., "color: red")
      if (/^[^():]+\s*:/.test(conditionText)) {
        const declAst = csstree.parse(conditionText, { context: "declaration" });
        if (declAst) {
          return this.#validateDeclarationAST(declAst.property, declAst.value);
        }
      }
      ...
      if (
        ...
        return this.#evaluateList(supportsRule.prelude.children);
      }
    } catch {
      // Fall through
    }
  ...
  static #supportsDeclaration(property, value) {
  ...
  static #validateDeclarationAST(property, ast) {
    const { isCustom, isKnown, lowerProp } = this.#getPropertyInfo(property);
    if (!isKnown) {
      return false;
    }
  ...
  static #evaluateList(list) {
    ...
    while (current !== null) {
      ...
      const { children, declaration, name: ident, property, type, value } = node;
  ...
  static #getPropertyInfo(property) {
