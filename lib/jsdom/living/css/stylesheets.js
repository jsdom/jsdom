...
exports.addStyleSheet = (sheet, elementImpl) => {
  ...
  const styleSheets = StyleSheetList.convert(_globalObject, _ownerDocument.styleSheets)._list;
  const styleSheet = CSSStyleSheet.convert(_globalObject, sheet);
  const { _alternate, _lastName, _preferredName, disabled, title } = styleSheet;
  ...
  if (title) {
    if (!_alternate && _preferredName === '') {
      styleSheet._preferredName = title;
    }
    if (
      _lastName
      && asciiLowercase(title) !== asciiLowercase(_lastName)
      && asciiLowercase(title) !== (_preferredName && asciiLowercase(_preferredName))
    ) {
