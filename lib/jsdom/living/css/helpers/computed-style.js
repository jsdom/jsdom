...
// Generational cache
const MAX_SELECTOR_CACHE_SIZE = 4096;
let currentSelectorCache = new Map();
let oldSelectorCache = new Map();
...
function getParsedSelectorKeys(selectorText) {
  let keysList = currentSelectorCache.get(selectorText);
  if (keysList !== undefined) {
    return keysList;
  }

  keysList = oldSelectorCache.get(selectorText);
  if (keysList !== undefined) {
    currentSelectorCache.set(selectorText, keysList);
    return keysList;
  }
  ...
  if (currentSelectorCache.size >= MAX_SELECTOR_CACHE_SIZE) {
    oldSelectorCache = currentSelectorCache;
    currentSelectorCache = new Map();
  }
