...
function getParsedSelectorKeys(selectorText) {
  let keysList = currentCache.get(selectorText);
  if (keysList !== undefined) {
    return keysList;
  }

  keysList = oldCache.get(selectorText);
  if (keysList !== undefined) {
    setCachedSelectorKeys(selectorText, keysList);
    return keysList;
  }
...
function setCachedSelectorKeys(key, value) {
