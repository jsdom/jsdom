...
function preparePartiallyResolvedValue(property, value, elementImpl, { inherit, initial, longhands }) {
  if (value === "") {
    ...
    if (inherit && elementImpl.parentElement) {
      value = getInheritedPropertyValue(property, elementImpl, { inherit, initial });
    } else {
      value = initial;
    }
  }
  ...
  if (isGlobalKeyword(value)) {
    value = resolveGlobalKeywords(property, value, elementImpl, { inherit, initial });
  }
