    ...
    if (CSSImportRule.isImpl(ruleImpl)) {
      if (ruleImpl.styleSheet !== null && evaluateMediaList(ruleImpl.media._list)) {
        for (const innerRule of ruleImpl.styleSheet.cssRules._list) {
          if (ruleMightMatchElement(innerRule, elementImpl)) {
...
function ruleMightMatchElement(ruleImpl, elementImpl) {
  ...
  for (const keys of ruleImpl._selectorSubjects) {
    if (keys.id) {
      if (keys.id === elementImpl.getAttributeNS(null, "id")) {
        return true;
      }
    } else if (keys.className) {
      if (elementImpl.classList._tokenSet.contains(keys.className)) {
        return true;
      }
    } else if (keys.tag) {
      if (keys.tag === elementImpl._localName) {
        return true;
      }
    } else {
      return true;
