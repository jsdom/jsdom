...
const ruleSelectorCache = new WeakMap();
...
function extractAndIndexRule(ruleImpl, index) {
  let cached = ruleSelectorCache.get(ruleImpl);
  if (!cached || cached.selectorText !== ruleImpl.selectorText) {
    const keysList = [];
    try {
      const ast = csstree.parse(ruleImpl.selectorText, { context: "selectorList" });
    ...
    cached = { selectorText: ruleImpl.selectorText, keysList };
    ruleSelectorCache.set(ruleImpl, cached);
  }

  for (const keys of cached.keysList) {
    if (keys.id) {
