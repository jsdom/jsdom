...
function replaceEmptyValueAndKeywords(
  ...
  value,
  elementImpl,
  { inherit, initial, isColor, isFontSize, longhands }
) {
  if (value === "") {
    if (longhands) {
      return "";
    ...
    value = getInheritedPropertyValue(property, elementImpl, { inherit, initial, isColor, isFontSize });
  }

  if (isGlobalKeyword(value)) {
    value = replaceGlobalKeywords(property, value, elementImpl, { inherit, initial, isColor, isFontSize });
  }

  return value;
}

function getInheritedPropertyValue(property, elementImpl, { inherit, initial, isColor, isFontSize }) {
  const styleCache = elementImpl._ownerDocument._styleCache;
...
function replaceGlobalKeywords(property, value, elementImpl, { inherit, initial, isColor, isFontSize }) {
  ...
  while (element) {
    switch (value) {
      ...
      case "inherit": {
        ...
        value = getInheritedPropertyValue(property, element, { inherit, initial, isColor, isFontSize });
        break;
      }
      case "unset": {
    ...
    if (element.parentElement) {
      if (!value) {
        ...
        return replaceGlobalKeywords(property, value, element, { inherit, initial, isColor, isFontSize });
      } else {
        return value;
      }
    } else {
