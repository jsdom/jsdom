  ...
  if (inNestingContext) {
    ...
    for (const child of block.children) {
      if (child.type === "Raw") {
        const nestedRules = parseRawAsNestedRules(child.value, globalObject, parentSheet, ruleImpl);
        for (const nestedRule of nestedRules) {
          // Outside nesting context, bare declarations are invalid and should be dropped.
          if (!CSSNestedDeclarations.isImpl(nestedRule)) {
            ruleImpl.cssRules._list.push(nestedRule);
          }
        }
      } else {
    ...
    try {
      ...
      for (const child of parsed.children) {
        if (child.type === "Identifier") {
          prefix = child.name;
        } else if (child.type === "Url" || child.type === "String") {
