...
// Shared helper: parse CSS text and append translated rules to an existing sheet impl.
function parseRules(cssText, globalObject, sheetImpl, onError) {
  let ast;
  try {
...
function parseStyleSheet(cssText, globalObject, { href, title, ownerNode, ownerRule, mediaText, onError } = {}) {
  ...
  parseRules(cssText, globalObject, sheetImpl, onError);
...
function translateStyleRule(node, globalObject, parentSheet, parentRule) {
  ...
  const parentIsScope = parentRule && "start" in parentRule;
    ...
    for (const child of block.children) {
      if (child.type === "Declaration") {
        ...
        if (pendingDeclarations.length > 0) {
          flushNestedDeclarations(pendingDeclarations, globalObject, parentSheet, ruleImpl, ruleImpl);
          pendingDeclarations = [];
        }
...
function translateContainerRule(node, globalObject, parentSheet, parentRule) {
  ...
  if (node.prelude) {
    const preludeText = generateFromNode(node.prelude);
    const parsed = csstree.parse(preludeText, { context: "atrulePrelude", atrule: "container" });
    const firstChild = parsed.children.head.data;
  ...
  if (node.block && node.block.children) {
    for (const child of node.block.children) {
      if (child.type === "Declaration") {
        ...
        if (propName) {
          ruleImpl[propName] = generateFromNode(child.value).trim();
        }
      }
    }
  }
...
function parseRawAsDeclarations(rawText, globalObject, parentSheet, parentRule) {
  try {
    ...
    const declarations = ast.children.filter(c => c.type === "Declaration");
