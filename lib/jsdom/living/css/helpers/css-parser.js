...
function translateImportRule(node, globalObject, parentSheet, parentRule) {
  ...
  try {
    ...
    csstree.walk(parsed, walked => {
      switch (walked.type) {
        ...
        case "String":
          ...
          break;
        case "Function":
          if (walked.name === "layer") {
            layerFnNode = walked;
            if (walked.children) {
              layerName = csstree.generate({ type: "Value", children: walked.children });
            } else {
              layerName = "";
            }
          }
          break;
        case "MediaQueryList": {
          const mqText = csstree.generate(walked).trim();
          if (mqText) {
            mediaText = mqText;
          }
          break;
    ...
    // Check for bare "layer" keyword (without parens) among direct children only,
    // to avoid matching "layer" identifiers nested inside supports() or media queries.
    if (layerName === null) {
