  ...
  if (inNestingContext) {
    ...
    for (const child of block.children) {
      if (child.type === "Declaration") {
        ...
        const nestedRules = parseRawAsNestedRules(child.value, globalObject, parentSheet, ruleImpl, true);
    ...
    for (const child of block.children) {
      if (child.type === "Raw") {
        const nestedRules = parseRawAsNestedRules(child.value, globalObject, parentSheet, ruleImpl, false);
        for (const nestedRule of nestedRules) {
          ruleImpl.cssRules._list.push(nestedRule);
        }
      } else {
  ...
  try {
    ...
    for (const child of parsed.children) {
      ...
      if (child.type === "Function" && child.name === "supports") {
        // Extract supports text from raw prelude using positions to preserve spacing
        // (csstree's generate() strips spaces from declaration values).
        supportsText = preludeText.slice(
          child.loc.start.offset + "supports(".length,
