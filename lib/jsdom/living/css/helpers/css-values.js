...
// Instance of the GenarationalCache. Stores up to 4096 items.
const genCache = new GenerationalCache(4096);
...
function parsePropertyValue(prop, val, opt = {}) {
  ...
  if (GLOBAL_KEYS.has(lowerCasedValue)) {
    ...
    genCache.set(cacheKey, globalResult);
    return globalResult;
  }
  const { ast, valid } = validatePropertyValue(prop, val);
  if (!valid) {
    genCache.set(cacheKey, false);
    return undefined;
  }
