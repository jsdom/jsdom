...
function matchPropertyAST(prop, ast) {
  const { error, matched } = cssTree.lexer.matchProperty(prop, ast);
  return error === null && matched !== null;
}

// css-tree matches any calc() as <length> or <number> without inspecting units.
// A percentage inside a math function is only legal when a percentage token is
// accepted at that same syntactic position (e.g. <length-percentage>,
// <opacity-value>, or <number> | <percentage>), not at a bare <length> slot
// such as a box-shadow offset.
function percentageTokenMatchesAt(prop, ast, functionNode) {
  const original = { ...functionNode };
  functionNode.type = AST_TYPES.PERCENTAGE;
  functionNode.value = "0";
  delete functionNode.name;
  delete functionNode.children;
  try {
    return matchPropertyAST(prop, ast);
  } finally {
    for (const key of Object.keys(functionNode)) {
      delete functionNode[key];
    }
    Object.assign(functionNode, original);
  }
}

function mathFunctionHasDisallowedPercentage(prop, ast) {
  let disallowed = false;
  cssTree.walk(ast, node => {
    if (disallowed || node.type !== AST_TYPES.FUNCTION || !calcNameRegEx.test(node.name)) {
      return;
    }
    if (cssTree.find(node, child => child.type === AST_TYPES.PERCENTAGE) !== null &&
        !percentageTokenMatchesAt(prop, ast, node)) {
      disallowed = true;
    }
  });
  return disallowed;
...
function serializeLength(val, opt = {}) {
  const res = parseNumericValue(
    val,
    opt,
    (type, value) => type === AST_TYPES.DIMENSION || (type === AST_TYPES.NUMBER && Number(value) === 0)
  );
  if (!res) {
    return undefined;
  }
