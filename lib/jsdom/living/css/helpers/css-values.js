...
function propertyDisallowsPercentages(prop) {
  const { percentages } = getPropertyDefinition(prop);
  const normalized = asciiLowercase(String(percentages ?? "").trim());
  return normalized === "n/a" || normalized === "na";
}

function mathFunctionContainsPercentage(val) {
  let ast;
  try {
    ast = cssTree.parse(val, { context: "value" });
  } catch {
    return false;
  }
  let found = false;
  cssTree.walk(ast, node => {
    if (found || node.type !== AST_TYPES.FUNCTION || !calcNameRegEx.test(node.name)) {
      return;
    }
...
function parsePropertyValue(prop, val, opt = {}) {
  ...
  if (val === "" || hasVarFunc(val)) {
    ...
    val = calculatedValue;
    // css-tree matches calc() against <length> without checking units. A mixed
    // px/% math function is <length-percentage>, which properties whose spec
    // percentages field is N/A (e.g. box-shadow) must still reject.
    if (propertyDisallowsPercentages(prop) && mathFunctionContainsPercentage(val)) {
      return undefined;
    }
  }
