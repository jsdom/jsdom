...
const { GenerationalCache } = require("./generational-cache");
...
// Generational cache
const generationalCache = new GenerationalCache(MAX_CACHE_SIZE);
...
function isValidPropertyValue(prop, val) {
  ...
  const cachedValue = generationalCache.get(cacheKey);
  ...
  generationalCache.set(cacheKey, result);
  return result;
}
...
function resolveCalc(val, opt = { format: "specifiedValue" }) {
  ...
  generationalCache.set(cacheKey, resolvedValue);
...
function parsePropertyValue(prop, val, opt = {}) {
  ...
  generationalCache.set(cacheKey, parsedValue);
