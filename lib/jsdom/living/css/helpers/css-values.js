...
function astContainsPercentage(node) {
  let found = false;
  cssTree.walk(node, inner => {
    if (inner.type === AST_TYPES.PERCENTAGE) {
      found = true;
    }
  });
  return found;
...
function percentageTokenMatchesAt(prop, ast, fnNode) {
  const { type, name, children, value } = fnNode;
  fnNode.type = AST_TYPES.PERCENTAGE;
  fnNode.value = "1";
  delete fnNode.name;
  delete fnNode.children;
  try {
    return matchPropertyAST(prop, ast);
  } finally {
    fnNode.type = type;
    fnNode.name = name;
    fnNode.children = children;
    if (value === undefined) {
      delete fnNode.value;
    } else {
      fnNode.value = value;
...
function mathFunctionHasDisallowedPercentage(prop, ast) {
  ...
  cssTree.walk(ast, node => {
    ...
    if (astContainsPercentage(node) && !percentageTokenMatchesAt(prop, ast, node)) {
      disallowed = true;
    }
  });
  return disallowed;
...
function parsePropertyValue(prop, val, opt = {}) {
  ...
  if (GLOBAL_KEYS.has(lowerCasedValue)) {
    ...
    try {
      ...
      const { error, matched } = cssTree.lexer.matchProperty(prop, ast);
      if (error || !matched || mathFunctionHasDisallowedPercentage(prop, ast)) {
        parsedValue = false;
      } else {
