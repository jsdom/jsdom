...
// Constants
const MAX_PARSE_CACHE_SIZE = 4096;
...
// Generational cache
let currentParseCache = new Map();
let oldParseCache = new Map();
...
function parseShorthand(shorthandParser, property, value) {
  const cacheKey = `parseShorthand\0${property}\0${value}`;
  let cached = currentParseCache.get(cacheKey);
  if (cached !== undefined) {
    return safeClone(cached);
  }

  cached = oldParseCache.get(cacheKey);
  if (cached !== undefined) {
    currentParseCache.set(cacheKey, cached);
    return safeClone(cached);
  }

  const parsed = shorthandParser(value);
  currentParseCache.set(cacheKey, parsed);
  if (currentParseCache.size >= MAX_PARSE_CACHE_SIZE) {
    oldParseCache = currentParseCache;
    currentParseCache = new Map();
  }
