  ...
  #getComputedValue(property, value) {
    ...
    if (longhands) {
      ...
      if (property === "border") {
        ...
        // If all four sides are exactly equal, the border shorthand is valid
        if (top && top === right && right === bottom && bottom === left) {
          return top;
        }
        return "";
      } else if (/^(?:border-(?:color|style|width)|margin|padding)$/.test(property)) {
      ...
      // TODO: resolve other shorthands.

      return value;
    }

    return this.#resolveLonghandPropertyValue(property, value, { caseSensitive, functionTypes, isColor });
  }
