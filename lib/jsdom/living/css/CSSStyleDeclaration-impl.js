...
class CSSStyleDeclarationImpl {
  ...
  getPropertyValue(property) {
    ...
    if (this.#computed) {
      ...
      const resolvedValue = this.#getComputedValue(property, value, metadata);
   ...
   */
  #getComputedValue(property, value, metadata) {
    if (!propertyDefinitions.has(property) && !property.startsWith("--")) {
      // Invalid or unsupported property.
      return "";
    }

    const { inherited, initial = "", longhands } = this.#getPropertyDefinition(property);

    if (!value || cssValues.isGlobalKeyword(value)) {
      value = computedStyle.replaceEmptyValueAndKeyword(
        property,
        value,
        this.#ownerNode,
    ...
    if (metadata.caseSensitive === false) {
    ...
    return this.#resolveLonghandPropertyValue(property, value, metadata);
  }
  ...
  /**
   * Returns the definition object for a given property.
   *
   * @private
   * @param {string} property - The property name.
   * @param {string} value - The specified value.
   ...
   * @returns {string} The computed value.
   */
  #resolveLonghandPropertyValue(property, value, { caseSensitive, functionTypes = {} }) {
    // TODO: Add dimensionTypes, imageType etc.
    const { color: colorType, paint: paintType } = functionTypes;
  ...
  #prepareComputedValueOpts() {
    ...
    const newColorScheme = computedStyle.replaceEmptyValueAndKeyword(
      "color-scheme",
      colorScheme,
      this.#ownerNode,
    ...
    if (
      ...
      let currentColor = computedStyle.replaceEmptyValueAndKeyword(
        "color",
