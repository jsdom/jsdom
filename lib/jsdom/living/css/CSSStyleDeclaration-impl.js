...
class CSSStyleDeclarationImpl {
  ...
  #resolveLonghand(property, value, { caseSensitive, dimensionTypes, isColor }) {
    ...
    if (isColor) {
      ...
      if (lengthType) {
        ...
        const resolvedValue = fontSizes.resolveLengthInPixels(value, opt, property === "font-size");
  ...
  #prepareComputedValueOpts() {
    ...
    if (this.#ownerNode._ownerDocument.documentElement.firstElementChild) {
      ...
      rem = fontSizes.resolveFontSizeInPixels(rootFontSize);
    } else {
      rem = fontSizes.resolveFontSizeInPixels("medium");
    }
    if (this.#ownerNode === this.#ownerNode._ownerDocument.documentElement) {
      ...
      if (typeof emInPixels === "number" && !Number.isNaN(emInPixels)) {
        ...
        em = fontSizes.resolveFontSizeInPixels("medium");
      }
    }
    const dimension = {
      em,
