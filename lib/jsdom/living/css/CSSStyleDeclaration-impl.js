...
class CSSStyleDeclarationImpl {
  ...
  #resolveLonghand(property, value, { caseSensitive, dimensionTypes, isColor }) {
    ...
    let options = this.#prepareComputedValueOpts();
    // Retrieve the em size from the parent when the property is font-size.
    if (isFontSize && this.#ownerNode.parentElement) {
      const styleCache = this.#ownerNode._ownerDocument._styleCache;
      const parentStyle = styleCache.get(this.#ownerNode.parentElement);
      const dimension = {
        ...options.dimension,
        em: parseFloat(parentStyle.getPropertyValue("font-size"))
      };
      options = {
        ...options,
        dimension
    ...
    if (isColor) {
      ...
      if (lengthType) {
        const opt = this.#ownerNode === this.#ownerNode._ownerDocument.documentElement ? {} : options.dimension;
        const resolvedValue = fontSizes.resolveLengthInPixels(this.#ownerNode, value, opt, isFontSize);
  ...
  #prepareComputedValueOpts() {
    ...
    if (this.#ownerNode === this.#ownerNode._ownerDocument.documentElement) {
      em = rem;
    } else {
      const emInPixels = fontSizes.resolveEmInPixels(this.#ownerNode, rem);
      if (typeof emInPixels === "number" && !Number.isNaN(emInPixels)) {
        em = Number(emInPixels.toPrecision(6));
      } else {
        em = fontSizes.resolveFontSizeInPixels(this.#ownerNode, "medium");
      }
    }
    const dimension = {
      em,
