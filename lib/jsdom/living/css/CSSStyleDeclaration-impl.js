...
class CSSStyleDeclarationImpl {
  ...
  _getComputedPropertyValue(property) {
    ...
    // Custom properties are never shorthands. Resolving an unset or inherited custom property reads the computed value
    // of every ancestor, so without caching each ancestor would resolve all the ancestors above it again.
    if (property.startsWith("--")) {
      this.#cachedPropertyValues.set(property, { computedValue, value });
    } else if (propertyDefinitions.has(property)) {
  ...
  _setPropertyValue(property, value, priority) {
    ...
    propertyDescriptors[property].set.call(this, value, priority);
  }
