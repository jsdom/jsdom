...
class CSSStyleDeclarationImpl {
  ...
  getPropertyValue(property) {
    ...
    if (this._computed) {
      if (this.#cachedPropertyValues.has(property)) {
        return this.#cachedPropertyValues.get(property);
      }
      ...
      if (propertyDefinitions.has(property)) {
        ...
        if (!longhands) {
          this.#cachedPropertyValues.set(property, resolvedValue);
        }
      }
  ...
  /**
   * Returns the computed value for a given property.
   *
   * @private
   * @param {string} property - The property name.
  ...
  #getComputedValue(property, value) {
    ...
    if (property === "color" && /currentcolor/i.test(value)) {
      value = computedStyle.getInheritedPropertyValue(
        property,
        this.#ownerNode,
        { inherit: inherited === "yes", initial, isColor }
      );
    }
    ...
    if (longhands) {
      if (isColor && systemColors.has(value)) {
        return value;
      }
      return this.#resolveShorthandPropertyValue(property, value);
    }

    return this.#resolveLonghandPropertyValue(property, value, { caseSensitive, isColor });
  }

  /**
   * Returns the metadata for a given property.
   *
   * @private
   * @param {string} property - The property name.
   * @returns {object} The metadata for resolving the computed value.
   */
  ...
  /**
   * Resolves shorthand property value.
   *
   * @private
   * @param {string} property - The property name.
   * @param {string} value - The specified value.
   * @returns {string} The computed value.
   */
  #resolveShorthandPropertyValue(property, value) {
    ...
    switch (property) {
      ...
      case "margin":
      case "padding": {
        return this.#resolvePositionShorthandPropertyValue(property);
      }
      default: {
        if (/^border-(?:top|right|bottom|left)$/.test(property)) {
          const width = this.getPropertyValue(`${property}-width`);
          const style = this.getPropertyValue(`${property}-style`);
          const color = this.getPropertyValue(`${property}-color`);
          if (width && style && color) {
            return `${width} ${style} ${color}`;
          }
          return "";
        } else if (/^border-(?:color|style|width)$/.test(property)) {
          return this.#resolvePositionShorthandPropertyValue(property);
        }
        return value;
      }
  ...
  /**
   * Resolves position shorthand property value.
   *
   * @private
   * @param {string} property - The property name.
   * @param {string} value - The specified value.
   * @returns {string} The computed value.
   */
  #resolvePositionShorthandPropertyValue(property) {
  ...
  /**
   * Resolves longhand property value.
   *
   * @private
   * @param {string} property - The property name.
   * @param {string} value - The specified value.
   * @param {object} metadata - The metadata for resolving the computed value.
   * @param {boolean} metadata.caseSensitive - The value is case sensitive.
   * @param {boolean} metadata.isColor - The value is a color.
   * @returns {string} The computed value.
   */
  #resolveLonghandPropertyValue(property, value, { caseSensitive, isColor }) {
    const computedValueOpts = this.#prepareComputedValueOpts();
    const parsedValue = cssValues.parsePropertyValue(property, value, {
      caseSensitive,
      ...computedValueOpts
    });
    ...
    if (isColor) {
      const resolvedValue = cssValues.serializeColor(parsedValue, computedValueOpts);
      if (resolvedValue) {
        return resolvedValue;
      }
    }
  ...
  /**
   * Prepare options for computed value. It is used when resolving relative values or specified values.
   *
   * @private
   * @returns {object}
   */
  #prepareComputedValueOpts() {
    if (!this.#computedValueOpts) {
      this.#computedValueOpts = {
        format: "computedValue"
      };
    }

    const currentColorScheme = this.#computedValueOpts.colorScheme;
    const colorScheme = this.#values.get("color-scheme") ?? "";
    const colorSchemeDef = cssValues.getPropertyDefinition("color-scheme");
    const colorDef = cssValues.getPropertyDefinition("color");
    if (
      currentColorScheme &&
      currentColorScheme === colorScheme &&
      this.#computedValueOpts.currentColor
    ) {
      return this.#computedValueOpts;
    }

    const newColorScheme = computedStyle.replaceEmptyValueAndKeywords(
      "color-scheme",
      colorScheme,
      this.#ownerNode,
      { inherit: colorSchemeDef.inherited === "yes", initial: colorSchemeDef.initial }
    );
    this.#computedValueOpts.colorScheme = newColorScheme;
    this.#cachedPropertyValues.set("color-scheme", newColorScheme);

    if (
      !this.#computedValueOpts.currentColor ||
      (currentColorScheme && currentColorScheme !== newColorScheme)
    ) {
      let currentColor = computedStyle.replaceEmptyValueAndKeywords(
        "color",
        this.#values.get("color") ?? "",
        this.#ownerNode,
        { inherit: colorDef.inherited === "yes", initial: colorDef.initial }
      );
      currentColor = asciiLowercase(currentColor);
      if (systemColors.has(currentColor)) {
        currentColor = cssValues.resolveSystemColorValue(currentColor, newColorScheme);
      }
      this.#computedValueOpts.currentColor = currentColor;
    }
