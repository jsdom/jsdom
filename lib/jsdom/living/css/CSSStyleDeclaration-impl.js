...
class CSSStyleDeclarationImpl {
  ...
  _getComputedValue(property, value, metadata) {
    ...
    if (value === "") {
      ...
      if (inherited === "yes" && this.#ownerNode.parentElement) {
        value = computedStyle.getInheritedPropertyValue(property, this.#ownerNode, {
          inherit: inherited === "yes",
          initial
        });
      } else {
        value = initial;
    ...
    if (isGlobalKeyword(value)) {
      value = computedStyle.resolveGlobalKeywords(property, value, this.#ownerNode, {
        inherit: inherited === "yes",
        initial
      });
    }
    ...
    // TODO: Prepare computed style options.
    // const computedStyleOpts = this.#prepareComputedStyleOpts();
    if (longhands) {
      // TODO: resolve shorthands.
    } else if (value) {
      // TODO: resolve computed value.
    } else {
      // TODO: resolve computed initial value.
    }
    return value;
  }
