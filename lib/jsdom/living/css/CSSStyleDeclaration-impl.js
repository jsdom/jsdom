...
class CSSStyleDeclarationImpl {
  ...
  #resolveLonghand(property, value, { caseSensitive, dimensionTypes, isColor }) {
    ...
    if (property === "font-size" && this.#ownerNode && this.#ownerNode.parentElement && options.dimension) {
      const parentStyle = computedStyle.getComputedStyleDeclaration(this.#ownerNode.parentElement);
      const parentFontSize = parseFloat(parentStyle.getPropertyValue("font-size"));
      if (!Number.isNaN(parentFontSize)) {
        options = {
          ...options,
          dimension: {
            ...options.dimension,
    ...
    if (isColor) {
      ...
      if (lengthType) {
        ...
        const resolvedValue = fontSizes.resolveLengthInPixels(this.#ownerNode, value, opt, property === "font-size");
