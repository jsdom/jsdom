...
// Generational cache
const MAX_CSSTEXT_CACHE_SIZE = 4096;
let currentCssTextCache = new Map();
let oldCssTextCache = new Map();
...
class CSSStyleDeclarationImpl {
  ...
  set cssText(text) {
    ...
    let cachedItems = getCachedCssText(text);
    if (!cachedItems) {
      cachedItems = [];
      if (text) {
        ...
        if (valueObj?.children) {
          ...
          for (const item of parsedProperties.values()) {
            cachedItems.push(item);
          }
        }
      }
      setCachedCssText(text, cachedItems);
    }

    for (const { property, priority, value } of cachedItems) {
  ...
  _setProperty(property, value, priority) {
    ...
    const oldPriority = this._priorities.get(property) || "";
    const newPriority = priority === "important" ? "important" : "";
    if (newPriority) {
      this._priorities.set(property, newPriority);
    } else {
      this._priorities.delete(property);
    }
    this.#values.set(property, value);

    const isValueChanged = oldValue !== value || oldPriority !== newPriority;
    if (isValueChanged) {
      this.#cachedCssText = null;
    }

    if (this.#ownerNode && !this.#updating && isValueChanged) {
      this.#updateStyleAttribute();
    }
  }
...
function getCachedCssText(text) {
  let value = currentCssTextCache.get(text);
  if (value !== undefined) {
    return value;
  }
  value = oldCssTextCache.get(text);
  if (value !== undefined) {
    currentCssTextCache.set(text, value);
    return value;
  }
  return undefined;
}

function setCachedCssText(text, value) {
  currentCssTextCache.set(text, value);
  if (currentCssTextCache.size >= MAX_CSSTEXT_CACHE_SIZE) {
    oldCssTextCache = currentCssTextCache;
    currentCssTextCache = new Map();
  }
}
