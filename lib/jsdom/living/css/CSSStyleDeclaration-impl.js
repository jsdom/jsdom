...
class CSSStyleDeclarationImpl {
   ...
   */
  _getComputedValue(property, value, metadata) {
    if (!propertyDefinitions.has(property) && !property.startsWith("--")) {
      // Invalid or unsupported property.
      return "";
    } else if (property === "color-scheme" && this.#computedValueOpts) {
      // Color scheme has already been resolved.
      return this.#computedValueOpts.colorScheme;
    } else if (property === "color" && (!value || /^[a-z]+$/i.test(value)) && this.#computedValueOpts) {
      // Reuse color value from computed value options.
      value = this.#computedValueOpts.currentColor;
    }

    const { inherited, initial = "", longhands } = propertyDefinitions.get(property) ?? {};

    if (!value || cssValues.isGlobalKeyword(value)) {
      value = computedStyle.preparePartiallyResolvedValue(
        property,
        value,
        this.#ownerNode,
        { inherit: inherited === "yes", initial, longhands }
      );
    }
    ...
    if (!metadata) {
      ...
      metadata = propertyMetadata.get(property);
    }
    if (!metadata.caseSensitive || metadata.colorType) {
      value = asciiLowercase(value);
    }

    if (longhands) {
      // TODO: resolve shorthands.
    } else if (value) {
      return this._resolveLonghandPropertyValue(property, value, metadata);
    }
    return value;
  }
   ...
   * @returns {string} The computed value.
   */
  _resolveLonghandPropertyValue(property, value, { caseSensitive, dimensionTypes = {}, functionTypes = {} }) {
    const { AST_TYPES } = cssValues;
    const {
      angle: angleType,
    ...
    const computedValueOpts = this._prepareComputedValueOpts();
    const parsedValue = cssValues.parsePropertyValue(property, value, {
      caseSensitive,
      ...computedValueOpts
    });
    ...
    switch (type) {
      ...
      case AST_TYPES.DIMENSION: {
        if (dimensionType && lengthType) {
          const val = cssValues.serializeLength(parsedValue, lengthType);
          if (val) {
            return val;
          }
          return cssValues.serializeDimension(parsedValue, dimensionType);
        } else if (lengthType) {
          return cssValues.serializeLength(parsedValue, lengthType);
        }
        return cssValues.serializeDimension(parsedValue, dimensionType);
      }
      case AST_TYPES.NUMBER: {
        if (numberType) {
          return cssValues.serializeNumber(parsedValue, numberType);
        } else if (angleType) {
          return cssValues.serializeAngle(parsedValue, angleType);
        } else if (lengthType) {
          return cssValues.serializeLength(parsedValue, lengthType);
        } else if (percentageType) {
          return cssValues.serializePercentage(parsedValue, percentageType);
        }
        break;
      }
      case AST_TYPES.PERCENTAGE: {
        ...
        if (numericType) {
          return cssValues.resolveNumericValue(parsedValue, numericType);
        }
        break;
      }
      case AST_TYPES.IDENTIFIER: {
        if (colorType || paintType) {
          return cssValues.serializeColor(parsedValue, computedValueOpts);
        }
        break;
      }
      case AST_TYPES.HASH: {
      ...
      case AST_TYPES.URL: {
        return cssValues.serializeURL(parsedValue);
      }
      case AST_TYPES.FUNCTION: {
        if (colorType || paintType) {
          return cssValues.serializeColor(parsedValue, computedValueOpts);
        } else if (imageType) {
          return cssValues.serializeGradient(parsedValue, computedValueOpts);
        }
        break;
      }
      default:
  ...
  /**
   * Prepare options for computed value.
   *
   * @private
   * @returns {object}
   */
  _prepareComputedValueOpts() {
    if (!this.#computedValueOpts) {
      // TODO: Add customProperty, dimension etc.
      const format = "computedValue";

      const colorScheme = computedStyle.preparePartiallyResolvedValue(
        "color-scheme",
        this.#values.get("color-scheme") ?? "",
        this.#ownerNode,
        { inherit: true, initial: propertyDefinitions.get("color-scheme").initial }
      );

      let currentColor = computedStyle.preparePartiallyResolvedValue(
        "color",
        this.#values.get("color") ?? "",
        this.#ownerNode,
        { inherit: true, initial: propertyDefinitions.get("color").initial }
      );
      currentColor = asciiLowercase(currentColor);
      if (systemColors.has(currentColor)) {
        const parsedValue = cssValues.parsePropertyValue("color", currentColor, {
          caseSensitive: false
        });
        currentColor = cssValues.serializeColor(parsedValue, { format, colorScheme });
      }
