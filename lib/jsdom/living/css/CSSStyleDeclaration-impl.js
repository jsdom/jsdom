...
class CSSStyleDeclarationImpl {
    ...
    switch (property) {
      ...
      default: {
        if (property.startsWith("border")) {
          return this.#resolveBorderShorthand(property);
        }
        return value;
      }
    }
  ...
  #resolveBorderShorthand(property) {
    switch (property) {
      case "border": {
        const values = [];
        const positions = ["top", "right", "bottom", "left"];
        for (const position of positions) {
          const value = this.getPropertyValue(`border-${position}`);
          if (!value) {
            return "";
          }
          values.push(value);
      ...
      case "border-left": {
        const values = [];
        const lines = ["width", "style", "color"];
        for (const line of lines) {
          const value = this.getPropertyValue(`${property}-${line}`);
          if (!value) {
            return "";
          }
          values.push(value);
