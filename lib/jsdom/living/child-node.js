...
module.exports = function (core) {
  ...
  function getViableNextSibling(el, container) {
    let previousSibling = el.previousSibling;

    for(; previousSibling !== null; perviousSibling = previousSibling.previousSibling) {
      let i = 0;
      for(; container.length > i; i++) {
    ...
    if(previousSibling === null) {
      ...
      for(; nextSibling !== null; nextSibling = nextSibling.nextSibling) {
        let i = 0;
        for(; container.length > i; i++) {
          const node = container[i];
          if(node === this) continue;
      ...
      viableNextSibling = nextSibling;
    } else {
      viableNextSibling = previousSibling;
    }
