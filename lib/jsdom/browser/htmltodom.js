...
module.exports = class HTMLToDOM {
  ...
  _parseWithParse5(html, isFragment, contextNode, options = {}) {
    const adapter = new JSDOMParse5Adapter(contextNode._ownerDocument || contextNode, this.core);
  ...
  _parseWithSax(html, isFragment, contextNode) {
    ...
    parser.ontext = text => {
      setChildForSax(this.core, openStack[openStack.length - 1], {
        type: "text",
        data: text
      });
    };
    ...
    parser.onopentag = arg => {
      const attrs = Object.keys(arg.attributes).map(key => {
        ...
        return { name: rawAttribute.name, value: rawAttribute.value, prefix, localName, namespace: rawAttribute.uri };
      });
      const tag = {
        type: "tag",
        name: arg.local,
      ...
      if (arg.local === "script" && arg.uri === "http://www.w3.org/1999/xhtml") {
        ...
        const elem = setChildForSax(this.core, openStack[openStack.length - 1], tag);
        openStack.push(elem);
      }
    };
    parser.onclosetag = () => {
      ...
      if (elem.constructor.name === "Object") { // we have an empty script tag
        setChildForSax(this.core, openStack[openStack.length - 1], elem);
      }
    };
...
function setChildForSax(core, parentImpl, node) {
  ...
  if (node.children) {
    for (let c = 0; c < node.children.length; c++) {
      setChildForSax(core, newNode, node.children[c]);
    }
  }
