...
const HTMLScriptElementImpl = require("../living/nodes/HTMLScriptElement-impl").implementation;
const HTMLStyleElementImpl = require("../living/nodes/HTMLStyleElement-impl").implementation;
...
module.exports = class HTMLToDOM {
  ...
  appendToElement(html, element) {
    ...
    const documentImpl = element._ownerDocument;
    const queue = documentImpl._queue;
    if (queue) {
      const unnest = queue.nest();
      const elem = this._doParse(html, true, element);
      unnest();
      return elem;
    }
  ...
  appendToDocument(html, documentImpl) {
    ...
    if (queue) {
      const unnest = queue.nest();
      const doc = this._doParse(html, false, documentImpl, documentImpl._parseOptions);
      unnest();
      return doc;
    }
...
function createNewNode(currentDocument, core, parentImpl, node) {
  let newNode;
  switch (node.type) {
    case "tag":
    case "script":
...
function setNewNodeAttributes(newNode, node) {
  if (node.attribs) {
...
function appendNewNodeToParent(newNode, parentImpl, node) {
  const isTemplateContents = node.type === "root";
...
function attachNewNode(newNode, parentImpl, node) {
  newNode[locationInfo] = node.__location;
  setNewNodeAttributes(newNode, node);
  appendNewNodeToParent(newNode, parentImpl, node);
}

function setChild(core, parentImpl, node) {
  const documentImpl = parentImpl && parentImpl._ownerDocument || parentImpl;
  const newNode = createNewNode(documentImpl, core, parentImpl, node);
  if (!newNode) {
    return null;
  }

  const shouldParseContentBefore =
    newNode instanceof HTMLScriptElementImpl ||
    newNode instanceof HTMLStyleElementImpl;

  if (shouldParseContentBefore && node.children) {
    for (let c = 0; c < node.children.length; c++) {
      setChild(core, newNode, node.children[c]);
    }
  }

  if (documentImpl._queue) {
    documentImpl._queue.push(() => {
      attachNewNode(newNode, parentImpl, node);
    })();
  } else {
    attachNewNode(newNode, parentImpl, node);
  }
