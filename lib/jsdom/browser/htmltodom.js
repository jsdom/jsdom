...
function HtmlToDom(parser) {
  if(parser && "DomHandler" in parser) {
    //htmlparser2
    parser = parser.Parser;

    this.appendHtmlToElement = function(html, element){
      var currentElement = element, tagStack = [];
      ...
      var instance = new parser({
        onopentagname: function(name){
          ...
          try {
            newElement = parent._createElementNoTagNameValidation(name);
          } catch (err) {
            parent.raise('error', 'invalid markup', {
              exception: err,
              node : name
            });
        ...
        onclosetag: function(name){
          var last = currentElement;
          currentElement = tagStack.pop();
          try {
            currentElement.appendChild(last);
          } catch(err) {
            var parent = currentElement._ownerDocument || currentElement;
            parent.raise('error', err.message, {
              exception: err,
              node : last
            });
        ...
        ontext: function(text){
          ...
          if(currentElement.lastChild && currentElement.lastChild.nodeType === 3){
            ...
            var node = parent.createTextNode(text);
            currentElement.appendChild(node);
          }
        },
        oncomment: function(data){
          var parent = currentElement._ownerDocument || currentElement;
          var commentNode = parent.createComment(data);
