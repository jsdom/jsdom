...
function setChild(core, parent, node) {
  const parentImpl = idlUtils.implForWrapper(parent);
  const currentDocument = parentImpl && parentImpl._ownerDocument || parent;
  ...
  switch (node.type) {
    ...
    case "script":
    case "style":
      newNode = idlUtils.implForWrapper(currentDocument)._createElementWithCorrectElementInterface(node.name, node.namespace);
  ...
  if (node.attribs) {
    for (let localName in node.attribs) {
      ...
      attributes.setAttributeValue(idlUtils.implForWrapper(newNode), localName, value, prefix, namespace);
    }
  }
