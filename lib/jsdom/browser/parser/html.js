...
class JSDOMParse5Adapter {
  ...
  createElement(localName, namespace, attrs) {
    ...
    if (localName === "style" && namespace === HTML_NS) {
      element._isOnStackOfOpenElements = true;
    }
    if (localName === "style" && namespace === HTML_NS && !this._fragment) {
      element._parserDocument = this._documentImpl;
    }
    this.adoptAttributes(element, attrs);
    if (localName === "link" && namespace === HTML_NS && !this._fragment) {
