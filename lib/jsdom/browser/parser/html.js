...
class JSDOMParse5Adapter {
  constructor(documentImpl, windowObject) {
  ...
  createDocumentFragment() {
    return DocumentFragment.createImpl(this._windowObject, [], { ownerDocument: this._currentElement._ownerDocument });
  }
  ...
  createCommentNode(data) {
    const ownerDocument = this._ownerDocument();
    return Comment.createImpl(this._windowObject, [], { data, ownerDocument });
  }
  ...
  setDocumentType(document, name, publicId, systemId) {
    const ownerDocument = this._ownerDocument();
    const documentType = DocumentType.createImpl(this._windowObject, [], { name, publicId, systemId, ownerDocument });
  ...
  insertText(parentNode, text) {
    ...
    if (lastChild && lastChild.nodeType === nodeTypes.TEXT_NODE) {
      ...
      const ownerDocument = this._ownerDocument();
      const textNode = Text.createImpl(this._windowObject, [], { data: text, ownerDocument });
  ...
  adoptAttributes(element, attrs) {
    for (const attr of attrs) {
      ...
      attributes.setAttributeValue(element, attr.name, attr.value, prefix, attr.namespace, this._windowObject);
    }
  }
}
...
function parseFragment(markup, contextElement, windowObject) {
  ...
  const config = Object.assign({}, ownerDocument._parseOptions, {
    treeAdapter: new JSDOMParse5Adapter(ownerDocument, windowObject)
  });
