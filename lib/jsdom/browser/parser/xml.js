...
function parseFragment(markup, contextElement) {
  const { _sdGlobalObject, _ownerDocument } = contextElement;

  const fragment = DocumentFragment.createImpl(_sdGlobalObject, [], { ownerDocument: _ownerDocument });

  // Only parseFragment needs resolvePrefix per the saxes documentation:
  // https://github.com/lddubeau/saxes#parsing-xml-fragments
  const parser = createParser(fragment, _sdGlobalObject, {
    fragment: true,
...
function parseIntoDocument(markup, ownerDocument) {
  const { _sdGlobalObject } = ownerDocument;

  const parser = createParser(ownerDocument, _sdGlobalObject, {
