...
function createParser(rootNode, globalObject, saxesOptions) {
  ...
  parser.on("opentag", tag => {
    ...
    if (tagLocal === "style" && tagNamespace === HTML_NS) {
      elem._isOnStackOfOpenElements = true;
    }
    ...
    if (tagLocal === "style" && tagNamespace === HTML_NS && !saxesOptions.fragment) {
      elem._parserDocument = rootNode._ownerDocument;
    }
  ...
  parser.on("closetag", () => {
    ...
    if ((elem.localName === "script" || elem.localName === "style") && elem.namespaceURI === HTML_NS) {
