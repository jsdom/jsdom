...
class WindowImpl extends EventTargetImpl {
  constructor(globalObject, args, { globalProxy, options, vmContext }) {
    super(globalObject);
    ...
    this.document = documents.createImpl(globalObject, {
    ...
    this._listOfActiveTimers = new Map();
    this._latestTimerId = 0;

    // https://html.spec.whatwg.org/multipage/imagebitmap-and-animations.html#animation-frames

    this._animationFrameCallbackId = 0;
    this._animationFrameNodejsInterval = null;
    this._mapOfAnimationFrameCallbacks = new Map();
    ...
    this._numberOfOngoingAnimationFrameCallbacks = 0;
    ...
    this.customElements = CustomElementRegistry.createImpl(globalObject);
  ...
  clearTimeout(handle = 0) {
    const nodejsTimer = this._listOfActiveTimers.get(handle);
    if (nodejsTimer) {
      clearTimeout(nodejsTimer);
      this._listOfActiveTimers.delete(handle);
    }
  }
  ...
  requestAnimationFrame(callback) {
    const handle = ++this._animationFrameCallbackId;
    this._mapOfAnimationFrameCallbacks.set(handle, callback);

    if (++this._numberOfOngoingAnimationFrameCallbacks === 1) {
      this._animationFrameNodejsInterval = setInterval(() => {
  ...
  cancelAnimationFrame(handle) {
    if (this._mapOfAnimationFrameCallbacks.has(handle)) {
      if (--this._numberOfOngoingAnimationFrameCallbacks === 0) {
        clearInterval(this._animationFrameNodejsInterval);
        this._animationFrameNodejsInterval = null;
      }
    }

    this._mapOfAnimationFrameCallbacks.delete(handle);
  }

  // TODO: Support `options.transfer`
  ...
  close() {
    ...
    for (const nodejsTimer of this._listOfActiveTimers.values()) {
      clearTimeout(nodejsTimer);
    }
    this._listOfActiveTimers.clear();

    if (this._animationFrameNodejsInterval !== null) {
      clearInterval(this._animationFrameNodejsInterval);
    }
...
function timerInitializationSteps(handler, timeout, args, {
  window,
  repeat,
  previousHandle: handle = ++window._latestTimerId
}) {
  ...
  function task() {
    if (!window._listOfActiveTimers.has(handle)) {
      return;
    }
    ...
    try {
      if (typeof handler === "function") {
        ...
        vm.runInContext(handler, window._vmContext, {
    ...
    if (window._listOfActiveTimers.has(handle)) {
      if (repeat) {
        ...
        window._listOfActiveTimers.delete(handle);
      }
    }
  }
  ...
  window._listOfActiveTimers.set(handle, nodejsTimer);

  return handle;
}

function runAnimationFrameCallbacks(window, now) {
  const { _mapOfAnimationFrameCallbacks } = window;

  // Converting to an array is important to get a sync snapshot and thus match spec semantics.
  const callbackHandles = [..._mapOfAnimationFrameCallbacks.keys()];
  for (const handle of callbackHandles) {
    // This has() can be false if a callback calls cancelAnimationFrame().
    if (_mapOfAnimationFrameCallbacks.has(handle)) {
      const callback = _mapOfAnimationFrameCallbacks.get(handle);
