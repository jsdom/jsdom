...
class WindowImpl extends EventTargetImpl {
  constructor(globalObject, args, privateData) {
    ...
    if (!globalObject[idlUtils.implSymbol]) {
    ...
    const { globalProxy, options } = privateData;
    ...
    this.parent = this.top = this._globalProxy;
  ...
  requestAnimationFrame(callback) {
    ...
    if (++this.numberOfOngoingAnimationFrameCallbacks === 1) {
      this.animationFrameNodejsInterval = setInterval(() => {
        runAnimationFrameCallbacks(this.rawPerformance.now() - this.windowInitialized);
      }, 1000 / 60);
    }
...
exports.init = (windowImpl, { wrapper }) => {
  wrapper.addEventListener = wrapper.addEventListener.bind(this);
  wrapper.removeEventListener = wrapper.removeEventListener.bind(this);
  wrapper.dispatchEvent = wrapper.dispatchEvent.bind(this);
...
function timerInitializationSteps(handler, timeout, args, {
  ...
  function task() {
    ...
    if (repeat && window._listOfActiveTimers.has(handle)) {
