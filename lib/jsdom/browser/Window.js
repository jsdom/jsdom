...
const { forEachMatchingSheetRuleOfElement, SHADOW_DOM_PSEUDO_REGEXP } = require("../living/helpers/style-rules.js");
...
const CSSStyleDeclarationComputed = require("../living/helpers/CSSStyleDeclarationComputed");
...
function Window(options) {
  ...
  this._defaultStyle = {}; // key is localName
  // key is eltImpl object
  this._sheetStyle = new Map();
  this._computedStyle = new Map();
  this._computedStyleTarget = new Map();

  this._removeElement = function (elt) {
    this._sheetStyle.delete(elt);
    if (this._computedStyle.has(elt)) {
      // delete contents of computedStyle
      // reference to computedStyle can be held in user-space
      const computedStyle = this._computedStyle.get(elt);
      computedStyle._write = ["set", "_sheetStyle", {}];
      this._sheetStyle.delete(elt);
      computedStyle._write = ["set", "_inlineStyle", {}];
      computedStyle._update();
    }
  };

  this._updateSheetStyle = function (argElt = null) {
    // elt null: update sheetStyle of all elt
    const eltList = argElt ? [argElt] : this._sheetStyle.keys();
    const { forEach } = Array.prototype;
    for (const elt of eltList) {
      const sheetStyle = this._sheetStyle.get(elt);
      if (!sheetStyle) {
        return;
      }
      forEachMatchingSheetRuleOfElement(elt, rule => {
        forEach.call(rule.style, property => {
          sheetStyle.setProperty(
            property,
            rule.style.getPropertyValue(property),
            rule.style.getPropertyPriority(property)
          );
        });
      }, false, true); // get only sheetStyle
      // update computed keys TODO remove
      if (this._computedStyle.has(elt)) {
        this._computedStyle.get(elt)._update();
      }
    }
  };
  ...
  // always reflecting the current style of elt
  this.getComputedStyle = function (elt) {
    elt = Element.convert(elt);
    // when called as getComputedStyle(e)
    // not window.getComputedStyle(e), then `this` is undefined
    const eltWindow = elt._globalObject;
    ...
    if (pseudoElt !== undefined && pseudoElt !== null && pseudoElt !== "") {
      ...
      notImplemented("window.computedStyle(elt, pseudoElt)", eltWindow);
    }

    if (!eltWindow._computedStyle.has(elt)) {
      const { forEach } = Array.prototype;

      let defaultStyle = eltWindow._defaultStyle[elt.localName];
      if (!defaultStyle) {
        defaultStyle = new CSSStyleDeclaration();
        eltWindow._defaultStyle[elt.localName] = defaultStyle;
        forEachMatchingSheetRuleOfElement(elt, rule => {
          forEach.call(rule.style, property => {
            defaultStyle.setProperty(
              property,
              rule.style.getPropertyValue(property)
              // default priority is empty
            );
          });
        }, true, false); // get only defaultStyle
      }

      let sheetStyle = eltWindow._sheetStyle.get(elt);
      if (!sheetStyle) {
        sheetStyle = new CSSStyleDeclaration();
        eltWindow._sheetStyle.set(elt, sheetStyle);
        eltWindow._updateSheetStyle(elt);
      }

      const computedStyleTarget = new CSSStyleDeclarationComputed(
        eltWindow,
        elt._style, // inlineStyle
        sheetStyle,
        defaultStyle
      );
      eltWindow._computedStyleTarget.set(elt, computedStyleTarget);

      const computedStyle = new Proxy(computedStyleTarget, {
        // catch-all handler
        get(target, key) {
          return target._get(key);
        },
        set(target, key, val) {
          return target._set(key, val);
        }
      });
      eltWindow._computedStyle.set(elt, computedStyle);
      ...
      // TODO handle element.remove()
      // delete inline + custom style (remove all props,
      // client can hold references and block garbage-collection)
      // computedStyle should still work
      // = return empty (firefox) or default values (chrome)
    }
    return eltWindow._computedStyle.get(elt);
  };
