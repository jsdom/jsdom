...
const { getDeclarationForElement, getResolvedValue,
  propertiesWithResolvedValueImplemented } = require("../living/helpers/style-rules.js");
...
function installOwnProperties(window, options) {
  ...
  window.getComputedStyle = function (elt, pseudoElt = undefined) {
    ...
    const declaration = getDeclarationForElement(elementImpl);

    // Set up a temporary method to toggle the read-only flag
    declaration.readOnly = function (bool = false) {
      this._readonly = bool;
    };
