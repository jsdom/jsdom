...
function Window(options) {
  ...
  this.localStorage = this.sessionStorage = options.localStorage;
  ...
  this._document = Document.create([], {
    options: {
	  ...
	  afterCreateElement: options.afterCreateElement,
      global: this
    }
  });
  ...
  this._notifyNavigateOnSameURL = options._notifyNavigateOnSameURL;
  ...
  this.setTimeout = function _setTimeout(fn, ms) {
    const args = [];
  ...
  this.setTimeout.toString = function() {
    return "function setTimeout() { [native code] }";
  }

  this.setInterval = function _setInterval(fn, ms) {
    const args = [];
    for (let i = 2; i < arguments.length; ++i) {
      args[i - 2] = arguments[i];
    }
  ...
  this.setInterval.toString = function () {
  ...
  this.switchOut = function () {
	this._pretendToBeVisual = false;
	fireAnEvent("visibilitychange", window._document, undefined, {}, window._document);
  }

  this.switchIn = function () {
    this._pretendToBeVisual = true;
	fireAnEvent("visibilitychange", window._document, undefined, {}, window._document);
  }

  this.open = function (url) {
    // Set new target temporery
    this.location.href = url;
  }
