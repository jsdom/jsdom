...
function Window(options) {
  ...
  this.getComputedStyle = function (elt) {
    ...
    if (!this._computedStyle[elt]) {
      const { forEach } = Array.prototype;

      let defaultStyle = this._defaultStyle[elt.localName];
      if (!defaultStyle) {
        defaultStyle =
          this._defaultStyle[elt.localName] =
          new cssstyle.CSSStyleDeclaration();
        defaultStyle = this._defaultStyle[elt.localName];
      ...
      let customStyle = this._customStyle[elt];
      if (!customStyle) {
        customStyle =
          this._customStyle[elt] =
          new cssstyle.CSSStyleDeclaration();
        forEachMatchingSheetRuleOfElement(elt, rule => {
          forEach.call(rule.style, property => {
      ...
      this._computedStyle[elt] = new Proxy(
        this._computedStyleTarget[elt], {
        // catch-all handler
        get: function(target, key) {return target._get(key)},
        set: function(target, key, val) {return target._set(key, val)},
      });

      // init props
      this._computedStyle[elt]._update();
    ...
    return this._computedStyle[elt];
  }
