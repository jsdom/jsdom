...
function Window(options) {
  ...
  this.getComputedStyle = function (node) {
    ...
    const currentSpecificities = {
      setPropertySpecificityIfGreater (propertyName, selectorText) {
        const newSpecificity = specificity.calculate(selectorText)[0].specificityArray;
      ...
      for (const selectorText of selectors) {
        if (selectorText !== "" && selectorText !== "," && !matched && matchesDontThrow(nodeImpl, selectorText)) {
          ...
          forEach.call(rule.style, property => {
            ...
            const newImportance = rule.style.getPropertyPriority(property);
