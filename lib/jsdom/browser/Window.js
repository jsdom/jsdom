...
const { CSSPseudoElement, CSSStyleProperties, parseGrammar } = require("@cdoublev/css");
...
const SHADOW_DOM_PSEUDO_REGEXP = /^::(?:details-content|part|slotted)\(/i;
...
function installOwnProperties(window, options) {
  ...
  window.getComputedStyle = function (elt, pseudoElt = undefined) {
    let elementImpl = Element.convert(window, elt);

    if (pseudoElt?.startsWith(':')) {
      if (SHADOW_DOM_PSEUDO_REGEXP.test(pseudoElt)) {
        elementImpl = null;
      } else if (pseudoElt === pseudoElt.trimEnd(' ')) {
        const type = parseGrammar(pseudoElt, '<pseudo-element-selector>');
        if (type === null || type instanceof Error) {
          elementImpl = null;
        } else {
          elementImpl = CSSPseudoElement.createImpl(window, {
            element: elementImpl,
            parent: elementImpl,
            selectorText: pseudoElt,
            type: pseudoElt.replace(/^([^(]+)\(.*\)$/, "$1()"),
          });
        }
      }
    }

    return CSSStyleProperties.create(window, undefined, {
      computed: true,
