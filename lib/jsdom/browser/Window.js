...
function Window(options) {
  ...
  this.getComputedStyle = function (elt) {
    ...
    if (!eltWindow._computedStyle[elt]) {
      const { forEach } = Array.prototype;

      let defaultStyle = eltWindow._defaultStyle[elt.localName];
      if (!defaultStyle) {
        defaultStyle =
          eltWindow._defaultStyle[elt.localName] =
      ...
      let customStyle = eltWindow._customStyle[elt];
      if (!customStyle) {
        customStyle =
          eltWindow._customStyle[elt] =
          new CSSStyleDeclaration();
        forEachMatchingSheetRuleOfElement(elt, rule => {
          forEach.call(rule.style, property => {
            customStyle.setProperty(
              property,
              rule.style.getPropertyValue(property),
              rule.style.getPropertyPriority(property)
            );
          });
        }, false, true); // get only customStyle
      }

      eltWindow._computedStyleTarget[elt] =
        new CSSStyleDeclarationComputed(
          eltWindow,
      ...
      eltWindow._computedStyle[elt] = new Proxy(eltWindow._computedStyleTarget[elt], {
        // catch-all handler
        get(target, key) {
      ...
      // init props
      eltWindow._computedStyle[elt]._update();

      // TODO listen for updates from custom style
      // like changed style sheets
      // TODO remove _update()
    ...
    return eltWindow._computedStyle[elt];
  };
