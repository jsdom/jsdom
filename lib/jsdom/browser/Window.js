...
const { focusFixupRule } = require("../living/helpers/focusing");
...
function installOwnProperties(window, options) {
  ...
  if (window._pretendToBeVisual) {
    ...
    function runAnimationFrameCallbacks(now) {
      ...
      // Step 17: For each fully active Document in docs, run the focus fixup rule for that Document.
      if (!window._document._isDestroyed) {
        const activeElement = window._document._lastFocusedElement;
        if (activeElement && activeElement.nodeType === 1) {
          focusFixupRule(window._document);
        }
      }
    }
