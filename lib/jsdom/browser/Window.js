...
function Window(options) {
  ...
  // TODO on elt.remove():
  //  * delete customStyle
  //  * maybe delete all props of computedStyle
  this.getComputedStyle = function (elt) {
    ...
    if (!this._computedStyle[elt]) {
      ...
      // handle updates from ElementCSSInlineStyle
      elt._onInlineStyleChange = function(newStyle) {
        forEach.call(newStyle, property => {
          const newValue = newStyle.getPropertyValue(property);
          if (newValue == defaultStyle.getPropertyValue(property)) {
            customStyle.removeProperty(property);
          } else {
            customStyle.setProperty(
              property,
              newValue,
      ...
      this._computedStyle[elt] = new Proxy(customStyle, {
        get: function(customStyle, property, receiver) {
          if (property == 'setProperty' || property == 'removeProperty') {
            receiver[0] = 0; // call set()
          }
          return customStyle[property] || defaultStyle[property];
        },
        set: function() {
          throw DOMException.create(window, [
