...
function Window(options) {
  ...
  this.getComputedStyle = function (node) {
    ...
    // container object to hold a rolling collection of property specificity
    let currentSpecificities = {
      setPropertySpecificityIfGreater: function(propertyName, selectorText){
        let newSpecificity = specificity.calculate(selectorText)[0].specificityArray;
        if(this[propertyName]){
          if(specificity.compare(newSpecificity, this[propertyName]) > -1){
            this[propertyName] = newSpecificity;
            return true;
          }
          else{
            return false;
      ...
      for (const selectorText of selectors) {
        if (selectorText !== "" && selectorText !== "," && !matched && matchesDontThrow(nodeImpl, selectorText)) {
          ...
          forEach.call(rule.style, property => {
            // Check priority (!important exceptions) first since it take precedence over specificity.
            // if priority matches (neither have priority or both have !important)
            //      take the one with higher specificity or the new if specificity is equal
            // if priority doesn't match
            //      take new if new is !important, otherwise old is !important so do nothing (keep old one)
            let newImportance = rule.style.getPropertyPriority(property);
            if(cs.getPropertyPriority(property) == newImportance){
              if(currentSpecificities.setPropertySpecificityIfGreater(property, selectorText)){
                cs.setProperty(property, rule.style.getPropertyValue(property), rule.style.getPropertyPriority(property));
              }
            }
            else if(newImportance){
              currentSpecificities.setPropertySpecificityIfGreater(property, selectorText);
              cs.setProperty(property, rule.style.getPropertyValue(property), rule.style.getPropertyPriority(property));
            }
          });
        }
    ...
    forEach.call(s, property => {
      // take inline style properties unless the property is !important and inline is not
      if(!cs.getPropertyPriority(property) || s.getPropertyPriority(property) != ''){
