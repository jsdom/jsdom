...
exports.stringifyElement = function stringifyElement(element, xhtml) {
...
exports.makeHtmlGenerator = function makeHtmlGenerator(indentUnit, eol, xhtml) {
    ...
    if (node) {
      ...
      switch (node.nodeType) {
        case node.ELEMENT_NODE:
          current = exports.stringifyElement(node, xhtml);
          ret += curIndent + current.start;
          if (node._childNodes.length > 0) {
            var prevType = node.ELEMENT_NODE;
            var curType = null;
            var nextIndent = curIndent;
            for (i=0; i<node._childNodes.length; i++) {
              curType = node._childNodes[i].nodeType;
              if (prevType == node.ELEMENT_NODE && curType == prevType) {
                ret += eol;
                nextIndent = curIndent + indentUnit;
              } else nextIndent = '';
              prevType = curType;
              ret += generateHtmlRecursive(node._childNodes[i], childNodesRawText, nextIndent);
            }
            if (nextIndent.length > 0) ret += eol + curIndent;
            ret += current.end;
          } else {
            ret += ((rawText ? node.nodeValue : HTMLEncode(node.nodeValue, false)) || '') + current.end;
          }
          break;
        case node.TEXT_NODE:
...
exports.domToHtml = function(dom, noformat, raw, xhtml) {
