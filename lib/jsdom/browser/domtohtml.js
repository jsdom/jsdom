...
exports.domToHtml = function(dom) {
  ...
  if (typeof dom.length !== 'undefined') {
    ...
    for (var i = 0, len = dom.length; i < len; i++) {
      if (dom[i]._tagName === '#document') {
        ret += serializer.serialize(dom[i]);
      } else {
        ret += serializer.serialize({ childNodes: [dom[i]] });
      }
    }
    return ret;
  } else {
    // single node
    if (dom._tagName === '#document') {
      return serializer.serialize(dom);
    } else {
      return serializer.serialize({ childNodes: [dom] });
    }
