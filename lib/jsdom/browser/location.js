...
var nodeURL = require("url"); // TODO Location: use new URL stuff
...
function Location(urlString, document) {
  this._url = nodeURL.parse(urlString);
...
Location.prototype = {
  ...
  reload: function () {
    notImplemented("location.reload", this._document._defaultView);
  },
  ...
  set href(val) {
    ...
    this._url = nodeURL.parse(resolveHref(oldUrl, val));
    ...
    if (oldProtocol === newProtocol && oldHost === newHost && oldPathname === newPathname && oldHash !== newHash) {
      ...
      notImplemented("location.href reloading", this._document._defaultView);
    }
  },
  ...
  set search(val) {
    ...
    if (val.length) {
      ...
      this._url = nodeURL.parse(resolveHref(oldUrl, val + oldHash));
    } else {
      this._url = nodeURL.parse(oldUrl.replace(/\?([^#]+)/, ""));
    }
  },
