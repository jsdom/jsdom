...
function readDataUrl(dataUrl, { defaultEncoding, detectMetaCharset }, callback) {
  try {
    ...
    if (defaultEncoding) {
      const mimeType = new MIMEType(data.type) || new MIMEType("text/plain");
      const sniffOptions = {
        transportLayerEncodingLabel: mimeType.get("charset"),
        defaultEncoding
      };
      ...
      const encoding = detectMetaCharset ?
                       ...
                       whatwgEncoding.getBOMEncoding(data.buffer) ||
                        whatwgEncoding.labelToName(mimeType.get("charset")) ||
                        defaultEncoding;
      const decoded = whatwgEncoding.decode(data.buffer, encoding);

      mimeType.set("charset", encoding);
      data.type = mimeType.toString();
...
exports.download = function (url, options, callback) {
  ...
  const req = request(url, requestOptions, (error, response, bufferData) => {
    if (!error) {
      // If default encoding does not exist, pass on binary data.
      if (defaultEncoding) {
        const mimeType = new MIMEType(response.headers["content-type"]) || new MIMEType("text/plain");
        const sniffOptions = {
          transportLayerEncodingLabel: mimeType.get("charset"),
          defaultEncoding
        };
        ...
        mimeType.set("charset", encoding);
        response.headers["content-type"] = mimeType.toString();
