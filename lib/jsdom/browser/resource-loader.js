...
exports.readFile = function (filePath, options, callback) {
  if(options.detectMetaCharset === undefined) {
    options.detectMetaCharset = false;
  }
  ...
  readableStream.on("end", () => {
    ...
    if (options.defaultEncoding !== null) {
      const decoded = decodeString(data, null, options.defaultEncoding, options.detectMetaCharset);
...
function readDataUrl(dataUrl, options, callback) {
  try {
    ...
    if (options.defaultEncoding !== null) {
      ...
      const decoded = decodeString(data.buffer, contentType, options.defaultEncoding, options.detectMetaCharset);
  ...
  const req = request(url, requestOptions, (error, response, bufferData) => {
    if (!error) {
      ...
      if (options.defaultEncoding !== null) {
        ...
        const decoded = decodeString(bufferData, contentType, options.defaultEncoding, options.detectMetaCharset);
        contentType.set("charset", decoded.encoding);
