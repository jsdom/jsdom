...
exports.enqueue = function (element, resourceUrl, callback, timing) {
  ...
  if (document._queue) {
    ...
    switch (timing) {
      case "async": {
        return document._queue.async(loadHandler);
      }
      case "defer": {
        return document._queue.defer(loadHandler);
      }
      default: {
        return document._queue.soon(loadHandler);
      }
    }
  }
...
exports.load = function (element, urlString, options, callback, timing) {
  ...
  const enqueued = exports.enqueue(element, urlString, callback, timing);
