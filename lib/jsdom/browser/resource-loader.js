...
function readDataUrl(dataUrl, { defaultEncoding, detectMetaCharset }, callback) {
  try {
    ...
    if (defaultEncoding) {
      const parsedMIMEType = firstMIMEType([data.type, "text/plain"]);
      const sniffOptions = {
        transportLayerEncodingLabel: parsedMIMEType.parameters.get("charset"),
        defaultEncoding
      };
      ...
      const encoding = detectMetaCharset ?
                       ...
                       whatwgEncoding.getBOMEncoding(data.buffer) ||
                        whatwgEncoding.labelToName(parsedMIMEType.parameters.get("charset")) ||
                        defaultEncoding;
      const decoded = whatwgEncoding.decode(data.buffer, encoding);

      parsedMIMEType.parameters.set("charset", encoding);
      data.type = parsedMIMEType.toString();
...
exports.download = function (url, options, callback) {
  ...
  const req = request(url, requestOptions, (error, response, bufferData) => {
    if (!error) {
      // If default encoding does not exist, pass on binary data.
      if (defaultEncoding) {
        const parsedMIMEType = firstMIMEType([response.headers["content-type"], "text/plain"]);
        const sniffOptions = {
          transportLayerEncodingLabel: parsedMIMEType.parameters.get("charset"),
          defaultEncoding
        };
        ...
        parsedMIMEType.parameters.set("charset", encoding);
        response.headers["content-type"] = parsedMIMEType.toString();
