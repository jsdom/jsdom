...
const { parseDataUrl, firstMIMEType } = require("../utils");
const fs = require("fs");
...
function readDataUrl(dataUrl, { defaultEncoding, detectMetaCharset }, callback) {
  try {
    ...
    if (defaultEncoding) {
      const parsedContentType = firstMIMEType([data.type, "text/plain"]);
      const sniffOptions = {
        transportLayerEncodingLabel: parsedContentType.parameters.get("charset"),
        defaultEncoding
      };
      ...
      const encoding = detectMetaCharset ?
                       ...
                       whatwgEncoding.getBOMEncoding(data.buffer) ||
                        whatwgEncoding.labelToName(parsedContentType.parameters.get("charset")) ||
                        defaultEncoding;
      const decoded = whatwgEncoding.decode(data.buffer, encoding);

      parsedContentType.parameters.set("charset", encoding);
      data.type = parsedContentType.toString();
...
exports.download = function (url, options, callback) {
  ...
  const req = request(url, requestOptions, (error, response, bufferData) => {
    if (!error) {
      // If default encoding does not exist, pass on binary data.
      if (defaultEncoding) {
        const parsedContentType = firstMIMEType([response.headers["content-type"], "text/plain"]);
        const sniffOptions = {
          transportLayerEncodingLabel: parsedContentType.parameters.get("charset"),
          defaultEncoding
        };
        ...
        parsedContentType.parameters.set("charset", encoding);
        response.headers["content-type"] = parsedContentType.toString();
