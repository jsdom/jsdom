...
exports.readFile = function (filePath, callback, encoding, detectMetaCharset) {
  ...
  readableStream.on("end", () => {
    // Passing null for encoding means binary
    if (encoding !== null) {
      const decoded = decodeString(data, null, encoding, detectMetaCharset);
      callback(null, decoded.data, { headers: {"content-type": "text/plain;charset=" + decoded.charset }});
    } else {
      callback(null, data);
    }
  });
...
function fetch(element, urlObj, cookieJar, referrer, pool, agentOptions,
                strictSSL, userAgent, callback, encoding, detectMetaCharset) {
  let req = null;
  if (urlObj.protocol === "data:") {
    try {
      ...
      if (encoding !== null) {
        ...
        const decoded = decodeString(data.buffer, contentType, encoding, detectMetaCharset);
        contentType.set("charset", decoded.charset);
    ...
    req = exports.download(urlObj, requestOptions, cookieJar, referrer, detectMetaCharset, callback);
  } else {
    const filePath = urlObj.pathname
      .replace(/^file:\/\//, "")
      .replace(/^\/([a-z]):\//i, "$1:/")
      .replace(/%20/g, " ");
    req = exports.readFile(filePath, callback, encoding, detectMetaCharset);
  }
  return req;
}
...
exports.download = function (url, options, cookieJar, referrer, detectMetaCharset, callback) {
  options = options || {};
  ...
  const req = request(url, options, (error, response, bufferData) => {
    if (!error) {
      // If encoding is null, pass on binary data.
      if (encoding !== null) {
        const contentType = parseContentType(response.headers["content-type"]) || parseContentType("text/plain");
        const decoded = decodeString(bufferData, contentType, encoding, detectMetaCharset);
        contentType.set("charset", decoded.charset);
...
exports.load = function (element, url, callback, encoding, detectMetaCharset) {
    ...
    req = customLoader({
      ...
      defaultFetch(fetchCallback) {
        return fetch(element, urlObj, cookieJar, baseUrl, pool, agentOptions,
            strictSSL, userAgent, fetchCallback, encoding, detectMetaCharset);
      }
    },
    wrappedEnqueued);
  } else {
    req = fetch(element, urlObj, cookieJar, baseUrl, pool, agentOptions,
        strictSSL, userAgent, wrappedEnqueued, encoding, detectMetaCharset);
  }
  if (req && requestManager) {
