...
function readDataUrl(dataUrl, { defaultEncoding, detectMetaCharset }, callback) {
  try {
    ...
    if (defaultEncoding) {
      ...
      const sniffOptions = {
        transportLayerEncodingLabel: parsedContentType.get("charset"),
        defaultEncoding
      };
      ...
      const encoding = detectMetaCharset ?
                       ...
                       whatwgEncoding.getBOMEncoding(data.buffer) ||
                        whatwgEncoding.labelToName(parsedContentType.get("charset")) ||
                        defaultEncoding;
      const decoded = whatwgEncoding.decode(data.buffer, encoding);

      parsedContentType.set("charset", encoding);
