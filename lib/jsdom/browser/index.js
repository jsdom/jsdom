var http          = require('http'),
    ...
    jsdom         = require('../../jsdom'),
    sizzle        = require('../selectors/sizzle'),
    Contextify    = null;
...
exports.createWindow = function(dom, options) {
  ...
  DOMWindow.prototype = {
    ...
    getComputedStyle: function(node) {
      var s = node.style,
          cs = new this.CSSStyleDeclaration(),
          Sizzle = sizzle(node.ownerDocument),
          forEach = Array.prototype.forEach;

      forEach.call(node.ownerDocument.styleSheets, function (sheet) {
        forEach.call(sheet.cssRules, function (rule_set) {
	  if (Sizzle.matchesSelector(node, rule_set.selectorText)) {
            forEach.call(rule_set.style, function (property) {
		cs.setProperty(property, rule_set.style.getPropertyValue(property), rule_set.style.getPropertyPriority(property));
	    });
	  }
	});
      });
