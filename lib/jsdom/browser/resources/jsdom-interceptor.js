...
async function handleHTTPWithoutRedirects(dispatch, opts, handler, { cookieJar, userAgent }) {
  ...
  const jar = shouldHandleCookies ? cookieJar : null;
  return dispatchWithCookieHandling(dispatch, dispatchOpts, handler, currentURL, jar);
}
  ...
  return new Promise(resolve => {
    ...
    dispatch(opts, {
      onRequestStart(controller, context) {
        // Forward onRequestStart with the current URL in history
...
async function handleHTTPWithRedirects(dispatch, opts, handler, { cookieJar, userAgent }) {
  ...
  for (let redirectCount = 0; redirectCount <= MAX_REDIRECTS; redirectCount++) {
    ...
    const result = await dispatchWithStreamingSupport(dispatch, dispatchOpts, handler, currentURL, cookieJar, history);
 ...
 * Uses NEW undici handler API throughout.
 */
function dispatchWithStreamingSupport(dispatch, opts, handler, currentURL, cookieJar, history) {
  return new Promise(resolve => {
    ...
    dispatch(opts, {
      ...
      onResponseStart(controller, status, headersObj, statusText) {
        ...
        if (!isRedirect) {
          ...
          // Forward onRequestStart with full redirect history
