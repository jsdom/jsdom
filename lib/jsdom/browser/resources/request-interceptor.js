...
module.exports = function requestInterceptor(fn) {
  return dispatch => (options, handler) => {
    ...
    const pendingResponse = { onResponse, onError, reason: undefined };
    const { undiciController, wrappedHandler } = createSignalHandler(handler, abortController, pendingResponse);
    ...
    async function intercept() {
      ...
      handleResponse(fn(request, { element }), pendingResponse);
    }
    ...
    intercept().catch(onError);

    return true;
  };
};

// Keep these reactions outside the dispatch closure so cancellation can release all document state.
function handleResponse(value, pendingResponse) {
  Promise.resolve(value).then(response => {
    if (pendingResponse.onResponse !== undefined) {
      pendingResponse.onResponse(response);
    } else if (response instanceof Response && response.body !== null) {
      return response.body.cancel(pendingResponse.reason);
    }
    return undefined;
  }).catch(error => {
    pendingResponse.onError?.(error);
  });
}
 ...
 */
function createSignalHandler(handler, abortController, pendingResponse) {
  const { signal } = abortController;
  ...
  function onPendingAbort() {
    pendingResponse.reason = signal.reason;
    pendingResponse.onResponse = undefined;
    pendingResponse.onError = undefined;
