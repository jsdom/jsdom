...
const { fetchCollected } = require("./jsdom-fetch");

module.exports = class PerDocumentResourceLoader {
  constructor(document) {
    this._document = document;
    this._defaultEncoding = document._encoding;
    this._dispatcher = document._defaultView ? document._defaultView._dispatcher : null;
  ...
  fetch(url, { element, onLoad, onError }) {
    ...
    const fetchPromise = fetchCollected(this._dispatcher, {
      ...
      opaque: {
        ...
        document: idlUtils.wrapperForImpl(this._document)
      }
    });
    ...
    const onLoadWrapped = response => {
      ...
      const { body: data, status, statusText, headers, url: responseURL } = response;
      const responseObj = {
