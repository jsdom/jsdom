...
module.exports = class PerDocumentResourceLoader {
  ...
  fetch(url, { element, onLoad, onError }) {
    ...
    const onErrorWrapped = error => {
      ...
      event.initCustomEvent("DocumentRequestFinishEvent", false, false, {url: url, error: error, response: request.response});
    ...
    const onLoadWrapped = data => {
      ...
      try {
        ...
        return Promise.resolve(result)
          .then(() => {
            const event = this._document.createEvent("CustomEvent");
            event.initCustomEvent("DocumentRequestFinishEvent", false, false, {url: url, data: data, response: request.response});
            this._document.dispatchEvent(event);
