      ...
      case "https": {
        ...
        const promise = new Promise((resolve, reject) => {
          ...
          requestClient.on("data", chunk => {
            accumulatedBytelength += chunk.length;
            accumulated.push(chunk);
          });
          requestClient.on("end", () => {
            if (requestClient.isAborted) {
              resolve(null);
            } else {
              try {
                resolve(Buffer.concat(accumulated, accumulatedBytelength));
              } catch (error) {
                // This try-catch-loop was written to avoid the test suite halting
                reject(error);
              }
            }
          });
