...
class JSDOMDispatcher extends Dispatcher {
   ...
   * @param {boolean} [opts.opaque.noRedirectFollow] - If true, don't follow redirects (used by XHR)
  ...
  dispatch(opts, handler) {
    ...
    return this.#dispatchHTTP(opts, wrappedHandler, { followRedirects, url });
  }
   ...
   */
  #dispatchHTTP(opts, handler, { followRedirects, url }) {
    if (followRedirects) {
      this.#dispatchHTTPWithRedirects(opts, handler, url);
    } else {
      this.#dispatchHTTPWithoutRedirects(opts, handler, url);
    }
    return true;
  }
   ...
   */
  async #dispatchHTTPWithoutRedirects(opts, handler, url) {
   ...
   */
  async #dispatchHTTPWithRedirects(opts, handler, url) {
    let currentURL = url;
    ...
    for (let redirectCount = 0; redirectCount <= MAX_REDIRECTS; redirectCount++) {
      ...
      const dispatchOpts = {
        ...
        headers: requestHeaders,
        body: currentBody,
        opaque: { ...opts.opaque, url: currentURL }
      };
