...
class JSDOMDispatcher extends Dispatcher {
  ...
  #dispatchDataURL(url, handler) {
    ...
    handler.onRequestStart?.(null, { history: [url.href] });
  ...
  async #dispatchWebSocket(opts, handler, url) {
    ...
    const dispatchOpts = {
      ...
      opaque: { ...opts.opaque, url: url.href }
    };
    ...
    innerDispatch(dispatchOpts, {
      onRequestStart(controller, context) {
        handler.onRequestStart?.(controller, { ...context, history: [urlHref] });
      },
      onRequestUpgrade(controller, statusCode, headersObj, socket) {
        if (shouldHandleCookies) {
          storeCookiesFromHeaders(cookieJar, headersObj, urlHref);
        }
      ...
      onResponseEnd(controller, trailers) {
        if (shouldHandleCookies) {
          storeCookiesFromHeaders(cookieJar, responseHeaders, urlHref);
        }
  ...
  async #dispatchHTTP(opts, handler, url) {
    ...
    for (let redirectCount = 0; redirectCount <= MAX_REDIRECTS; redirectCount++) {
      ...
      if (origin !== undefined && origin !== currentURL.origin) {
        const acao = result.headers["access-control-allow-origin"];
