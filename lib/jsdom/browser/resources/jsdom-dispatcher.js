...
const { parseURL, serializeURL, serializeURLOrigin, serializePath } = require("../../living/helpers/url");
const { samePartition } = require("../../living/file-api/blob-url-store");
 ...
 *
 * Handles:
 * - blob: URLs (read the retained Blob)
...
class JSDOMDispatcher extends Dispatcher {
   ...
   * @param {object} [opts.opaque.urlRecord] - Parsed URL with the blob entry retained when the request was created
   * @param {Document|string|null} [opts.opaque.environment] - Requesting document or "top-level-navigation"
  ...
  dispatch(opts, handler) {
    ...
    const retainedURL = opts.opaque?.urlRecord;
    const urlRecord = retainedURL?.scheme === "blob" && serializeURL(retainedURL) === urlString ?
      retainedURL :
      parseURL(urlString);
    ...
    if (urlRecord.scheme === "blob") {
      return this.#dispatchBlobURL(urlRecord, handler, opts);
    }
  ...
  /**
   * https://fetch.spec.whatwg.org/#concept-scheme-fetch, "blob" scheme.
   */
  #dispatchBlobURL(urlRecord, handler, opts) {
    const entry = urlRecord.blobURLEntry;
    const environment = opts.opaque?.environment ?? null;
    if ((opts.method || "GET") !== "GET" || entry === null ||
        (environment !== "top-level-navigation" && !samePartition(entry, environment))) {
      handler.onResponseError?.(null, new TypeError("Cannot fetch blob URL"));
      return false;
    }

    const blob = entry.object;
    let bytes = blob._bytes;
    let status = 200;
    let statusText = "OK";
    const headers = { "content-type": blob.type, "content-length": String(bytes.byteLength) };
    const requestHeaders = this.#normalizeHeadersToObject(opts.headers);
    const rangeName = Object.keys(requestHeaders).find(name => name.toLowerCase() === "range");

    if (rangeName !== undefined) {
      // https://fetch.spec.whatwg.org/#simple-range-header-value, with allowWhitespace set to true.
      // Only match whitespace after a nonempty start, avoiding overlapping whitespace runs and quadratic backtracking.
      const match = /^bytes[\t ]*=[\t ]*(?:([0-9]+)[\t ]*)?-[\t ]*([0-9]*)$/.exec(requestHeaders[rangeName]);
      if (match === null || (match[1] === undefined && match[2] === "")) {
        handler.onResponseError?.(null, new TypeError("Invalid blob URL range"));
        return false;
      }
      const fullLength = bytes.byteLength;
      let start = match[1] === undefined ? null : Number(match[1]);
      let end = match[2] === "" ? null : Number(match[2]);
      if (start !== null && ((end !== null && start > end) || start >= fullLength)) {
        handler.onResponseError?.(null, new TypeError("Unsatisfiable blob URL range"));
        return false;
      }
      if (start === null) {
        start = fullLength - end;
        end = fullLength - 1;
      } else if (end === null || end >= fullLength) {
        end = fullLength - 1;
      }
      // Blobs are immutable. Keep a view until the response consumer copies the bytes.
    ...
    sendStreamResponse(handler, Readable.from([bytes]), {
      status,
      statusText,
      headers,
  ...
  async #dispatchHTTP(urlRecord, handler, opts) {
    ...
    if (BAD_PORTS.has(currentURL.port)) {
      handler.onResponseError?.(null, new Error(`URL uses a disallowed port: ${currentURL.port}`));
      return;
    }

    // Cancellation belongs to the whole request; the transport controller only controls the current hop.
    // Hop controllers can report `aborted` after normal completion, or ignore aborts once they have finished.
    ...
    const requestController = {
      ...
      resume() {
        transportController.resume();
      },
      get paused() {
        return transportController.paused;
      },
      get aborted() {
        return aborted;
      },
      ...
      get rawHeaders() {
        return transportController.rawHeaders;
      },
      get rawTrailers() {
        return transportController.rawTrailers;
      }
    };
    ...
    function onControllerReady(controller) {
      ...
      if (!onRequestStartCalled) {
        ...
        handler.onRequestStart?.(requestController, ctx);
      } else if (aborted) {
    ...
    if (preflight) {
      ...
      if (preflightResult.error) {
        handler.onResponseError?.(onRequestStartCalled ? requestController : null, preflightResult.error);
        return;
      }

      // Validate preflight response status
      if (preflightResult.status < 200 || preflightResult.status > 299) {
        handler.onResponseError?.(requestController, new Error(
      ...
      if (acao !== "*" && acao !== origin) {
        handler.onResponseError?.(requestController, new Error("Cross origin " + origin + " forbidden"));
        return;
      }
      if (withCredentials) {
        const acac = preflightResult.headers["access-control-allow-credentials"];
        if (acac !== "true") {
          handler.onResponseError?.(requestController, new Error("Credentials forbidden"));
          return;
        }
      }
      ...
      if (!acah.has("*")) {
        for (const unsafeHeader of preflight.unsafeHeaders || []) {
          if (!acah.has(unsafeHeader.toLowerCase())) {
            handler.onResponseError?.(requestController, new Error("Header " + unsafeHeader + " forbidden"));
            return;
          }
        }
      }
    ...
    for (let redirectCount = 0; redirectCount <= MAX_REDIRECTS; redirectCount++) {
      ...
      // WebSocket upgrade
      if (result.upgraded) {
        handler.onRequestUpgrade?.(requestController, result.statusCode, result.headers, result.socket);
        return;
      }

      if (result.error) {
        handler.onResponseError?.(onRequestStartCalled ? requestController : null, result.error);
        return;
      }
      ...
      if (result.status >= 300 && result.status < 400 && location) {
        ...
        if (!targetURL) {
          handler.onResponseError?.(requestController, new TypeError("Invalid redirect URL"));
          return;
        }
        ...
        if (targetURL.scheme !== "http" && targetURL.scheme !== "https") {
          handler.onResponseError?.(requestController, new Error("Cannot redirect to non-HTTP(S) URL"));
          return;
        }
        ...
        if (corsMode || targetIsCrossOrigin) {
          ...
          if (origin !== currentOrigin) {
            ...
            if (targetURL.username || targetURL.password) {
              handler.onResponseError?.(requestController, new Error("Userinfo forbidden in cors redirect"));
              return;
            }
      ...
      if (origin !== undefined && (origin !== currentOrigin || effectiveOrigin === "null")) {
        ...
        if (acao !== "*" && acao !== effectiveOrigin) {
          handler.onResponseError?.(requestController, new Error("Cross origin " + effectiveOrigin + " forbidden"));
          return;
        }
        if (withCredentials) {
          const acac = result.headers["access-control-allow-credentials"];
      ...
      handler.onResponseStart?.(requestController, result.status, result.headers, result.statusText);
    ...
    handler.onResponseError?.(requestController, new Error(`Too many redirects (max ${MAX_REDIRECTS})`));
  }
   ...
   * @param {object} requestController - Stable controller for cancellation and handler callbacks
   */
      ...
      innerDispatch(dispatchOpts, {
        ...
        onRequestUpgrade: (controller, statusCode, headersObj, socket) => {
          if (requestController.aborted) {
            resolve({ error: requestController.reason });
            return;
          }
          if (shouldHandleCookies) {
        ...
        onResponseStart: (controller, statusCode, headersObj, statusText) => {
          ...
          function forwardBodyTo(fwdHandler) {
            ...
            for (const chunk of pendingChunks) {
              fwdHandler.onResponseData?.(requestController, chunk);
            }
            pendingChunks = null;
            if (streamError) {
              fwdHandler.onResponseError?.(requestController, streamError);
            } else if (ended) {
              fwdHandler.onResponseEnd?.(requestController, {});
            }
          }
        ...
        onResponseData: (controller, chunk) => {
          ...
          if (bodyHandler) {
            bodyHandler.onResponseData?.(requestController, chunk);
          } else {
        ...
        onResponseEnd: (controller, trailers) => {
          if (requestController.aborted) {
            if (bodyHandler) {
              bodyHandler.onResponseError?.(requestController, requestController.reason);
            } else {
              streamError = requestController.reason;
            }
            return;
          }
          if (shouldHandleCookies) {
          ...
          if (bodyHandler) {
            bodyHandler.onResponseEnd?.(requestController, trailers);
          } else {
            ended = true;
          }
        },
        onResponseError: (controller, err) => {
          if (responseStarted) {
            ...
            if (bodyHandler) {
              bodyHandler.onResponseError?.(requestController, err);
            } else {
              streamError = err;
            }
          } else {
 ...
 * @param {object} [opts.urlRecord] - Parsed URL retained before unloading a frame's previous document
 ...
 * @param {Document|string|null} [opts.environment] - Requesting document or "top-level-navigation" (default: null)
 * @returns {Promise<{status: number, headers: object, body: Uint8Array, url: string, urlRecord: object, ok: boolean}>}
 */
async function fetchCollected(dispatcher, {
  url, headers, signal, element = null, environment = null, urlRecord = parseURL(url)
}) {
  if (!urlRecord) {
  ...
  const response = await dispatcher.request({
    ...
    headers,
    signal,
    opaque: { element, url, urlRecord, environment }
  });
  ...
  return {
    ...
    body,
    url: finalURL,
    urlRecord: response.context.finalURL,
