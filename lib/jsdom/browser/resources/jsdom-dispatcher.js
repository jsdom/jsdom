  ...
  async #dispatchHTTP(urlRecord, handler, opts) {
    ...
    function onControllerReady(controller) {
      ...
      if (!onRequestStartCalled) {
        ...
        handler.onRequestStart?.(proxyController, { history });
      }
    }
...
async function fetchCollected(dispatcher, { url, headers, signal, element = null }) {
  ...
  // History contains URL records; serialize when reading
  const historyEntry = response.context?.history?.[response.context.history.length - 1];
  const finalURL = historyEntry ? serializeURL(historyEntry) : url;
