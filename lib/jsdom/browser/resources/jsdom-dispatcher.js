...
class JSDOMDispatcher extends Dispatcher {
  ...
  async #dispatchHTTP(opts, handler, url) {
    ...
    if (preflight) {
      ...
      const acao = this.#getHeader(preflightResult.headers, "access-control-allow-origin");
      if (acao !== "*" && acao !== origin) {
        handler.onResponseError?.(null, new Error("Cross origin " + origin + " forbidden"));
        return;
      }
      if (withCredentials) {
        const acac = this.#getHeader(preflightResult.headers, "access-control-allow-credentials");
        if (acac !== "true") {
      ...
      const acahStr = this.#getHeader(preflightResult.headers, "access-control-allow-headers");
    ...
    for (let redirectCount = 0; redirectCount <= MAX_REDIRECTS; redirectCount++) {
      ...
      // Handle 401 auth challenge (generic HTTP behavior)
      if (result.status === 401 && auth && !authSent) {
        const wwwAuth = this.#getHeader(result.headers, "www-authenticate") || "";
      ...
      // Handle redirect
      const location = this.#getHeader(result.headers, "location");
      if (result.status >= 300 && result.status < 400 && location) {
        ...
        const methodChange = (result.status === 303 && !["GET", "HEAD"].includes(currentMethod)) ||
          ((result.status === 301 || result.status === 302) && currentMethod === "POST");
        if (methodChange) {
          currentMethod = "GET";
          currentBody = undefined;
          // Remove content headers when body is nullified
          ...
          delete currentHeaders["content-encoding"];
          delete currentHeaders["Content-Language"];
          delete currentHeaders["content-language"];
          delete currentHeaders["Content-Location"];
          delete currentHeaders["content-location"];
        }

        // Authorization header removal on cross-origin redirect (generic HTTP)
        ...
        if (corsMode || targetIsCrossOrigin) {
          ...
          if (origin !== sourceOrigin && !sourceURL.startsWith("data:")) {
            const acao = this.#getHeader(result.headers, "access-control-allow-origin");
            if (acao !== "*" && acao !== origin) {
              handler.onResponseError?.(null, new Error("Cross origin " + origin + " forbidden"));
              return;
            }
            if (withCredentials) {
              const acac = this.#getHeader(result.headers, "access-control-allow-credentials");
              if (acac !== "true") {
                handler.onResponseError?.(null, new Error("Credentials forbidden"));
                return;
              }
  ...
  /**
   * Perform a single HTTP request (no redirects). Matches Fetch spec "HTTP-network-or-cache fetch".
  ...
  async #doSingleRequest(url, method, headers, body, opaque, signal) {
    ...
    const isPreflight = method === "OPTIONS" &&
      this.#hasHeaderCaseInsensitive(headers, "Access-Control-Request-Method");
  ...
  /**
   * Get a header value (case-insensitive), trimmed.
   */
