    ...
    for (let redirectCount = 0; redirectCount <= MAX_REDIRECTS; redirectCount++) {
      ...
      if (result.status >= 300 && result.status < 400 && location) {
        ...
        if (corsMode || targetIsCrossOrigin) {
          ...
          if (origin !== currentOrigin && !currentURL.startsWith("data:")) {
            ...
            if (currentOrigin !== targetOrigin && currentOrigin !== origin) {
      ...
      if (origin !== undefined && origin !== currentOrigin && !currentURL.startsWith("data:")) {
        const acao = result.headers["access-control-allow-origin"];
