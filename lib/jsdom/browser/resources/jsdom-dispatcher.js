...
//
// It also fixes undici's header value encoding: undici decodes header values as UTF-8,
// but HTTP headers must use "isomorphic decode" per RFC 7230 and the Fetch spec.
// See https://github.com/nodejs/undici/issues/4753. Fixed in an unreleased undici version, so this
 ...
 * @returns {Object} Object with lowercase keys and correctly decoded string values
 */
function rawHeadersToObject(rawHeaders) {
  ...
  for (let i = 0; i < rawHeaders.length; i += 2) {
    ...
    const key = rawHeaders[i].toString("latin1").toLowerCase();
