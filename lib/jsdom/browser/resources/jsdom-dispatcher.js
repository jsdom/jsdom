...
class JSDOMDispatcher {
  ...
  dispatch(opts, handler) {
    ...
    if (url.startsWith("data:")) {
      return this.#dispatchDataURL(url, handler);
    }

    if (url.startsWith("file:")) {
      return this.#dispatchFileURL(url, handler, opts.signal);
    }
  ...
  async #dispatchHTTPWithoutRedirects(opts, handler) {
    ...
    innerDispatch(dispatchOpts, {
      onRequestStart(controller, context) {
        wrappedHandler.onRequestStart?.(controller, { ...context, history: [url] });
      },
