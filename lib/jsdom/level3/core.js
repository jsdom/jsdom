...
/**
* Return a bitmask of the above constants depending on the relative position of this node and (other)
* according to the rules defined in
*/
var newCompareFn = function compareDocumentPosition (other) {
  ...
  // Recursively walk up parent nodes untill the root node is found
  ...
  //Recursively walk up the tree checking if otherNode is an ancestor of node
  ...
  // Note that this function assumes that on the ORIGINAL call node !== otherNode
...
var oldCompareFn = function compareDocumentPositionOld( otherNode ) {
  ...
  var previous = null
  while( point ) {
    if( point == otherNode ) DOCUMENT_POSITION_PRECEDING + DOCUMENT_POSITION_CONTAINS
    parents.push( point )
...
core.Node.prototype.compareDocumentPosition = oldCompareFn;
core.Node.prototype.compareDocumentPosition = newCompareFn;
/*
    // Introduced in DOM Level 3:
...
core.Node.prototype.contains = function (other) {
  return this.isSameNode(other) || !!(this.compareDocumentPosition(other) & DOCUMENT_POSITION_CONTAINED_BY);
}
