...
function eachContext(nodeMultiSet) {
  ...
  //console.log('eachContext returns ' + r.map(stringifyContext).join('; '));
  return r;
}
...
function followingHelper2(nodeList  /*destructive!*/, nodeTypeNum, nodeName, shouldLowerCase) {
  var matcher = new NodeMatcher(nodeTypeNum, nodeName, shouldLowerCase);
...
function precedingHelper2(nodeList  /*destructive!*/, nodeTypeNum, nodeName, shouldLowerCase) {
  var matcher = new NodeMatcher(nodeTypeNum, nodeName, shouldLowerCase);
...
function descenantHelper(nodeList  /*destructive!*/, nodeTypeNum, nodeName, shouldLowerCase, andSelf) {
  ...
  for (var i = attrNodes.length-1; i >= 0; --i) {
    //console.log('splicing ', attrIndices[i], attrNodes[i]);
...
exp.sortUniqDocumentOrder = function sortUniqDocumentOrder(nodes) {
  var a = [];
...
var axes = exp.axes = {
  ...
  'child':
    function child(nodeList  /*destructive!*/, nodeTypeNum, nodeName, shouldLowerCase) {
      var matcher = new NodeMatcher(nodeTypeNum, nodeName, shouldLowerCase);
      var nodeMultiSet = new NodeMultiSet(false);
      //console.log('child: '+ nodeList.length + ' parents');
      for (var i = 0; i < nodeList.length; ++i) {
        var n = nodeList[i];
        //console.log('child: trying ' + n);
        if (n.ownerElement)  // skip attribute nodes' text child.
          continue;
        if (n.childNodes) {
          nodeMultiSet.pushSeries();
          var childList = 1 === nodeTypeNum && null != n.children ? n.children : n.childNodes;
          //console.log('child: childList has length ' + childList.length);
  ...
  'following':
    function following(nodeList  /*destructive!*/, nodeTypeNum, nodeName, shouldLowerCase) {
      return followingHelper2(nodeList  /*destructive!*/, nodeTypeNum, nodeName, shouldLowerCase);
    },
  'following-sibling':
...
var fn = {
  ...
  'id': function id(object) {
    ...
    if ('object' === typeof object) {
      ...
      ids = [];
      // console.log('id of ' + exp.stringifyContext(object));
    ...
    r.nodes = exp.sortUniqDocumentOrder(r.nodes);
    return r;
  },
...
function comparisonHelper(test, x, y, isNumericComparison) {
  var coersion;
  //console.log('comparison helper: ' + stringifyContext(x) + ' vs ' + stringifyContext(y));
  ...
  if ('object' === typeof x && 'object' === typeof y) {
    ...
    for (var i = 0; i < x.nodes.length; ++i) {
      ...
      for (var j = 0; j < y.nodes.length; ++j) {
        ...
        //console.log('comparing ' + xi + ' vs ' + yj + ' using ' +test);
...
function compareSiblings(a, b) {
  ...
  throw new Error('a and b are not siblings: ' + exp.stringifyContext(a) + ' vs ' + exp.stringifyContext(b));
}
...
function mergeNodeLists(x, y) {
  ...
  while (true) {
    ...
    if (c < 0) {
      ...
      //console.log('can\'t distinguish: ' + stringifyContext(a),'vs',stringifyContext(b));
...
exp.stringifyContext = function stringifyContext(ctx) {
...
Evaluator.prototype = {
  val: function val(ast, ctx) {
    ...
    //console.log('val ' + JSON.stringify(ast) + ' in ctx ' + exp.stringifyContext(ctx));
    ...
    switch (ast[0]) {
      ...
      case 'Predicate':
        ...
        for (var i = 0; i < contexts.length; ++i) {
          ...
          ///console.log('Finished predicate: ' + ast[2] + ' within ' + exp.stringifyContext(singleNodeSet) + ': success=' + success + ' for ' + exp.stringifyContext(rhs));
          if (success) {
...
function evaluateImpl(expr, doc, context) {
  ...
  //console.log('It all Evaluated to  ' + stringifyContext(val));
  return val;
...
function XPathResult(doc, value, resultType) {
  this._value = value;
  this._resultType = resultType;
  this._i = 0;
  this._invalidated = false;
  if (this.resultType === XPathResult.prototype.UNORDERED_NODE_ITERATOR_TYPE ||
      this.resultType === XPathResult.prototype.ORDERED_NODE_ITERATOR_TYPE) {
