...
exports.isArrayBufferDetached = function (value) {
  try {
...
/**
 * Warning: both `resizable` and `maxByteLength` are experimental properties at the moment of writing
 * (March 2023). No support is currently available in Browsers and Node, so this function will always return `false`.
 *
 * There might be another way to determine `maxByteLength`, and therefore `resizable` (it is tied to the previous prop):
 * @see https://stackoverflow.com/a/72124984/2929433
 *
 * @param {ArrayBuffer} buffer
 * @returns {boolean}
 */

exports.isArrayBufferResizable = function (buffer) {
  return (
    (Object.prototype.hasOwnProperty.call(buffer, "resizable") && buffer.resizable) ||
    (Object.prototype.hasOwnProperty.call(buffer, "maxByteLength") && buffer.maxByteLength !== buffer.byteLength)
  );
};

/**
 * Object function returns the object itself if already an object
 * @param {*} value
 * @returns
 */
