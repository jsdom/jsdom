...
core.Node.prototype.isSupported = function(/* string */ feature,
                                           /* string */ version)
{
  if (this._ownerDocument && this._ownerDocument._implementation) {
    for (var i in this._ownerDocument._implementation._features) {
        if (this._ownerDocument._implementation._features.hasOwnProperty(i) &&
            i.toLowerCase() === feature.toLowerCase() &&
            parseFloat(this._ownerDocument._implementation._features[i]) >= version)
        {
          return true;
        }
      }
                                                 ...
                                                 /* string */ value)
{
  ...
  attr.localName = qualifiedName.split(':').pop();
  return attr;
};
                                                     ...
                                                     /* string */ qualifiedName)
{
  ...
  attribute._localName = qualifiedName.split(':').pop();
  return attribute;
};
