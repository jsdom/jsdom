...
cssom.CSSStyleDeclaration.prototype.__defineSetter__('border',function(val) {
if (typeof(this.node)!="undefined")
{
console.log(this.node+' '+val+' ggggggggggggggggggg');
this.setProperty('border',val);
};
});

cssom.CSSStyleDeclaration.prototype.__defineSetter__('borderColor',function(val) {
if (typeof(this.node)!="undefined")
{
console.log(this.node+' '+val+' ggggggggggggggggggg');
this.setProperty('border-color',val);
};
});
...
function evaluateStyleAttribute(data) {
  ...
  while (style.length > 0) {
	...
	//modif
	// transformer les styles crees en js (getcomputed ?)
	//modif
  }
  if (styleSheet.cssRules.length > 0 && styleSheet.cssRules[0].style) {
    ...
    for (var i = 0; i < newStyle.length; ++i) {
      var prop = newStyle[i];
	  console.log( prop+' '+newStyle.getPropertyValue(prop)+' '+newStyle.getPropertyPriority(prop));
      style.setProperty(
          prop,
  	...
  	//Modif
	//if (!this.attributes.exist('style'))
	//{
	//this.setAttribute('style',this._cssStyleDeclaration); //Add to attributes list - case of style created in js
	//}
	//Modif
}
html.HTMLElement.prototype.__defineGetter__('style', function() {
  if (!this._cssStyleDeclaration) {
	...
	this._cssStyleDeclaration.node=this;
	...
	//evaluateStyleAttribute.call(this,"width:400;height:400;border:1px solid red;background-color:white");
  }
    
  //console.log(this._cssStyleDeclaration.border);
...
try {var tmp=this.style;} catch(ee) {};
