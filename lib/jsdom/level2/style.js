...
StyleAttr.prototype = {
  get nodeValue() {
    return this._node._undigestedStyle || this._node.style.cssText;
  },
  set nodeValue(value) {
    this._node._undigestedStyle = value;
    delete this._node._cssStyleDeclaration;
  }
};
...
html.HTMLElement.prototype.__defineGetter__('style', function() {
  ...
  if (!style) {
    ...
    var styleSheet = cssom.parse('dummy{' + this._undigestedStyle + '}');
    while (style.length) {
