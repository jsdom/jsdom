...
//modifew
  push: function(callback) {
	...
	if (data)
	{
	//data not empty, execute it if readyState complete or if data is end and queue has only one element(fire onload for doc and set readystate complete)

	if ((that._doc.readyState=='complete')||((data=='end')&&(that.tail.length==1)))
	{
    item.check();
	}
	} else {
