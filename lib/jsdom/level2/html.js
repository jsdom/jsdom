...
// Factory for HTMLDocuments. Sets the innerHTML and dispatches a load event.
core.createDocument = function(options) {
  options = options || {};
  
  var doc = new core.HTMLDocument(options);
  doc.innerHTML = options.html || '<html><head></head><body></body></html>';
  ...
  if (doc.createEvent) {
    ...
    // Dispatch load event once all resources are loaded
    core.resourceLoader.enqueue(doc, function() {
      doc.readyState = 'complete';
      var loadEvent = doc.createEvent('HTMLEvents');
...
core.HTMLDocument.prototype = {
  ...
  get documentElement() {
    var html = this._documentElement;
    if (!html) {
      html = firstChild(this, 'HTML');
      if (!html) {
        html = this.createElement('HTML');
        this._childNodes = [html];
      }
      this._documentElement = html;
    } 
    return html;
  },
    ...
    setAttribute: function(name, value) {
      ...
      if (name === "src") {
        ...
        core.resourceLoader.load(this, value, function(html, filename) {
          this._contentDocument = core.createDocument({
            url: value,
