...
core.resourceLoader = {
  load: function(element, href, callback) {
    var ownerImplementation = element.ownerDocument.implementation, url;
  ...
  proto : {
    ...
    dispatchEvent : function (event) {
      ...
      if (outcome                    &&
          ...
          event.target._eventDefault &&
          typeof event.target._eventDefault === 'function')
      {
        event.target._eventDefault(event)
      }
      return outcome;
    },
    _eventDefault : null
...
define('HTMLScriptElement', {
  ...
  proto: {
    _eval: function(text, filename) {
      if (this.ownerDocument.implementation.hasFeature("ProcessExternalResources", "script") &&
          this.language                                                                      &&
    ...
    get text() {
      var i=0, children = this.children, l = children.length, ret = [];
