...
define('HTMLCanvasElement', {
  ...
  elementBuilder: function (element) {
    ...
    try {
      var Element = Object.getPrototypeOf( element );
      var canvas = new (require('canvas'))(0,0);
      canvas.__proto__.__proto__.__proto__ = Element;
      for (var attr in element) {
        if (element.hasOwnProperty(attr) && !canvas[attr]) {
          canvas[attr] = element[attr];
        }
      }
      var canvasSetter = function(name, value){
        if(name == 'width' || name == 'height') {
          this[ name ] = parseInt(value);
        }
      };
      canvas.__proto__._setAttributeNoValidation = function(name, value) {
        canvasSetter.call(this, name, value);
        Element._setAttributeNoValidation.call(canvas, name, value);
      };
      canvas.__proto__.setAttribute = function(name, value) {
        canvasSetter.call(this, name, value);
        Element.setAttribute.call(canvas, name, value);
      };
      return canvas;
    } catch (e) {
      return element;
