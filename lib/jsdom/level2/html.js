...
RequestManager.prototype = {
  ...
  close: function() {
    for (const openedRequest of this.openedRequests) {
      openedRequest.abort(false);
    }
    this.openedRequests = [];
  },
  stop: function() {
    for (const openedRequest of this.openedRequests) {
      openedRequest.abort(true);
    }
    this.openedRequests = [];
  },
  size: function() {
...
core.HTMLDocument = function HTMLDocument(options) {
  ...
  this._requestManager = new RequestManager(options);
...
function loadFrame (frame) {
  ...
  var urlObj = url.match(/(.+?):(.*)/);
  var wnd = new Window({
    parsingMode: 'html',
    url: urlObj[1] === 'javascript' || url === 'about:blank' ? parentDoc.URL : url,
  ...
  } else if(urlObj && urlObj[1] === 'javascript') {
    // Cannot be done inside the enqueued callback; the documentElement etc. need to be immediately available.
    contentDoc.write("<html><head></head><body></body></html>");
    contentDoc.close();
    contentWindow.eval(urlObj[2]);
    resourceLoader.enqueue(frame, undefined, function () { })(); // to fire the load event
  } else {
...
define('HTMLTemplateElement', { tagName: 'TEMPLATE' });
