...
core.resourceLoader = {
  load: function(element, href, callback) {
    var ownerDoc = element._ownerDocument,
        ownerImplementation = ownerDoc.implementation;
    ...
    if (ownerImplementation._hasFeature('FetchExternalResources', element.tagName.toLowerCase())) {
      ...
      var cookie = ownerDoc._cookie,
          cookieDomain = ownerDoc._cookieDomain,
          baseUrl = this.baseUrl(ownerDoc),
          enqueued = this.enqueue(element, callback, full);

      if (typeof ownerDoc._resourceLoader == "function") {
        ownerDoc._resourceLoader.call(this, url, cookie, cookieDomain, baseUrl, enqueued);
      } else {
