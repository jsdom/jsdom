...
core.resourceLoader = {
  load: function(element, href, callback) {
    ...
    if (ownerImplementation.hasFeature('FetchExternalResources', element.tagName.toLowerCase())) {
      ...
      if (url.hostname) {
        this.download(url, element._ownerDocument._cookie, element._ownerDocument._cookie_domain, this.baseUrl(element._ownerDocument), this.enqueue(element, callback, full));
      }
      else {
  ...
  download: function(url, cookie, cookie_domain, referrer, callback) {
    ...
    if (cookie) {
      ...
      if (host.indexOf(cookie_domain, host.length - cookie_domain.length) !== -1) {
    ...
    request.on('response', function (response) {
      ...
      function success () {
        if ([301, 302, 303, 307].indexOf(response.statusCode) > -1) {
          ...
          core.resourceLoader.download(URL.parse(redirect), cookie, cookie_domain, referrer, callback);
        } else {
          callback(null, data);
        }
      }
...
core.HTMLDocument = function HTMLDocument(options) {
  ...
  this._cookie_domain = options.cookie_domain || '127.0.0.1';
