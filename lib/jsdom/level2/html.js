...
function define(elementClass, def) {
  ...
  var elem = core[elementClass] = function(document, name) {
    if (parentClass.setup) {
      parentClass.setup(this, [], { ownerDocument: document, localName: name || tagName.toUpperCase() });
    } else {
  ...
  if (parentClass.setup) {
    inheritFrom(parentClass.interface, elem, proto);
  } else {
  ...
  tagNames.forEach(function(tag) {
    DocumentImpl.implementation.prototype._elementBuilders[tag.toLowerCase()] = function(doc, s) {
...
const ElementImpl = require("../living/nodes/Element-impl").implementation;
define('HTMLElement', {
  parentClass: Element,
  init: function () {
  ...
  proto : {
    ...
    _attrModified: function (name, value, oldValue) {
      ...
      ElementImpl.prototype._attrModified.apply(idlUtils.implForWrapper(this), arguments);
    }
  },
  attributes: [
    'id',
