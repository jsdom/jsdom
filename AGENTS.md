...
Follow the full [AI-assisted contribution policy](Contributing.md#ai-assisted-contributions).

- Add `Co-authored-by: Agent Name <agent-email>` to every commit containing material generated by an AI agent. For Codex, use `Co-authored-by: codex <codex@openai.com>`.
- Keep pull request descriptions brief. State the human contributor's concrete real-world motivation.
- Run the relevant web platform tests. If hosts file access prevents this, stop and ask the human contributor to run them before opening a pull request.
- Never invent motivations, test results, specification support, or citations.
